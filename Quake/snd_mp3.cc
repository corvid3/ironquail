...
static int
mp3_inputdata(snd_stream_t* stream)
{
  ...
  bytes_read = FS_fread(
    p->mp3_buffer + remaining, 1, MP3_BUFFER_SIZE - remaining, &stream->fh);
  if (bytes_read == 0)
    return -1;
...
static int
mp3_startread(snd_stream_t* stream)
{
  mp3_priv_t* p = (mp3_priv_t*)stream->priv;
  size_t ReadSize;
...
static int
mp3_decode(snd_stream_t* stream, byte* buf, int len)
{
  mp3_priv_t* p = (mp3_priv_t*)stream->priv;
  int donow, i, done = 0;
  mad_fixed_t sample;
...
static int
mp3_stopread(snd_stream_t* stream)
{
  mp3_priv_t* p = (mp3_priv_t*)stream->priv;
...
static int
mp3_madseek(snd_stream_t* stream, unsigned long offset)
{
  mp3_priv_t* p = (mp3_priv_t*)stream->priv;
  ...
  while (1) /* Read data from the MP3 file */
  {
    ...
    depadded = true;
    mad_stream_buffer(
      &p->Stream, p->mp3_buffer + padding, leftover + bytes_read - padding);
...
static qboolean
S_MP3_CodecInitialize(void)
{
  return true;
}

static void
S_MP3_CodecShutdown(void)
{
}

static qboolean
S_MP3_CodecOpenStream(snd_stream_t* stream)
{
  int err;
...
static int
S_MP3_CodecReadStream(snd_stream_t* stream, int bytes, void* buffer)
{
...
static void
S_MP3_CodecCloseStream(snd_stream_t* stream)
{
  mp3_stopread(stream);
  free(stream->priv);
...
static int
S_MP3_CodecRewindStream(snd_stream_t* stream)
{
  /*
  mp3_stopread(stream);
  FS_rewind(&stream->fh);
