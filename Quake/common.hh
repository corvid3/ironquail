...
void
SZ_Print(sizebuf_t* buf, const char* data); // strcats onto the sizebuf
...
#define PTR_IN_RANGE(ptr, begin, end)                                          \
...
#define VEC_POP(v)                                                             \
  do {                                                                         \
    SDL_assert(v&& VEC_HEADER(v).size >= 1);                                   \
    VEC_HEADER(v).size--;                                                      \
  } while (0)
#define VEC_POP_N(v, n)                                                        \
  do {                                                                         \
    SDL_assert(v&& VEC_HEADER(v).size >= (n));                                 \
    VEC_HEADER(v).size -= (n);                                                 \
  } while (0)
...
GetBit(const uint32_t* arr, uint32_t i)
{
  return (arr[i / 32u] & (1u << (i % 32u))) != 0u;
}

static inline void
SetBit(uint32_t* arr, uint32_t i)
{
  arr[i / 32u] |= 1u << (i % 32u);
}

static inline void
ClearBit(uint32_t* arr, uint32_t i)
{
  arr[i / 32u] &= ~(1u << (i % 32u));
}

static inline void
ToggleBit(uint32_t* arr, uint32_t i)
{
  arr[i / 32u] ^= 1u << (i % 32u);
}
...
void
MSG_WriteAngle16(sizebuf_t* sb, float f, unsigned int flags); // johnfitz
...
float
MSG_ReadAngle16(unsigned int flags); // johnfitz
...
extern int
q_snprintf(char* str, size_t size, const char* format, ...) FUNC_PRINTF(3, 4);
extern int
...
char*
va(const char* format, ...) FUNC_PRINTF(1, 2);
// does a varargs printf into a temp buffer

char*
COM_TintSubstring(const char* in,
                  const char* substr,
                  char* out,
                  size_t outsize);
char*
