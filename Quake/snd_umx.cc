...
static int
get_objtype(fshandle_t* f, int32_t ofs, int type)
{
  char sig[16];
_retry:
...
static int
read_typname(fshandle_t* f, const struct upkg_hdr* hdr, int idx, char* out)
{
  int i, s;
  long l;
  char buf[64];
...
static int32_t
probe_header(fshandle_t* f, struct upkg_hdr* hdr)
{
...
static int
process_upkg(fshandle_t* f, int32_t* ofs, int32_t* objsize)
{
...
static qboolean
S_UMX_CodecInitialize(void)
{
  return true;
}

static void
S_UMX_CodecShutdown(void)
{
}

static qboolean
S_UMX_CodecOpenStream(snd_stream_t* stream)
{
  int type;
...
static int
S_UMX_CodecReadStream(snd_stream_t* stream, int bytes, void* buffer)
{
  return -1;
}

static void
S_UMX_CodecCloseStream(snd_stream_t* stream)
{
  S_CodecUtilClose(&stream);
}

static int
S_UMX_CodecRewindStream(snd_stream_t* stream)
{
  return -1;
}
