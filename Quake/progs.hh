...
#include "common.hh"
#include "pr_comp.hh"  /* defs shared with qcc */
#include "progdefs.hh" /* generated by program cdefs */
#include "protocol.hh"
...
{
  qboolean free; /* don't modify directly, use
                    ED_AddToFreeList/ED_RemoveFromFreeList */
  link_t freechain;
  ...
  qboolean sendforcewater; /* waterlevel override has changed and needs to be
  ...
  int showbboxframe; /* last render frame this entity was highlighted by
                        r_showbboxes & co */
...
typedef struct qcvm_s
{
  ...
  int effects_mask; // only enable 2021 rerelease quad/penta dlights when
                    // applicable
...
int
PR_FindExtensionByName(const char* name); // returns known extension index, or 0
                                          // for unrecognized extensions

void
PR_ReloadPics(qboolean purge); // for gamedir or video changes
...
int
PR_AllocString(int bufferlength, char** ptr);
...
FUNC_NORETURN void
PR_RunError(const char* error, ...) FUNC_PRINTF(1, 2);
#ifdef __WATCOMC__
