...
static int hipweapons[4] = { HIT_LASER_CANNON_BIT,
                             HIT_MJOLNIR_BIT,
                             4,
...
Sbar_DrawPicAlpha(int x, int y, qpic_t* pic, float alpha)
{
  ...
  Draw_Pic(x, y + 24, pic);
  GL_SetCanvasColor(
    1, 1, 1, 1); // ericw -- changed from glColor3f to work around intel 855 bug
...
*/
void
Sbar_DrawScrollString(int x, int y, int width, const char* str)
{
  int len, ofs;
...
Sbar_DrawNum(int x, int y, int num, int digits, int color)
{
  ...
  num = q_min(999,
              num); // johnfitz -- cap high values rather than truncating number
...
*/
void
Sbar_DrawSmallNum(int x, int y, int num)
{
  char str[12];
  int i;
...
Sbar_SoloScoreboard(void)
{
  ...
  sprintf(
    str, "Kills: %i/%i", cl.stats[STAT_MONSTERS], cl.stats[STAT_TOTALMONSTERS]);
  left = 8 + strlen(str) * 8;
  Sbar_DrawString(8, 12, str);

  sprintf(
    str, "Secrets: %i/%i", cl.stats[STAT_SECRETS], cl.stats[STAT_TOTALSECRETS]);
...
*/
int
Sbar_MiniScoreboardSizeCheck(void)
{
  float scale;
  ...
  if (scr_viewsize.value < 110) {
    ...
    for (i = 0; i < 7; i++) {
      if (cl.items & (IT_SHOTGUN << i)) {
        ...
        Sbar_DrawPic(24,
                     -69 - scoreboard_y_gap - (16 * (7 + extraguns)) + i * 16,
      ...
      for (i = 0; i < 4; i++) {
        if (cl.items & (1 << hipweapons[i])) {
          ...
          if (i == 2) {
            ...
            Sbar_DrawPic(24,
                         -69 - scoreboard_y_gap - (16 * 9) + (7 + i) * 16,
    ...
    for (i = 0; i < 4; i++) {
      Sbar_DrawSmallNum(
        13, -69 - scoreboard_y_gap + (i * 11), cl.stats[STAT_SHELLS + i]);
    }
...
Sbar_DrawInventory2(void)
{
  ...
  if (scr_viewsize.value < 110) {
    ...
    y = (int)(LERP(glcanvas.top, glcanvas.bottom - 148, 0.5f) +
              ROW_HEIGHT * 7 * 0.5f + 0.5f);
  ...
  if (scr_viewsize.value < 110) {
    ...
    if (hudstyle == HUD_MODERN_SIDEAMMO ||
        hudstyle == HUD_QUAKEWORLD) // right side, 2x2
    {
      ...
      for (i = 0; i < 4; i++)
        Sbar_DrawSmallNum(x + 11 + ITEM_WIDTH * (i & 1),
                          y - 10 * (i >> 1),
  ...
  // items
  if (scr_viewsize.value < 110 &&
      (hudstyle == HUD_MODERN_SIDEAMMO || hudstyle == HUD_QUAKEWORLD)) {
...
Sbar_DrawFragsQW(void)
{
  ...
  for (i = 0, x = -88 + ((4 - numscores) * 32); i < numscores;
       i++, x += 32) // fill from the right
  {
    s = &cl.scores[fragsort[i]];
    if (!s->name[0])
      continue;
...
Sbar_Draw(void)
{
  ...
  if (sb_lines && glwidth > w) {
    ...
    if (cl.gametype == GAME_DEATHMATCH)
      ...
      Draw_TileClear((glwidth - w) / 2.0f + w,
                     glheight - sb_lines,
                     (glwidth - w) / 2.0f,
                     sb_lines);
    }
  ...
  if (hudstyle == HUD_CLASSIC || hudstyle == HUD_QUAKEWORLD) {
    ...
    } else // qw hud
    {
      ...
      if (cl.maxclients != 1 &&
          scr_viewsize.value <
            110) // qw hides frag count if MiniDeathmatchOverlay is showing
      {
    ...
    if (sb_showscores || cl.stats[STAT_HEALTH] <= 0) {
      Sbar_DrawPicAlpha(
        0, 0, sb_scorebar, scr_sbaralpha.value); // johnfitz -- scr_sbaralpha
      Sbar_DrawScoreboard();
      sb_updates = 0;
               ...
               120) // johnfitz -- check viewsize instead of sb_lines
    {
      if (hudstyle == HUD_CLASSIC)
        Sbar_DrawPicAlpha(
          0, 0, sb_sbar, scr_sbaralpha.value); // johnfitz -- scr_sbaralpha
               ...
               120) // johnfitz -- check viewsize instead of sb_lines
    {
      ...
      Sbar_DrawNum(
        x + 32, y, cl.stats[STAT_HEALTH], 3, cl.stats[STAT_HEALTH] <= 25);
      ...
      Sbar_DrawNum(
        x - 48, y, cl.stats[STAT_AMMO], 3, cl.stats[STAT_AMMO] <= 10);
...
*/
qpic_t*
Sbar_IntermissionPicForChar(char c, int color)
{
  if ((unsigned)(c - '0') < 10)
...
*/
int
Sbar_IntermissionTextWidth(const char* str, int color)
{
  int len = 0;
  while (*str) {
...
*/
void
Sbar_IntermissionText(int x, int y, const char* str, int color)
{
  while (*str) {
    qpic_t* pic = Sbar_IntermissionPicForChar(*str++, color);
    if (!pic)
...
#if 0
{
    ...
    M_Print(
      x + 64,
      y,
      s->name); // johnfitz -- was Draw_String, changed for stretched overlays
