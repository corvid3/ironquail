...
==================
*/
static filelist_item_t*
FileList_AddWithData(const char* name,
                     const void* data,
                     size_t datasize,
                     filelist_item_t** list)
{
  ...
  if (!item)
    Sys_Error("FileList_AddWithData: out of memory on %" SDL_PRIu64
              " bytes (%s)",
              (uint64_t)(sizeof(filelist_item_t) + datasize),
              name);
...
*/
static void
FileList_Add(const char* name, filelist_item_t** list)
{
...
*/
static void
FileList_Clear(filelist_item_t** list)
{
...
*/
static const char*
RightPad(const char* str, size_t minlen, char c)
{
...
*/
static void
FileList_Print(filelist_item_t* list, const char* types[2], const char* substr)
{
  int i;
...
*/
static maptype_t
ExtraMaps_Categorize(const char* name, const searchpath_t* source)
{
  ...
  if (source->path_id != com_searchpaths.front().path_id) {
    if (is_start)
...
==================
*/
static const levelinfo_t*
ExtraMaps_GetInfo(const filelist_item_t* item)
{
...
*/
const char*
ExtraMaps_GetMessage(const filelist_item_t* item)
{
  const levelinfo_t* info = ExtraMaps_GetInfo(item);
...
*/
static void
ExtraMaps_WaitForParsingThread(void)
{
...
ExtraMaps_Init(void)
{
  ...
  for (auto const& search : com_searchpaths) {
    ...
    {
      qboolean const isbase = search.pack->filename.contains(ignorepakdir);
      for (i = 0, pak = search.pack.get(); i < pak->numfiles; i++) {
...
static qboolean
Download(const char* url, download_t* download)
{
#ifdef WITHOUT_CURL
...
modstatus_t
Modlist_GetStatus(const filelist_item_t* item)
{
  const modinfo_t* info = (const modinfo_t*)(item + 1);
...
float
Modlist_GetDownloadProgress(const filelist_item_t* item)
{
  const modinfo_t* info = (const modinfo_t*)(item + 1);
...
double
Modlist_GetDownloadSize(const filelist_item_t* item)
{
  const modinfo_t* info = (const modinfo_t*)(item + 1);
...
static void
Modlist_RegisterAddons(void* param)
{
...
static void
Modlist_PrintJSONHTTPError(void* param)
{
...
static void
Modlist_PrintJSONCurlError(void* param)
{
  Con_Printf("Failed to download add-on list (%s)\n", (const char*)param);
}

static const char*
Modlist_GetInstallDir(void)
{
...
Modlist_DownloadJSON(__attribute__((unused)) void* unused)
{
  ...
  if ((size_t)q_snprintf(
        url, sizeof(url), "%s/" ADDON_MANIFEST_FILE, extramods_addons_url) >=
      sizeof(url))
    return 1;
  ...
  // cached
  if ((size_t)q_snprintf(
        cacheurlpath, sizeof(cacheurlpath), "%s/addons.url.dat", basedir) >=
      sizeof(cacheurlpath))
    cacheurlpath[0] = '\0';
  else {
    char* cachedurl =
  ...
  if ((size_t)q_snprintf(
        cachepath, sizeof(cachepath), "%s/addons.json", installdir) >=
      sizeof(cachepath))
    cachepath[0] = '\0';
  ...
  if (json && cachepath[0] && cacheurlpath[0]) {
    ...
    COM_WriteFile_OSPath(
      cacheurlpath, extramods_addons_url, strlen(extramods_addons_url));
  }
...
static size_t
WriteModChunk(void* buffer, size_t size, size_t nmemb, void* stream)
{
...
static void
Modlist_FinishInstalling(void* param)
{
...
static void
Modlist_OnInstallHTTPError(void* param)
{
  uintptr_t status = (uintptr_t)param;
  Modlist_FinishInstalling(NULL);
...
static void
Modlist_OnInstallCurlError(void* param)
{
  Modlist_FinishInstalling(NULL);
...
static int
Modlist_InstallerThread(void* param)
{
  char url[MAX_URL];
  char tmp[MAX_OSPATH];
...
qboolean
Modlist_StartInstalling(const filelist_item_t* item)
{
  const char* desc;
  double size;
...
qboolean
Modlist_IsInstalling(void)
{
  return extramods_install_thread != NULL;
}

static void
Modlist_Add(const char* name)
{
  filelist_item_t* item;
  modinfo_t* info;
  int i;
  ...
  if (info->full_name.empty()) {
    ...
    if (mapdb) {
      qboolean is_base_mapdb =
        !com_searchpaths.empty() || path_id < com_searchpaths.front().path_id;
...
static qboolean
Modlist_Check(const char* modname, const char* base)
{
...
Modlist_FindOnline(void)
{
  ...
  if (p && p < com_argc - 1)
    q_strlcpy(
      extramods_addons_url, com_argv[p + 1], sizeof(extramods_addons_url));
  else
    q_strlcpy(
      extramods_addons_url, DEFAULT_ADDON_SERVER, sizeof(extramods_addons_url));
...
qboolean
Modlist_IsInstalled(const char* game)
{
  filelist_item_t* item;
...
DemoList_Init(void)
{
  ...
  for (auto const& search : com_searchpaths) {
    ...
    {
      if (!search.pack->filename.contains(
...
SaveList_Init(void)
{
  ...
  if ((size_t)q_snprintf(dirname, sizeof(dirname), "%s/autosave", com_gamedir) <
    ...
    for (find = Sys_FindFirst(dirname, "sav"); find;
      ...
      if ((size_t)q_snprintf(
            savename, sizeof(savename), "autosave/%s", filename) <
          sizeof(savename))
        FileList_Add(savename, &savelist);
    }
  }
...
static qboolean
SkyList_AddFile(const char* path)
{
...
static void
SkyList_AddDirRec(const char* root, const char* relpath)
{
  findfile_t* find;
...
Host_Restart_f(void)
{
  ...
  q_strlcpy(
    mapname, sv.name, sizeof(mapname)); // mapname gets cleared in spawnserver
  PR_SwitchQCVM(&sv.qcvm);
...
*/
void
Host_SavegameComment(char text[SAVEGAME_COMMENT_LENGTH + 1])
{
  int i;
  char* levelname;
  char kills[20];
...
static void
Host_InvalidateSave(const char* relname)
{
...
static int
Host_BackgroundSave(void* param)
{
...
Host_Spawn_f(void)
{
  ...
  for (i = 0; i < 2; i++)
    if (sv.loadgame)
      MSG_WriteAngle(
        &host_client->message, ent->v.v_angle[i], sv.protocolflags);
    else
...
static void
PrintFrameName(qmodel_t* m, int frame)
{
  aliashdr_t* hdr;
