...
R_InitParticles(void)
{
  ...
  Cvar_SetCallback(&r_particles, (cvarcallback_t)R_SetParticleTexture_f);
...
R_FlushParticleBatch(void)
{
  ...
  GL_Upload(GL_ARRAY_BUFFER,
            partverts,
            sizeof(partverts[0]) * numpartverts,
            &buf,
            &ofs);
...
/*
===============
R_DrawParticles_Real -- johnfitz -- moved all non-drawing code to
CL_RunParticles
===============
*/
static void
R_DrawParticles_Real(qboolean alpha, qboolean showtris)
{
  particle_t* p;
  particlevert_t* v;
  GLubyte color[4] = { 255, 255, 255, 255 },
          *c;                // johnfitz -- particle transparency
  ...
  // this bakes in the additional scaling of vup and vright by 1.5f for
  // billboarding, then down by 0.25f for quad particles
  scalex = scaley = texturescalefactor * 0.375f;
  // projection factors (see GL_FrustumMatrix), negated to make things easier in
  // the shader
