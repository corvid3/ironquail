...
qboolean bgmloop;
cvar_t bgm_extmusic = { "bgm_extmusic", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
...
static void
BGM_Play_noext(const char* filename, unsigned int allowed_types)
{
  char tmp[MAX_QPATH];
  ...
  while (handler) {
    ...
    q_snprintf(
      tmp, sizeof(tmp), "%s/%s.%s", handler->dir, filename, handler->ext);
...
BGM_PlayCDtrack(byte track, qboolean looping)
{
  ...
  if (ext == NULL)
    ...
    q_snprintf(
      tmp, sizeof(tmp), "%s/track%02d.%s", MUSIC_DIRNAME, (int)track, ext);
