...
static int
ovc_fclose(__attribute__((unused)) void* f)
{
  return 0; /* we fclose() elsewhere. */
}

static int
ovc_fseek(void* f, ogg_int64_t off, int whence)
{
  if (f == NULL)
    return (-1);
...
static qboolean
S_VORBIS_CodecInitialize(void)
{
  return true;
}

static void
S_VORBIS_CodecShutdown(void)
{
}

static qboolean
S_VORBIS_CodecOpenStream(snd_stream_t* stream)
{
  ...
  if (res != 0) {
    Con_Printf(
      "%s is not a valid Ogg Vorbis file (error %i).\n", stream->name, res);
    goto _fail;
  }
...
static int
S_VORBIS_CodecReadStream(snd_stream_t* stream, int bytes, void* buffer)
{
  ...
  while (1) {
     ...
     */
    res = ov_read((OggVorbis_File*)stream->priv,
                  ptr,
                  rem,
...
static void
S_VORBIS_CodecCloseStream(snd_stream_t* stream)
{
...
static int
S_VORBIS_CodecRewindStream(snd_stream_t* stream)
{
