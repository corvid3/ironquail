...
static int
MIK_Seek(MREADER* r, long ofs, int whence)
{
...
static BOOL
MIK_Read(MREADER* r, void* ptr, size_t siz)
{
...
static qboolean
S_MIKMOD_CodecOpenStream(snd_stream_t* stream)
{
  mik_priv_t* priv;
...
static int
S_MIKMOD_CodecReadStream(snd_stream_t* stream, int bytes, void* buffer)
{
  if (!Player_Active())
    return 0;
...
static void
S_MIKMOD_CodecCloseStream(snd_stream_t* stream)
{
  Player_Stop();
...
static int
S_MIKMOD_CodecJumpToOrder(snd_stream_t* stream, int to)
{
...
static int
S_MIKMOD_CodecRewindStream(snd_stream_t* stream)
{
