...
*/
static void
S_MakeBlackmanWindowKernel(float* kernel, int M, float f_c)
{
  int i;
  for (i = 0; i <= M; i++) {
    if (i == M / 2) {
...
static void
S_UpdateFilter(filter_t* filter, int M, float f_c)
{
  if (filter->f_c != f_c || filter->M != M) {
    ...
    if (!filter->memory || !filter->kernel)
      Sys_Error("S_UpdateFilter: out of memory (%" SDL_PRIu64 " bytes)",
                (uint64_t)(filter->kernelsize * sizeof(float)));
...
*/
static void
S_ApplyFilter(filter_t* filter, int* data, int stride, int count)
{
  int i, j;
  size_t inputsize;
  float* input;
...
*/
static void
S_LowpassFilter(int* data, int stride, int count, filter_t* memory)
{
  int M;
  float bw, f_c;
...
S_UnderwaterFilter(int endtime)
{
  ...
  for (i = 0; i < endtime; i++) {
    underwater.accum[0] +=
      underwater.alpha * (paintbuffer[i].left - underwater.accum[0]);
    underwater.accum[1] +=
      underwater.alpha * (paintbuffer[i].right - underwater.accum[1]);
...
S_UpdateLevels(int endtime)
{
  ...
  for (i = 0; i < endtime; i++) {
    float sample =
      (abs(paintbuffer[i].left) + abs(paintbuffer[i].right)) * scale;
...
float
S_GetLoFreqLevel(void)
{
  return snd_lofreqlevel;
}

float
S_GetHiFreqLevel(void)
{
...
static void
SND_PaintChannelFrom8(channel_t* ch,
                      sfxcache_t* sc,
                      int endtime,
                      int paintbufferstart);
static void
SND_PaintChannelFrom16(channel_t* ch,
                       sfxcache_t* sc,
                       int endtime,
                       int paintbufferstart);
