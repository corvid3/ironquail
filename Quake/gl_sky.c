...
*/
void
Sky_LoadTexture(qmodel_t* mod, texture_t* mt)
{
  char texturename[64];
...
*/
void
Sky_LoadTextureQ64(qmodel_t* mod, texture_t* mt)
{
  char texturename[64];
...
Skywind_LookDir_f(void)
{
  ...
  // invert view direction so that clouds move towards the player, not away from
  // them
  skybox->wind_yaw = fmod(cl.viewangles[YAW] + 180.0, 360.0);
  skybox->wind_pitch = -cl.viewangles[PITCH];

  // first argument, if present, overrides the loop duration (default: 30
  // seconds)
  if (Cmd_Argc() >= 2)
  ...
  // second argument, if present, overrides the amplitude of the movement
  // (default: 1.0)
  if (Cmd_Argc() >= 3)
...
Skywind_Rotate_f(void)
{
  ...
  if (Cmd_Argc() >= 3)
    skybox->wind_pitch =
      fmod(skybox->wind_pitch + atof(Cmd_Argv(2)) + 90.0, 180.0) - 90.0;
}
...
Sky_LoadSkyBox(const char* name)
{
  ...
  if (samesize >
      0) // create a single cubemap texture if all faces are the same size
  {
    ...
    for (i = 0; i < 6; i++) {
      ...
      if (srcpixels)
        ...
        memset(dstpixels,
               0,
               numfacebytes); // TODO: average out existing faces instead?
      newsky.cubemap_offsets[i] = dstpixels;
    }

    q_snprintf(filename, sizeof(filename), "gfx/env/%s", name);
    newsky.cubemap = TexMgr_LoadImage(cl.worldmodel,
                                      filename,
                                      samesize,
                                      samesize,
                                      SRC_RGBA,
                                      (byte*)newsky.cubemap_offsets,
                                      "",
                                      (src_offset_t)newsky.cubemap_offsets,
                                      TEXPREF_CUBEMAP | TEXPREF_NOPICMIP |
                                        TEXPREF_MIPMAP | TEXPREF_ALPHA);
  } else // create a separate texture for each side
  {
    for (i = 0; i < 6; i++) {
      q_snprintf(filename, sizeof(filename), "gfx/env/%s%s", name, suf[i]);
      newsky.textures[i] = TexMgr_LoadImage(cl.worldmodel,
                                            filename,
                                            width[i],
                                            height[i],
                                            SRC_RGBA,
...
Sky_Init(void)
{
  ...
  Cvar_SetCallback(&r_skyfog, (cvarcallback_t)R_SetSkyfog_f);
...
*/
void
Sky_EmitSkyBoxVertex(float s, float t, int axis, float* uv, float* pos)
{
  vec3_t v, b;
  int j, k;
  float w, h;
...
Sky_SetupFrame(void)
{
  ...
  float period =
    r_skywind.value && skybox ? skybox->wind_period / r_skywind.value : 0.0;
