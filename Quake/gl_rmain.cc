...
extern byte*
SV_FatPVS(vec3_t org, qmodel_t* worldmodel);
extern qboolean
SV_EdictInPVS(edict_t* test, byte* pvs);
extern qboolean
SV_BoxInPVS(vec3_t mins, vec3_t maxs, byte* pvs, mnode_t* node);
...
cvar_t r_norefresh = { "r_norefreshh", "0", CVAR_NONE, 0, 0, 0, 0, 0 };
...
cvar_t gl_finish = { "gl_finishh", "0", CVAR_NONE, 0, 0, 0, 0, 0 };
...
cvar_t r_showbboxes_health = {
  "r_showbboxes_healthh", "0", CVAR_NONE, 0, 0, 0, 0, 0
...
*/
static GLuint
GL_CreateFBOAttachment(GLenum format,
                       int samples,
                       GLenum filter,
                       const char* name)
{
...
*/
static GLuint
GL_CreateSimpleFBO(GLenum target,
                   GLuint colors,
                   GLuint depth,
                   GLuint stencil,
                   const char* name)
{
  return GL_CreateFBO(
    target, colors ? &colors : NULL, colors ? 1 : 0, depth, stencil, name);
}
...
GL_CreateFrameBuffers(void)
{
  ...
  framebufs.max_samples =
    q_min(framebufs.max_color_tex_samples, framebufs.max_depth_tex_samples);

  /* main framebuffer (color + depth + stencil) */
  framebufs.composite.color_tex =
    GL_CreateFBOAttachment(color_format, 1, GL_NEAREST, "composite colors");
  framebufs.composite.depth_stencil_tex = GL_CreateFBOAttachment(
    depth_format, 1, GL_NEAREST, "composite depth/stencil");
  framebufs.composite.fbo =
    GL_CreateSimpleFBO(GL_TEXTURE_2D,
                       framebufs.composite.color_tex,
  ...
  framebufs.scene.samples =
    CLAMP(1, framebufs.scene.samples, framebufs.max_samples);

  framebufs.scene.color_tex = GL_CreateFBOAttachment(
    color_format, framebufs.scene.samples, GL_NEAREST, "scene colors");
  framebufs.scene.depth_stencil_tex = GL_CreateFBOAttachment(
    depth_format, framebufs.scene.samples, GL_NEAREST, "scene depth/stencil");
  framebufs.scene.fbo = GL_CreateSimpleFBO(
    framebufs.scene.samples > 1 ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D,
    framebufs.scene.color_tex,
    framebufs.scene.depth_stencil_tex,
    framebufs.scene.depth_stencil_tex,
    "scene fbo");

  /* weighted blended order-independent transparency (accum + revealage,
   * potentially multisampled */
  framebufs.oit.accum_tex = GL_CreateFBOAttachment(
    GL_RGBA16F, framebufs.scene.samples, GL_NEAREST, "oit accum");
  framebufs.oit.revealage_tex = GL_CreateFBOAttachment(
    GL_R8, framebufs.scene.samples, GL_NEAREST, "oit revealage");
  framebufs.oit.fbo_scene = GL_CreateFBO(
    framebufs.scene.samples > 1 ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D,
    framebufs.oit.mrt,
    2,
    framebufs.scene.depth_stencil_tex,
  ...
  if (framebufs.scene.samples > 1) {
    framebufs.resolved_scene.color_tex = GL_CreateFBOAttachment(
      color_format, 1, GL_NEAREST, "resolved scene colors");
    framebufs.resolved_scene.fbo =
      GL_CreateSimpleFBO(GL_TEXTURE_2D,
                         framebufs.resolved_scene.color_tex,
                         0,
                         0,
                         "resolved scene fbo");
  } else {
    ...
    framebufs.oit.fbo_composite =
      GL_CreateFBO(GL_TEXTURE_2D,
                   framebufs.oit.mrt,
                   2,
                   framebufs.composite.depth_stencil_tex,
                   framebufs.composite.depth_stencil_tex,
...
*/
qboolean
R_CullBox(vec3_t emins, vec3_t emaxs)
{
  int i;
  mplane_t* p;
  byte signbits;
...
*/
void
R_GetEntityBounds(const entity_t* e, vec3_t mins, vec3_t maxs)
{
...
*/
qboolean
R_CullModelForEntity(entity_t* e)
{
  vec3_t mins, maxs;
...
*/
void
R_EntityMatrix(float matrix[16],
               vec3_t origin,
               vec3_t angles,
               unsigned char scale)
{
...
static int cl_modtype_ofs[mod_numtypes * 2 +
                          1]; // x2: opaque/translucent; +1: total in last slot
...
R_SortEntities(void)
{
  ...
  for (i = 0; i < cl_numvisedicts; i++) {
    ...
    if (translucent && alphasort) {
      ...
      for (j = 0, dist = 0.f; j < 3; j++) {
        delta =
          CLAMP(mins[j], r_refdef.vieworg[j], maxs[j]) - r_refdef.vieworg[j];
        dist += delta * delta;
      }
      dist = sqrt(dist);
      ...
      if (ent->model->type == mod_alias)
        visedict_keys[i] =
          ent->model->sortkey | (ent->skinnum & MODSORT_FRAMEMASK);
      else
        visedict_keys[i] =
          ent->model->sortkey | (ent->frame & MODSORT_FRAMEMASK);
    }
  ...
  // write sorted list
  if (r_drawworld.value)
    cl_sorted_visedicts[typebins[mod_brush * 2 + 0]++] =
      &cl_entities[0]; // add the world as the first brush entity
  for (i = 0; i < cl_numvisedicts; i++) {
...
*/
static void
GL_FrustumMatrix(float matrix[16], float fovx, float fovy, float n, float f)
{
  ...
  if (gl_clipcontrol_able) {
    // reversed Z projection matrix with the coordinate system conversion baked
    // in
    matrix[0 * 4 + 2] = -n / (f - n);
    matrix[0 * 4 + 3] = 1.f;
    matrix[1 * 4 + 0] = -w;
...
*/
void
ExtractFrustumPlane(float mvp[16],
                    int axis,
                    float ndcval,
                    qboolean flip,
                    mplane_t* out)
...
R_SetupGL(void)
{
  if (!GL_NeedsSceneEffects()) {
    ...
    glViewport(0,
               0,
               r_refdef.vrect.width / r_refdef.scale,
               r_refdef.vrect.height / r_refdef.scale);
  }
}
...
*/
void
R_UploadFrameData(void)
{
  GLuint buf;
  GLbyte* ofs;
  size_t size;

  size = sizeof(r_lightbuffer.lightstyles) +
         sizeof(r_lightbuffer.lights[0]) *
           q_max(r_framedata.numlights, 1); // avoid zero-length array
  ...
  GL_BindBufferRange(
    GL_UNIFORM_BUFFER, 0, buf, (GLintptr)ofs, sizeof(r_framedata));
}
...
R_SetupView(void)
{
  ...
  if (r_waterwarp.value) {
    ...
    if (contents == CONTENTS_WATER || contents == CONTENTS_SLIME ||
        contents == CONTENTS_LAVA || cl.forceunderwater) {
...
*/
entity_t**
R_GetVisEntities(modtype_t type, qboolean translucent, int* outcount)
{
...
*/
static void
R_DrawWater(qboolean translucent)
{
  entity_t** entlist = cl_sorted_visedicts;
...
*/
void
R_DrawEntitiesOnList(qboolean alphapass) // johnfitz -- added parameter
{
  int* ofs;
  entity_t** entlist = cl_sorted_visedicts;
  ...
  R_DrawBrushModels(entlist + ofs[2 * mod_brush],
                    ofs[2 * mod_brush + 1] - ofs[2 * mod_brush]);
  R_DrawAliasModels(entlist + ofs[2 * mod_alias],
                    ofs[2 * mod_alias + 1] - ofs[2 * mod_alias]);
  if (!alphapass)
    R_DrawSpriteModels(entlist + cl_modtype_ofs[2 * mod_sprite],
                       cl_modtype_ofs[2 * mod_sprite + 2] -
                         cl_modtype_ofs[2 * mod_sprite]);
...
R_FlushDebugGeometry(void)
{
  if (numdebugverts && numdebugidx) {
    ...
    GL_SetState(GLS_BLEND_ALPHA | GLS_NO_ZTEST | GLS_NO_ZWRITE | GLS_CULL_NONE |
                GLS_ATTRIBS(2));

    GL_Upload(GL_ARRAY_BUFFER,
              debugverts,
              sizeof(debugverts[0]) * numdebugverts,
              &buf,
              &ofs);
...
*/
static void
R_AddDebugGeometry(const debugvert_t verts[],
                   int numverts,
                   const uint16_t idx[],
                   int numidx)
{
...
*/
static void
R_EmitLine(const vec3_t a, const vec3_t b, uint32_t color)
{
  debugvert_t verts[2];
  uint16_t idx[2];
...
*/
static void
R_EmitWirePoint(const vec3_t origin, uint32_t color)
{
...
================
*/
static const uint16_t boxidx[12 * 2] = {
  0, 1, 0, 2, 0, 4, 1, 3, 1, 5, 2, 3, 2, 6, 3, 7, 4, 5, 4, 6, 5, 7, 6, 7,
};

static void
R_EmitWireBox(const vec3_t mins, const vec3_t maxs, uint32_t color)
{
  int i;
  debugvert_t v[8];
...
*/
static void
R_EmitArrow(const vec3_t from, const vec3_t to, uint32_t color)
{
  float frac, len;
...
*/
static void
R_EmitEdictLink(const edict_t* from, const edict_t* to, showbboxflags_t flags)
{
...
static qboolean
R_ShowBoundingBoxesFilter(edict_t* ed)
{
...
edict_t** bbox_linked =
  NULL; // focused edict, followed by edicts linked from/to it
...
*/
static void
R_AddHighlightedEntity(edict_t* ed, showbboxflags_t flags)
{
...
R_ShowBoundingBoxes(void)
{
  ...
  if ((!mode && !r_showfields.value) || cl.maxclients > 1 ||
      !r_drawentities.value || !sv.active)
    return;
  ...
  // Use PVS if r_showbboxes >= 2, or if r_showbboxes is 0 (which means
  // r_showfields is active)
  if (mode >= 2 || mode == 0) {
    vec3_t org;
  ...
  for (i = 1, ed = NEXT_EDICT(qcvm->edicts); i < qcvm->num_edicts;
    ...
    if (r_showbboxes_think.value &&
        (ed->v.nextthink <= 0) == (r_showbboxes_think.value > 0))
      continue;

    if (r_showbboxes_health.value &&
        (ed->v.health <= 0) == (r_showbboxes_health.value > 0))
      continue;
    ...
    if (RayVsBox(r_origin, rcpdelta, mins, maxs, &dist) && dist > 0.f &&
        dist < bestdist) {
      bestdist = dist;
      focused = ed;
    }
  ...
  if (focused && r_showbboxes_links.value) {
    ...
    if ((int)r_showbboxes_links.value & SHOWBBOX_LINK_OUTGOING) {
      for (i = 0; i < qcvm->numentityfields; i++) {
        ...
        if (qcvm->entityfieldofs[i] == offsetof(entvars_t, chain) ||
            !val->edict)
          continue;
    ...
    if ((int)r_showbboxes_links.value & SHOWBBOX_LINK_INCOMING ||
      ...
      for (i = 1, ed = NEXT_EDICT(qcvm->edicts); i < qcvm->num_edicts;
        ...
        if (r_showbboxes_targets.value &&
            (*focus_target || *focus_targetname)) {
...
R_ShowTris(void)
{
  ...
  ofs = cl_modtype_ofs;
  R_DrawBrushModels_ShowTris(entlist + ofs[2 * mod_brush],
                             ofs[2 * mod_brush + 2] - ofs[2 * mod_brush]);
  R_DrawAliasModels_ShowTris(entlist + ofs[2 * mod_alias],
                             ofs[2 * mod_alias + 2] - ofs[2 * mod_alias]);
  R_DrawSpriteModels_ShowTris(entlist + ofs[2 * mod_sprite],
                              ofs[2 * mod_sprite + 2] - ofs[2 * mod_sprite]);
...
R_EndTranslucency(void)
{
  if (r_oit.value) {
    ...
    GL_BindNative(GL_TEXTURE0,
                  framebufs.scene.samples > 1 ? GL_TEXTURE_2D_MULTISAMPLE
                                              : GL_TEXTURE_2D,
                  framebufs.oit.accum_tex);
    GL_BindNative(GL_TEXTURE1,
                  framebufs.scene.samples > 1 ? GL_TEXTURE_2D_MULTISAMPLE
                                              : GL_TEXTURE_2D,
                  framebufs.oit.revealage_tex);
...
R_WarpScaleView(void)
{
  ...
  needwarpscale = r_refdef.scale != 1 || water_warp ||
                  (v_blend[3] && gl_polyblend.value && !softemu);
  ...
  glTexParameteri(GL_TEXTURE_2D,
                  ...
                  water_warp && msaa ? GL_LINEAR : GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D,
