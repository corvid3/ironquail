...
int
UDP_CloseSocket(sys_socket_t socketid)
{
...
int
UDP_Connect(__attribute__((unused)) sys_socket_t socketid,
            __attribute__((unused)) struct qsockaddr* addr)
{
  return 0;
}
...
int
UDP_Read(sys_socket_t socketid, byte* buf, int len, struct qsockaddr* addr)
{
...
static int
UDP_MakeSocketBroadcastCapable(sys_socket_t socketid)
{
  int i = 1;
...
int
UDP_Broadcast(sys_socket_t socketid, byte* buf, int len)
{
  int ret;
...
int
UDP_Write(sys_socket_t socketid, byte* buf, int len, struct qsockaddr* addr)
{
  int ret;

  ret = sendto(
    socketid, buf, len, 0, (struct sockaddr*)addr, sizeof(struct qsockaddr));
  if (ret == SOCKET_ERROR) {
    int err = SOCKETERRNO;
...
int
UDP_GetSocketAddr(sys_socket_t socketid, struct qsockaddr* addr)
{
  socklen_t addrlen = sizeof(struct qsockaddr);
  in_addr_t a;
