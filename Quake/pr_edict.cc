...
*/
static void
PR_HashInit(prhashtable_t* table, int capacity, const char* name)
{
  capacity *= 2; // 50% load factor
  table->capacity = capacity;
  table->strings =
    (const char**)Hunk_AllocName(sizeof(*table->strings) * capacity, name);
  table->indices =
    (int*)Hunk_AllocName(sizeof(*table->indices) * capacity, name);
}
...
*/
static int
PR_HashGet(prhashtable_t* table, const char* key)
{
...
*/
static void
PR_HashAdd(prhashtable_t* table, int skey, int value)
{
...
*/
static void
ED_AddToFreeList(edict_t* ed)
{
  ed->free = true;
  if ((byte*)ed <=
      (byte*)qcvm->edicts + q_max(svs.maxclients, 1) * qcvm->edict_size)
    return;
...
*/
static void
ED_RemoveFromFreeList(edict_t* ed)
{
  ed->free = false;
  if (ed->freechain.prev) {
    RemoveLink(&ed->freechain);
...
ED_Alloc(void)
{
  ...
  if (qcvm->num_edicts ==
      qcvm->max_edicts) // johnfitz -- use sv.max_edicts instead of MAX_EDICTS
    Host_Error("ED_Alloc: no free edicts (max_edicts is %i)", qcvm->max_edicts);

  e = EDICT_NUM(qcvm->num_edicts++);
  memset(e, 0, qcvm->edict_size); // ericw -- switched sv.edicts to malloc(), so
...
*/
int
ED_FindFieldOffset(const char* name)
{
...
*/
static dfunction_t*
ED_FindFunction(const char* fn_name)
{
  dfunction_t* func;
  int i;
...
*/
eval_t*
GetEdictFieldValue(edict_t* ed, int fldofs)
{
  if (fldofs < 0)
    return NULL;
...
*/
eval_t*
GetEdictFieldValueByName(edict_t* ed, const char* name)
{
...
*/
static const char*
PR_ValueString(int type, eval_t* val)
{
  ...
  switch (type) {
    ...
    case ev_void:
      ...
      break;
    case ev_float:
      // Note: leading space, so that float fields are aligned with the first
      // value in vector fields
...
*/
static const char*
PR_UglyValueString(int type, eval_t* val)
{
  ...
  switch (type) {
    ...
    case ev_entity:
      q_snprintf(
        line, sizeof(line), "%i", NUM_FOR_EDICT(PROG_TO_EDICT(val->edict)));
      break;
    case ev_function:
      f = qcvm->functions + val->function;
      q_snprintf(line, sizeof(line), "%s", PR_GetString(f->s_name));
...
*/
static const char*
PR_GetSaveString(savedata_t* save, int num)
{
...
*/
static const char*
PR_UglySaveValueString(savedata_t* save, int type, eval_t* val)
{
  static char line[1024];
  ddef_t* def;
  dfunction_t* f;
  ...
  switch (type) {
    ...
    case ev_entity:
      q_snprintf(
        ...
        sizeof(line),
        "%i",
        SAVE_NUM_FOR_EDICT(save, SAVE_PROG_TO_EDICT(save, val->edict)));
      break;
    case ev_function:
      f = qcvm->functions + val->function;
...
PR_GlobalString(int ofs)
{
  ...
  if (!def)
    ...
    q_snprintf(
      line, sizeof(line), "%i(%s)%s", ofs, PR_GetString(def->s_name), s);
  }
...
*/
qboolean
ED_IsRelevantField(edict_t* ed, ddef_t* d)
{
  const char* name;
  size_t l;
  int* v;
...
*/
static void
ED_AppendFlagString(char* dst, size_t dstsize, const char* desc)
{
  if (*dst)
...
*/
const char*
ED_FieldValueString(edict_t* ed, ddef_t* d)
{
...
#undef DEAD_CASE
  ...
  // .takedamage
  if (ofs == offsetof(entvars_t, takedamage) &&
...
*/
void
ED_Write(savedata_t* save, edict_t* ed)
{
  ddef_t* d;
  int* v;
  int i, j;
  ...
  for (i = 1; i < qcvm->progs->numfielddefs; i++) {
    ...
    fprintf(save->file,
            "\"%s\" \"%s\"\n",
            PR_GetSaveString(save, d->s_name),
...
*/
static void
ED_PrintEdict_Completion_f(const char* partial)
{
  qcvm_t* oldqcvm;
  int i;
  ...
  for (i = 0; i < (int)VEC_SIZE(bbox_linked); i++) {
    ...
    Con_AddToTabList(
      va("%d", NUM_FOR_EDICT(ed)), partial, PR_GetString(ed->v.classname));
  }
  PR_PopQCVM(oldqcvm);
}
...
*/
void
ED_WriteGlobals(savedata_t* save)
{
  ddef_t* def;
  int i;
  int type;
...
static void
ED_RezoneString(string_t* ref, const char* str)
{
  char* buf;
  ...
  if (*ref) { // if the reference is already a zoned string then free it first.
    ...
    if (id < qcvm->knownzonesize &&
        (qcvm->knownzone[id >> 3] & (1u << (id & 7)))) { // okay, it was zoned.
...
ED_ParseEdict(const char* data, edict_t* ent)
{
  ...
  while (1) {
    ...
    data =
      COM_ParseEx(data, !strcmp(keyname, "wad") ? CPE_ALLOWTRUNC : CPE_NOTRUNC);
    if (!data)
      Host_Error("ED_ParseEntity: EOF without closing brace");
...
void
PR_PushQCVM(qcvm_t* newvm, qcvm_t** oldvm)
{
  *oldvm = qcvm;
  PR_SwitchQCVM(NULL);
  PR_SwitchQCVM(newvm);
...
static func_t
PR_FindExtFunction(const char* entryname)
...
static void*
PR_FindExtGlobal(int type, const char* name)
{
...
#undef QCEXTFUNC
  ...
  for (i = 0; i < (unsigned int)qcvm->progs->numglobaldefs; i++) {
    ...
    if (!strncmp(n, "autocvar_", 9)) {
      ...
      if (!ED_ParseEpair(
            (void*)qcvm->globals, &qcvm->globaldefs[i], var->string, true))
        Con_Warning("EXT: Unable to configure %s\n", n);
...
// makes sure extension fields are actually registered so they can be used for
// mappers without qc changes. eg so scale can be used.
static void
PR_MergeEngineFieldDefs(void)
{
  struct
  {
    const char* fname;
    ...
    // table of engine fields to add. we'll be using ED_FindFieldOffset for
    // these later. this is useful for fields that should be defined for mappers
  ...
  if (maxdefs !=
    ...
    memcpy(qcvm->fielddefs,
           olddefs,
           qcvm->progs->numfielddefs * sizeof(*qcvm->fielddefs));
    if (olddefs != (ddef_t*)((byte*)qcvm->progs + qcvm->progs->ofs_fielddefs))
      free(olddefs);

    // allocate the extra defs
    for (j = 0; j < countof(extrafields); j++) {
      if (extrafields[j].newidx >= qcvm->progs->entityfields &&
          extrafields[j].newidx <
            maxofs) { // looks like its new. make sure ED_FindField can find it.
        ...
        qcvm->fielddefs[qcvm->progs->numfielddefs].s_name =
          ED_NewString(extrafields[j].fname);
        qcvm->progs->numfielddefs++;

        if (extrafields[j].type ==
            ev_vector) { // vectors are weird and annoying.
          for (a = 0; a < 3; a++) {
            qcvm->fielddefs[qcvm->progs->numfielddefs].ofs =
              extrafields[j].newidx + a;
            qcvm->fielddefs[qcvm->progs->numfielddefs].type = ev_float;
            qcvm->fielddefs[qcvm->progs->numfielddefs].s_name =
              ED_NewString(va("%s_%c", extrafields[j].fname, 'x' + a));
            qcvm->progs->numfielddefs++;
          }
        }
      }
...
*/
static qboolean
PR_HasGlobal(const char* name, float value)
{
...
PR_FindSupportedEffects(void)
{
  ...
  return isqex
           ? -1
           : -1 & ~(EF_QEX_QUADLIGHT | EF_QEX_PENTALIGHT | EF_QEX_CANDLELIGHT);
}
...
Quake 2021 release update 1 adds bprint/sprint/centerprint builtins with new
id's (see
...
*/
static void
PR_PatchRereleaseBuiltins(void)
{
  dfunction_t* f;
...
Finds all the .entity fields (used by r_showbboxes & co for identifying entity
links)
===============
*/
static void
PR_FindEntityFields(void)
{
  int i, count;
  ...
  qcvm->entityfieldofs =
    (int*)Hunk_AllocName(qcvm->numentityfields * sizeof(int), "entityfieldofs");
  qcvm->entityfields = (ddef_t**)Hunk_AllocName(
    qcvm->numentityfields * sizeof(ddef_t*), "entityfields");
...
*/
static int
PR_CompareFunction(const void* pa, const void* pb)
{
...
*/
static void
PR_FindFunctionRanges(void)
{
  int i, mark;
  int* order;

  qcvm->functionsizes = (int*)Hunk_AllocName(
    qcvm->progs->numfunctions * sizeof(*order), "func_sizes");
  mark = Hunk_LowMark();
...
PR_FillOffsetTables(void)
{
  ...
  for (pass = 0; pass < (int)Q_COUNTOF(passes); pass++) {
    ...
    *passes[pass].maxofs = maxofs;
    ...
    data = *passes[pass].offsets = (int*)Hunk_AllocNameNoFill(
      (maxofs + 1) * sizeof(int), passes[pass].allocname);
    for (i = 0; i <= maxofs; i++)
      data[i] = -1;

    // fill actual offsets in descending order so that earlier defs are written
    // last this preserves the behavior of ED_FieldAtOfs/ED_GlobalAtOfs, which
...
*/
qboolean
PR_LoadProgs(const char* filename, qboolean fatal)
{
  int i;
...
#undef QCEXTFIELD
  ...
  qcvm->edict_size =
    qcvm->progs->entityfields * 4 + sizeof(edict_t) - sizeof(entvars_t);
...
PR_Init(void)
{
  ...
  Cvar_SetCallback(&nomonsters, (cvarcallback_t)ED_Nomonsters_f);
...
int
SAVE_NUM_FOR_EDICT(savedata_t* save, edict_t* e)
{
  int b;
...
PR_AllocStringSlot(void)
{
  ...
  if (qcvm->firstfreeknownstring) {
    ...
    if (i < 0 || i >= qcvm->maxknownstrings)
      Sys_Error(
        "PR_AllocStringSlot failed: invalid free list index %" SDL_PRIs64
        "/%i\n",
        (int64_t)i,
...
static qboolean
PR_IsValidString(const char* p)
{
  uintptr_t d;
  if (!p)
    return false;
  // pointers inside the knownstrings array make up the free list and shouldn't
  // be treated as actual strings
...
int
PR_SetEngineString(const char* s)
{
  int i;
...
int
PR_AllocString(int size, char** ptr)
{
  int i;
...
void
SaveData_Clear(savedata_t* save)
{
  if (save->file)
    fclose(save->file);
  free(save->buffer);
...
SaveData_Fill(savedata_t* save)
{
  ...
  memcpy(save->globals,
         qcvm->globals,
         sizeof(*save->globals) * qcvm->progs->numglobals);
...
void
SaveData_WriteHeader(savedata_t* save)
{
  int i;
