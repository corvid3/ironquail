...
void
VID_SetMouseCursor(mousecursor_t cursor)
{
  switch (cursor) {
...
*/
static int
VID_GetCurrentRefreshRate(void)
{
...
*/
void
VID_SetWindowTitle(const char* title)
{
...
VID_SDL2_GetDisplayMode(int width, int height, int refreshrate)
{
...
*/
static qboolean
VID_ValidMode(int width, int height, int refreshrate, qboolean fullscreen)
{
...
*/
static qboolean
VID_SetMode(int width, int height, int refreshrate, qboolean fullscreen)
{
  int temp;
  Uint32 flags;
  char caption[50];
...
*/
void
VID_Changed_f(cvar_t* var)
{
  if (vid_initialized && !vid_locked && key_dest != key_menu)
    Con_SafePrintf(
      "%s %s will be applied after a vid_restart\n", var->name, var->string);
  vid_changed = true;
}

void
VID_RecalcConsoleSize(void)
{
...
*/
static void
GL_Info_Completion_f(const char* partial)
{
  int i;
...
*/
static qboolean
GL_FindExtension(const char* name)
{
  int i;
  for (i = 0; i < gl_num_extensions; i++) {
...
GL_DebugCallback(GLenum source,
                 GLenum type,
                 GLuint id,
                 GLenum severity,
                 ...
                 __attribute__((unused)) const void* userParam)
{
  ...
  if (severity == GL_DEBUG_SEVERITY_NOTIFICATION) {
    ...
    Con_SafePrintf(
      "\x02GL %s %s[#%u/%s]: ", str_source, str_type, id, str_severity);
    Con_SafePrintf("%s\n", message);
  }
}
...
*/
static void
GL_SetStateEx(unsigned mask, unsigned force)
{
...
VID_DescribeModes_f(void)
{
  ...
  for (i = 0; i < nummodes; i++) {
    if (lastwidth != modelist[i].width || lastheight != modelist[i].height ||
      ...
      if (count > 0)
        Con_SafePrintf("\n");
      Con_SafePrintf("  %5i x %4i | %2i bit | %i Hz",
                     modelist[i].width,
                     modelist[i].height,
                     modelist[i].bpp,
...
*/
static void
VID_CompleteModeField(cvar_t* cvar, const char* partial, size_t ofs)
{
  int i;
...
#define GET_FIELD_FOR_MODE(idx) *(int*)((uintptr_t) & modelist[idx] + ofs)
  ...
  for (i = 0; i < nummodes; i++) {
    ...
    Con_AddToTabList(
      buf, partial, cvar->value == GET_FIELD_FOR_MODE(i) ? "current" : NULL);
  }
...
*/
static void
VID_Width_Completion_f(cvar_t* cvar, const char* partial)
{
...
*/
static void
VID_Height_Completion_f(cvar_t* cvar, const char* partial)
{
...
*/
static void
VID_Refresh_Completion_f(cvar_t* cvar, const char* partial)
{
...
#define num_readvars Q_COUNTOF(read_vars)
  ...
  Cvar_SetCallback(&vid_fullscreen, (cvarcallback_t)VID_Changed_f);
  Cvar_SetCallback(&vid_width, (cvarcallback_t)VID_Changed_f);
  Cvar_SetCallback(&vid_height, (cvarcallback_t)VID_Changed_f);
  Cvar_SetCallback(&vid_refreshrate, (cvarcallback_t)VID_Changed_f);
  Cvar_SetCallback(&vid_vsync, (cvarcallback_t)VID_VSync_f);
  Cvar_SetCallback(&vid_fsaa, (cvarcallback_t)VID_FSAA_f);
  Cvar_SetCallback(&vid_fsaamode, (cvarcallback_t)VID_FSAAMode_f);
  Cvar_SetCallback(&vid_desktopfullscreen, (cvarcallback_t)VID_Changed_f);
  Cvar_SetCallback(&vid_borderless, (cvarcallback_t)VID_Changed_f);

  Cvar_SetCompletion(&vid_width, (cvarcompletion_t)VID_Width_Completion_f);
  Cvar_SetCompletion(&vid_height, (cvarcompletion_t)VID_Height_Completion_f);
  Cvar_SetCompletion(&vid_refreshrate,
                     (cvarcompletion_t)VID_Refresh_Completion_f);

  Cvar_RegisterVariable(&gl_texture_anisotropy);
  Cvar_SetCallback(&gl_texture_anisotropy, (cvarcallback_t)TexMgr_Anisotropy_f);

  Cvar_RegisterVariable(&gl_compress_textures);
  Cvar_SetCallback(&gl_compress_textures,
                   (cvarcallback_t)TexMgr_CompressTextures_f);

  Cvar_RegisterVariable(&r_softemu_metric);

  Cvar_RegisterVariable(&scr_pixelaspect);
  Cvar_SetCallback(&scr_pixelaspect, (cvarcallback_t)SCR_PixelAspect_f);
