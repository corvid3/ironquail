...
static const char* const pr_extnames[QCEXT_COUNT] = { "STD_QC",
...
*/
int
PR_FindExtensionByName(const char* name)
{
  int i;
...
PR_EnterFunction(dfunction_t* f)
{
  ...
  for (i = 0; i < c; i++)
    qcvm->localstack[qcvm->localstack_used + i] =
      ((int*)qcvm->globals)[f->parm_start + i];
...
PR_LeaveFunction(void)
{
  ...
  for (i = 0; i < c; i++)
    ((int*)qcvm->globals)[qcvm->xfunction->parm_start + i] =
      qcvm->localstack[qcvm->localstack_used + i];
...
*/
static void
PR_CheckBuiltinExtension(dfunction_t* func)
{
  ...
  Con_DWarning(checked ? "[%s] \"%s\" ignored when calling %s (%s: %s)\n"
  ...
  while (1) {
    ...
    switch (st->op) {
      ...
      case OP_RETURN:
        ...
        qcvm->globals[OFS_RETURN + 1] =
          qcvm->globals[(unsigned short)st->a + 1];
        qcvm->globals[OFS_RETURN + 2] =
