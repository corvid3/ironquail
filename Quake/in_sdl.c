...
static void
IN_ResetFlickState(void)
{
  memset(&flick, 0, sizeof(flick));
}

static int SDLCALL
IN_FilterMouseEvents(const SDL_Event* event)
{
  switch (event->type) {
...
static void
IN_BeginIgnoringMouseEvents(void)
{
...
static void
IN_EndIgnoringMouseEvents(void)
{
...
#ifdef MACOS_X_ACCELERATION_HACK
static cvar_t in_disablemacosxmouseaccel = { "in_disablemacosxmouseaccel",
                                             "1",
                                             CVAR_ARCHIVE };
  ...
  if (mouseDev != 0) {
    if (IOHIDGetAccelerationWithKey(mouseDev,
      ...
      if (IOHIDSetAccelerationWithKey(mouseDev,
        ...
        Con_Printf("WARNING: Could not disable mouse acceleration (failed at "
...
IN_ReenableOSXMouseAccel(void)
{
  ...
  if (mouseDev != 0) {
    if (IOHIDSetAccelerationWithKey(mouseDev,
                                    CFSTR(kIOHIDMouseAccelerationType),
                                    originalMouseSpeed) != kIOReturnSuccess)
      Con_Printf("WARNING: Could not re-enable mouse acceleration (failed at "
                 "IOHIDSetAccelerationWithKey).\n");
    IOServiceClose(mouseDev);
  } else {
    Con_Printf("WARNING: Could not re-enable mouse acceleration (failed at "
...
void
IN_Deactivate(qboolean free_cursor)
{
  if (no_mouse)
    return;
...
void
IN_DeactivateForConsole(void)
{
  IN_Deactivate(true);
}

void
IN_DeactivateForMenu(void)
{
  IN_Deactivate(modestate == MS_WINDOWED || ui_mouse.value);
}

static qboolean
IN_UseController(int device_index)
{
  ...
  if (joy_active_device != -1) {
    ...
    // Only show "gamepad removed" message when disabling the gamepad
    // altogether, not when switching to a different device
    if (device_index == -1)
  ...
  // Save controller name so we can print it when unplugged
  // (SDL_GameControllerName would return NULL)
  ...
  if (SDL_GameControllerHasSensor(joy_active_controller, SDL_SENSOR_GYRO) &&
      !SDL_GameControllerSetSensorEnabled(
        joy_active_controller, SDL_SENSOR_GYRO, SDL_TRUE)) {
    gyro_present = true;
#if SDL_VERSION_ATLEAST(2, 0, 16)
    Con_Printf("Gyro sensor enabled at %g Hz\n",
               SDL_GameControllerGetSensorDataRate(joy_active_controller,
                                                   SDL_SENSOR_GYRO));
#else
...
IN_StartupJoystick(void)
{
  ...
  for (i = 0; i < com_numbasedirs; i++) {
    q_snprintf(controllerdb,
               sizeof(controllerdb),
               "%s/gamecontrollerdb.txt",
               com_basedirs[i]);
...
const char*
IN_GetGamepadName(void)
{
  return joy_active_controller ? joy_active_name : NULL;
}

gamepadtype_t
IN_GetGamepadType(void)
{
  return joy_active_type;
}

void
IN_UseNextGamepad(int dir, qboolean allow_disable)
{
...
IN_Init(void)
{
  ...
  Cvar_SetCallback(&joy_flick, (cvarcallback_t)Joy_Flick_f);
  ...
  Cvar_SetCallback(&joy_device, (cvarcallback_t)Joy_Device_f);
  Cvar_SetCompletion(&joy_device, (cvarcompletion_t)Joy_Device_Completion_f);
...
IN_FovScale(void)
{
  return tan(DEG2RAD(r_refdef.basefov) * 0.5f) /
         tan(DEG2RAD(scr_fov.value) * 0.5f);
}
...
IN_MouseMotion(int dx, int dy)
{
  ...
  if (cls.state != ca_connected || cls.signon != SIGNONS ||
      key_dest != key_game || CL_InCutscene()) {
    total_dx = 0;
    total_dy = 0;
    return;
  }
...
*/
static vec_t
IN_AxisMagnitude(joyaxis_t axis)
{
...
*/
static int
IN_KeyForControllerButton(SDL_GameControllerButton button)
{
  switch (button) {
    ...
    case SDL_CONTROLLER_BUTTON_Y:
      return K_YBUTTON;
    // Note: back and start are always mapped to TAB/ESC, the player cannot
    // rebind them
...
*/
static void
IN_JoyKeyEvent(qboolean wasdown, qboolean isdown, int key, double* timer)
{
  static const double repeatdelay =
    0.5; // time (in seconds) between initial press and first repetition
...
IN_Commands(void)
{
  ...
  if (key_dest != key_game) {
    int xaxis = joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_RIGHTX
                                       : SDL_CONTROLLER_AXIS_LEFTX;
    int yaxis = joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_RIGHTY
                                       : SDL_CONTROLLER_AXIS_LEFTY;
    IN_JoyKeyEvent(joy_axisstate.axisvalue[xaxis] < -stickthreshold,
                   newaxisstate.axisvalue[xaxis] < -stickthreshold,
                   K_LEFTARROW,
    ...
    IN_JoyKeyEvent(joy_axisstate.axisvalue[yaxis] < -stickthreshold,
                   newaxisstate.axisvalue[yaxis] < -stickthreshold,
                   K_UPARROW,
  ...
  if (key_dest == key_console) {
    ...
    raw.x = newaxisstate
              .axisvalue[joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_LEFTX
                                                : SDL_CONTROLLER_AXIS_RIGHTX];
    raw.y = newaxisstate
              .axisvalue[joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_LEFTY
                                                : SDL_CONTROLLER_AXIS_RIGHTY];
    deadzone = IN_ApplyDeadzone(
      raw, joy_deadzone_look.value, joy_outer_threshold_look.value);
  ...
  IN_JoyKeyEvent(
    joy_axisstate.axisvalue[SDL_CONTROLLER_AXIS_TRIGGERLEFT] > triggerthreshold,
  ...
  if (joy_has_rumble && joy_rumble.value > 0.f) {
    ...
    SDL_GameControllerRumble(
      joy_active_controller, lofreq * strength, hifreq * strength, 100);
  }
#endif // SDL_VERSION_ATLEAST (2, 0, 9)
}
...
*/
static float
IN_FlickStickEasing(float frac)
{
  frac = 1.f - frac;
  frac = 1.f - frac * frac;
  return frac;
...
IN_JoyMove(usercmd_t* cmd)
{
  ...
  moveDeadzone = IN_ApplyDeadzone(
    moveRaw, joy_deadzone_move.value, joy_outer_threshold_move.value);
  lookDeadzone = IN_ApplyDeadzone(
    lookRaw, joy_deadzone_look.value, joy_outer_threshold_look.value);
  ...
  if (joy_flick.value && gyro_present && gyro_enable.value) {
    ...
    if (joy_flick_time.value > 0.f) {
      lerp_frac =
        flick.prev_lerp_frac + host_rawframetime / joy_flick_time.value;
      lerp_frac = CLAMP(0.f, lerp_frac, 1.f);
    } else
      lerp_frac = 1.f;
    delta = IN_FlickStickEasing(lerp_frac) -
            IN_FlickStickEasing(flick.prev_lerp_frac);
    cl.viewangles[YAW] -= flick.yaw * delta;
    cl.viewangles[PITCH] -=
      flick.pitch * delta * CLAMP(0.f, joy_flick_recenter.value, 1.f);
  ...
  } else // traditional joystick look
  {
    ...
    cl.viewangles[YAW] -=
      lookEased.x * joy_sensitivity_yaw.value * host_rawframetime;
    cl.viewangles[PITCH] += lookEased.y * joy_sensitivity_pitch.value *
                            (joy_invert.value ? -1.0 : 1.0) * host_rawframetime;
...
IN_GyroMove(usercmd_t* cmd)
{
  ...
  // store angle delta and reset centering anim if centerview was used this
  // frame
...
keydevice_t
IN_GetLastActiveDeviceType(void)
{
  return lastactivetype;
}

static inline int
IN_SDL2_ScancodeToQuakeKey(SDL_Scancode scancode)
{
  switch (scancode) {
...
static void
IN_DebugTextEvent(SDL_Event* event)
{
  Con_Printf(
...
static void
IN_DebugKeyEvent(SDL_Event* event)
{
...
IN_StartGyroCalibration(void)
{
  ...
  // Note: we modify updates_countdown first, before printing the message to the
  // console, because Con_Printf triggers a redraw, which in turn calls
  // M_Calibration_Draw, which would see that updates_countown is 0 and consider
  // that calibration must be done. Alternatively, we could use Con_SafePrintf
  // (which doesn't refresh the screen).

  Con_Printf("Calibrating, please wait...\n");
}

static qboolean
IN_UpdateGyroCalibration(const float newsample[3])
{
...
float
IN_GetRawLookMagnitude(void)
{
  joyaxis_t axis;
  if (!joy_active_controller)
    return 0.f;

  axis.x = joy_axisstate
             .axisvalue[joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_LEFTX
                                               : SDL_CONTROLLER_AXIS_RIGHTX];
  axis.y = joy_axisstate
             .axisvalue[joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_LEFTY
                                               : SDL_CONTROLLER_AXIS_RIGHTY];
...
IN_GetRawMoveMagnitude(void)
{
  ...
  axis.x =
    joy_axisstate.axisvalue[joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_RIGHTX
                                                   : SDL_CONTROLLER_AXIS_LEFTX];
  axis.y =
    joy_axisstate.axisvalue[joy_swapmovelook.value ? SDL_CONTROLLER_AXIS_RIGHTY
                                                   : SDL_CONTROLLER_AXIS_LEFTY];
...
float
IN_GetRawTriggerMagnitude(void)
{
  float left, right;
  if (!joy_active_controller)
    return 0.f;
...
qboolean
IN_IsCalibratingGyro(void)
{
  return updates_countdown != 0;
}

float
IN_GetGyroCalibrationProgress(void)
{
  return (GYRO_CALIBRATION_SAMPLES - updates_countdown) /
         (float)GYRO_CALIBRATION_SAMPLES;
}

static float
IN_FilterGyroSample(float prev, float cur)
{
    ...
    switch (event.type) {
      ...
      case SDL_CONTROLLERSENSORUPDATE:
        if (event.csensor.sensor == SDL_SENSOR_GYRO &&
            event.csensor.which == joy_active_instanceid) {
          ...
          gyro_raw_mag =
            RAD2DEG(sqrt(gyro_yaw * gyro_yaw + gyro_pitch * gyro_pitch));
