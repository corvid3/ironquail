...
// johnfitz -- new cvars
cvar_t scr_menuscale = { "scr_menuscale", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_menubgalpha = {
  "scr_menubgalpha", "0.7", CVAR_ARCHIVE, 0, 0, 0, 0, 0
};
cvar_t scr_sbarscale = { "scr_sbarscale", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_sbaralpha = { "scr_sbaralpha", "0.75", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_conwidth = { "scr_conwidth", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_conscale = { "scr_conscale", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_crosshairscale = {
  "scr_crosshairscale", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0
};
cvar_t scr_pixelaspect = {
...
// johnfitz
cvar_t scr_usekfont = {
  "scr_usekfont", "0", CVAR_NONE, 0, 0, 0, 0, 0
}; // 2021 re-release

cvar_t scr_hudstyle = { "hudstyle", "2", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t cl_screenshotname = { "cl_screenshotname",
                             "screenshots/%map%_%date%_%time%",
                             CVAR_ARCHIVE,
                             0,
                             0,
                             0,
                             0,
                             0 };
cvar_t scr_demobar_timeout = {
  "scr_demobar_timeout", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0
};

cvar_t scr_viewsize = { "viewsize", "100", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_fov = { "fov", "90", CVAR_ARCHIVE, 0, 0, 0, 0, 0 }; // 10 - 170
cvar_t scr_fov_adapt = { "fov_adapt", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_zoomfov = {
  "zoom_fov", "30", CVAR_ARCHIVE, 0, 0, 0, 0, 0
}; // 10 - 170
cvar_t scr_zoomspeed = { "zoom_speed", "8", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_conspeed = { "scr_conspeed", "2000", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t scr_centertime = { "scr_centertime", "2", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t scr_showturtle = { "showturtle", "0", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t scr_showpause = { "showpause", "1", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t scr_printspeed = { "scr_printspeed", "8", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t gl_triplebuffer = {
  "gl_triplebuffer", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0
};

cvar_t cl_gun_fovscale = {
  "cl_gun_fovscale", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0
}; // Qrack
cvar_t cl_gun_x = { "cl_gun_x", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t cl_gun_y = { "cl_gun_y", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t cl_gun_z = { "cl_gun_z", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
...
*/
void
SCR_CenterPrint(const char* str) // update centerprint data
{
  ...
  if (!cl.intermission)
    scr_centertime_off +=
      q_max(0.f, con_notifyfade.value * con_notifyfadetime.value);
...
void
SCR_DrawCenterString(void) // actually do the drawing
{
  char* start;
  int l;
  int j;
...
*/
float
AdaptFovx(float fov_x, float width, float height)
{
  float a, x;
...
*/
float
CalcFovy(float fov_x, float width, float height)
{
  float a, x;
...
SCR_CalcRefdef(void)
{
  ...
  if (size >= 120 || cl.intermission || scr_sbaralpha.value < 1 ||
      ...
      cl.qcvm.extfuncs.CSQC_DrawHud) // johnfitz -- scr_sbaralpha.value
    sb_lines = 0;
  else if (size >= 110)
    sb_lines = 24 * scale;
  else
...
static void
SCR_Callback_refdef(__attribute__((unused)) cvar_t* var)
{
  vid.recalc_refdef = 1;
}
...
*/
void
SCR_Conwidth_f(__attribute__((unused)) cvar_t* var)
{
  vid.recalc_refdef = 1;
...
*/
void
SCR_PixelAspect_f(__attribute__((unused)) cvar_t* cvar)
{
  vid.recalc_refdef = 1;
...
SCR_Init(void)
{
  ...
  Cvar_SetCallback(&scr_sbaralpha, (cvarcallback_t)SCR_Callback_refdef);
  Cvar_RegisterVariable(&scr_sbaralpha);
  Cvar_SetCallback(&scr_conwidth, (cvarcallback_t)SCR_Conwidth_f);
  Cvar_SetCallback(&scr_conscale, (cvarcallback_t)SCR_Conwidth_f);
  ...
  Cvar_SetCallback(&scr_fov, (cvarcallback_t)SCR_Callback_refdef);
  Cvar_SetCallback(&scr_fov_adapt, (cvarcallback_t)SCR_Callback_refdef);
  Cvar_SetCallback(&scr_zoomfov, (cvarcallback_t)SCR_Callback_refdef);
  Cvar_SetCallback(&scr_viewsize, (cvarcallback_t)SCR_Callback_refdef);
  Cvar_SetCallback(&scr_hudstyle, (cvarcallback_t)SCR_HUDStyle_f);
...
*/
static void
SCR_PrintMirrored(int x, int y, const char* str)
{
  x += strlen(str) * 8;
  while (*str) {
...
SCR_DrawDemoControls(void)
{
  ...
  if (cls.demospeed != prevspeed ||
      cls.basedemospeed != prevbasespeed ||      // speed/base speed changed
      ...
      !scr_demobar_timeout.value)                // controls always shown
  {
    ...
    showtime =
      scr_demobar_timeout.value > 0.f ? scr_demobar_timeout.value : 1.f;
  } else {
  ...
  frac =
    (Sys_ftell(cls.demofile) - cls.demofilestart) / (double)cls.demofilesize;
  ...
  // Note: character #13 works well as a forward symbol, but Alkaline 1.2
  // changes it to a disk. If we have a custom conchars texture we switch to a
  // safer alternative, the '>' character.
  if (!cls.demospeed)
    str = "II";
...
SCR_DrawDevStats(void)
{
  ...
  sprintf(
    str, "Packet   |%5i %5i", dev_stats.packetsize, dev_peakstats.packetsize);
  Draw_String(x, (y++) * 8 - x, str);

  sprintf(
    str, "Visedicts|%5i %5i", dev_stats.visedicts, dev_peakstats.visedicts);
  Draw_String(x, (y++) * 8 - x, str);
  ...
  sprintf(str,
          "GL upload|%4iK %4iK",
          dev_stats.gpu_upload / 1024,
          dev_peakstats.gpu_upload / 1024);
  Draw_String(x, (y++) * 8 - x, str);
}
...
{
  if (cl.intermission || CL_InCutscene() || !crosshair.value ||
...
*/
static void
SCR_SetupProjToCanvasMap(canvasmap_t* remap)
{
  float xmin, ymin, width, height;

  width =
    r_refdef.vrect.width / (float)glwidth * (glcanvas.right - glcanvas.left);
  height =
    r_refdef.vrect.height / (float)glheight * (glcanvas.top - glcanvas.bottom);
  xmin = glcanvas.left +
         (r_refdef.vrect.x / (float)glwidth) * (glcanvas.right - glcanvas.left);
  ymin =
    glcanvas.bottom -
    ((glheight - r_refdef.vrect.y - r_refdef.vrect.height) / (float)glheight) *
      (glcanvas.bottom - glcanvas.top);
...
*/
static void
SCR_ProjToCanvas(const vec3_t proj,
                 const canvasmap_t* remap,
                 float* outx,
                 float* outy)
{
...
*/
static void
SCR_GetEntityBottom(const edict_t* ed, vec3_t pos)
{
  ...
  // If it's a point entity, move anchor point down by 8 units to avoid
  // overlapping debug visualization
...
*/
static void
SCR_GetEntityCenter(const edict_t* ed, vec3_t pos)
{
  if (!VectorCompare(ed->v.mins, ed->v.maxs)) {
    pos[0] = ed->v.origin[0] + (ed->v.mins[0] + ed->v.maxs[0]) * 0.5f;
    pos[1] = ed->v.origin[1] + (ed->v.mins[1] + ed->v.maxs[1]) * 0.5f;
...
*/
static void
SCR_ClipToFrustum(vec3_t pos, const vec3_t ref)
{
  int i;
...
*/
static void
SCR_DrawKeyValueOverlay(float x,
                        float y,
                        const char* multistr,
                        const vec3_t bgcolor)
{
  int ofs, numlines, keylen, vallen, maxkey, maxval;
  const char *key, *val;

  // Compute column widths and total number of lines
  for (ofs = numlines = maxkey = maxval = 0; ofs < (int)VEC_SIZE(multistr);
       numlines++) {
    key = multistr + ofs;
  ...
  // Draw background box
  Draw_FillEx(
    x - maxkey, y, maxkey + maxval, numlines * CHARSIZE, bgcolor, 0.75f);
...
SCR_DrawEdictInfo(void)
{
  ...
  for (i = (int)VEC_SIZE(bbox_linked) - 1; i >= 0; i--) {
    ...
    switch (ed->showbboxflags) {
      ...
      case SHOWBBOX_LINK_NONE:
        VectorSet(bgcolor, 0.f, 0.f, 0.f);
        break;
      case SHOWBBOX_LINK_INCOMING:
        VectorSet(bgcolor, 0.25f, 0.125f, 0.125f);
        break;
      case SHOWBBOX_LINK_OUTGOING:
        VectorSet(bgcolor, 0.125f, 0.125f, 0.25f);
        break;
      case SHOWBBOX_LINK_BOTH:
        VectorSet(bgcolor, 0.25f, 0.125f, 0.25f);
        break;
    }
  ...
  // Show all relevant entity fields for the highlighted entity (first in
  // bbox_linked)
  if (r_showfields.value) {
    ...
    // Add all relevant fields, excluding classname (already added to the
    // header)
    for (i = 1; i < qcvm->progs->numfielddefs; i++) {
      ddef_t* d = &qcvm->fielddefs[i];
      if (d->ofs * 4 == offsetof(entvars_t, classname) ||
          !ED_IsRelevantField(ed, d))
        continue;
...
SCR_SetUpToDrawConsole(void)
{
  ...
  if (con_forcedup) {
    ...
    scr_conlines =
      glheight / 2; // half screen //johnfitz -- glheight instead of vid.height
  else
...
*/
static void
SCR_GetCleanMapTitle(char* buf, size_t maxchars)
{
...
*/
static qboolean
SCR_ExpandVariables(const char* fmt, char* dst, size_t maxchars)
{
  time_t now;
  struct tm* lt;
  char var[256];
...
SCR_ScreenShot_f(void)
{
  ...
  has_vars =
    SCR_ExpandVariables(cl_screenshotname.string, basename, sizeof(basename));
  if (!basename[0])
  ...
  if (Sys_FileType(checkname) !=
      FS_ENT_NONE) // base name already used, try appending an index
  {
  append_index:
    // append underscore if basename ends with a digit
    i = (int)strlen(basename);
    if (i && i + 1 < (int)countof(basename) &&
        (unsigned int)(basename[i - 1] - '0') < 10u) {
      basename[i] = '_';
      basename[i + 1] = '\0';
    }
  ...
  if (!q_strncasecmp(ext, "png", sizeof(ext)))
    ...
    ok =
      Image_WriteJPG(imagename, buffer, glwidth, glheight, 24, quality, false);
  else
    ok = false;
...
*/
int
SCR_ModalMessage(const char* text, float timeout) // johnfitz -- timeout
{
  ...
  do {
    ...
    if (timeout)
      time2 = Sys_DoubleTime(); // johnfitz -- zero timeout means wait forever.
           ...
           lastkey != K_BBUTTON && lastkey != K_MOUSE2 && lastkey != K_MOUSE4 &&
           time2 <= time1);
  Key_EndInputGrab();
...
SCR_TileClear(void)
{
  ...
  if (r_refdef.vrect.y > 0) {
    ...
    Draw_TileClear(r_refdef.vrect.x,
                   ...
                   r_refdef.vrect.width,
                   glheight - r_refdef.vrect.y - r_refdef.vrect.height -
                     sb_lines);
  }
}
