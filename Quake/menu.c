...
M_Menu_Video_f(void);
void
M_Menu_Gamepad_f(void);
void
M_Menu_Mods_f(void);
void
M_Menu_ModInfo_f(const filelist_item_t* item);
void
M_Menu_Help_f(void);
void
M_Menu_Quit_f(void);
...
void
M_ServerList_Key(int key);
void
M_Options_Key(int key);
void
M_Keys_Key(int key);
...
void
M_Options_Mousemove(int cx, int cy);
void
...
void
M_SetSkillMenuMap(const char* name);
void
M_Options_SelectMods(void);
void
M_Options_Init(enum m_state_e state);
...
static void
M_ThrottledSound(const char* sound)
{
  if (strcmp(m_lastsound, sound) == 0 &&
...
void
M_DrawArrowCursor(int cx, int cy)
{
  M_DrawCharacter(cx, cy, 12 + ((int)(realtime * 4) & 1));
}

void
M_DrawTextCursor(int cx, int cy)
{
  M_DrawCharacter(cx, cy, 10 + ((int)(realtime * 4) & 1));
}

void
M_PrintEx(int cx, int cy, int dim, const char* str)
{
  while (*str) {
...
void
M_PrintWhiteEx(int cx, int cy, int dim, const char* str)
{
  while (*str) {
...
void
M_PrintAligned(int cx, int cy, int align, const char* str)
{
...
static void
M_PrintSubstring(int x, int y, const char* text, int numchars, qboolean color)
{
...
static void
M_PrintDotFill(int x, int y, const char* text, int cols, qboolean color)
{
  char mask = color ? 0x80 : 0;
  while (*text && cols >= 2) {
    M_DrawCharacter(x, y, *text++ ^ mask);
...
static int
M_LineWrap(const char** text, int maxchars)
{
  const char* str = *text;
  int i;
...
{
  Draw_Pic(
    x, y, pic); // johnfitz -- simplified becuase centering is handled elsewhere
}
...
void
M_DrawQuakeCursor(int cx, int cy)
{
  qpic_t* pic =
    Draw_CachePic(va("gfx/menudot%i.lmp", (int)(realtime * 10) % 6 + 1));
  M_DrawTransPic(cx, cy, pic);
}

void
M_DrawQuakeBar(int x, int y, int cols)
{
...
void
M_DrawEllipsisBar(int x, int y, int cols)
{
  while (cols > 0) {
...
void
M_UpdateCursorWithTable(int mousey, const int* table, int numitems, int* cursor)
{
  int i, dy;
...
void
M_List_CheckIntegrity(const menulist_t* list)
{
...
void
M_List_ClearSearch(menulist_t* list)
{
...
void
M_List_KeepSearchVisible(menulist_t* list, double duration)
{
  if (!list->search.len)
    return;
  if (duration <= 0.0)
...
void
M_List_AutoScroll(menulist_t* list)
{
...
void
M_List_CenterCursor(menulist_t* list)
{
  if (list->cursor >= list->viewsize) {
    if (list->cursor + list->viewsize >= list->numitems)
      list->scroll =
        list->numitems - list->viewsize; // last page, scroll to the end
    else
...
int
M_List_GetOverflow(const menulist_t* list)
{
...
qboolean
M_List_GetScrollbar(const menulist_t* list, int* y, int* height)
{
  if (list->numitems <= list->viewsize) {
    *y = *height = 0;
    return false;
  ...
  *height =
    (int)(list->viewsize * list->viewsize / (float)list->numitems + 0.5f);
  *height = q_max(*height, 2);
  *y = (int)(list->scroll * 8 / (float)(list->numitems - list->viewsize) *
               (list->viewsize - *height) +
             0.5f);
...
void
M_List_DrawScrollbar(const menulist_t* list, int cx, int cy)
{
  int y, h;
...
void
M_List_DrawSearch(const menulist_t* list, int cx, int cy, int maxlen)
{
  int i, ofs;
  float alpha;
...
qboolean
M_List_UseScrollbar(menulist_t* list, int yrel)
{
  ...
  list->scroll =
    (int)(yrel * (float)(list->numitems - list->viewsize) / range + 0.5f);
...
void
M_List_GetVisibleRange(const menulist_t* list, int* first, int* count)
{
  *first = list->scroll;
...
qboolean
M_List_IsItemVisible(const menulist_t* list, int i)
{
  int first, count;
...
void
M_List_Rescroll(menulist_t* list)
{
  ...
  if (list->cursor >= 0 && list->cursor < list->numitems &&
      !M_List_IsItemVisible(list, list->cursor))
    M_List_AutoScroll(list);
}

qboolean
M_List_SelectNextMatch(menulist_t* list,
                       qboolean (*match_fn)(int idx),
                       int start,
                       int dir,
                       qboolean wrap)
{
...
qboolean
M_List_SelectNextSearchMatch(menulist_t* list, int start, int dir)
{
...
qboolean
M_List_SelectNextActive(menulist_t* list, int start, int dir, qboolean wrap)
{
  return M_List_SelectNextMatch(list, list->isactive_fn, start, dir, wrap);
}

void
M_List_UpdateMouseSelection(menulist_t* list)
{
  M_ForceMousemove();
  if (list->cursor < list->scroll)
...
void
M_List_Update(menulist_t* list)
{
  list->search.errtimeout =
    q_max(0.0, list->search.errtimeout - host_rawframetime);
  list->search.backspacecooldown =
    q_max(0.0, list->search.backspacecooldown - host_rawframetime);
...
qboolean
M_List_Key(menulist_t* list, int key)
{
    ...
    case K_KP_PGDN:
      ...
      if (list->search.len) {
        ...
        if (list->cursor - list->scroll < list->viewsize - 1)
          sel = M_List_SelectNextActive(
            list, list->scroll + list->viewsize - 1, 1, false);
        else
          sel = M_List_SelectNextActive(
            list, list->cursor + list->viewsize - 1, 1, false);
        if (!sel)
          M_List_SelectNextActive(list, list->numitems - 1, -1, false);
      }
      return true;
...
void
M_List_Char(menulist_t* list, int key)
{
  int maxlen, start;
...
void
M_List_Mousemove(menulist_t* list, int yrel)
{
...
static void
M_Ticker_Init(menuticker_t* ticker)
{
...
static void
M_Ticker_Update(menuticker_t* ticker)
{
  if (ticker->scroll_wait_time <= 0.0)
    ticker->scroll_time += host_rawframetime;
  else
    ticker->scroll_wait_time =
      q_max(0.0, ticker->scroll_wait_time - host_rawframetime);
}

static qboolean
M_Ticker_Key(menuticker_t* ticker, int key)
{
  switch (key) {
    case K_RIGHTARROW:
...
const char*
M_Maps_GetMessage(const mapitem_t* item)
{
  if (!item->source)
    return item->name;
...
static qboolean
M_Maps_IsActive(const char* map)
{
  return cls.state == ca_connected && cls.signon == SIGNONS &&
         !strcmp(cl.mapname, map);
}

static void
M_Maps_AddDecoration(const char* text)
{
  mapitem_t item;
...
static void
M_Maps_AddSeparator(maptype_t before, maptype_t after)
{
#define QBAR "\35\36\37"
  ...
  if (after >= MAPTYPE_ID_START) {
    ...
    M_Maps_AddDecoration("");
  } else if (after >= MAPTYPE_CUSTOM_ID_START &&
             before < MAPTYPE_CUSTOM_ID_START) {
    M_Maps_AddDecoration("");
...
static qboolean
M_Maps_IsSelectable(int index)
{
  return mapsmenu.items[index].source != NULL;
}

static void
M_Maps_UpdateLayout(void)
{
  int height;
...
M_Maps_Draw(void)
{
  ...
  for (i = 0; i < numvis; i++) {
    ...
    if (!item->source) {
      M_PrintWhite(
        x + (cols - strlen(item->name)) / 2 * 8, y + i * 8, item->name);
    } else {
      char buf[256];
      if (mapsmenu.list.search.len > 0)
        COM_TintSubstring(
          item->name, mapsmenu.list.search.text, buf, sizeof(buf));
      else
      ...
      if (!message || message[0]) {
        if (!message) {
          ...
          COM_TintSubstring(
            message, mapsmenu.list.search.text, buf, sizeof(buf));
        else
  ...
  M_List_DrawSearch(
    &mapsmenu.list, x, y + mapsmenu.list.viewsize * 8 + 4, namecols);
}
...
M_SetSkillMenuMap(const char* name)
{
  ...
  if (!Mod_LoadMapDescription(
        m_skill_maptitle, sizeof(m_skill_maptitle), name) ||
      !m_skill_maptitle[0])
...
M_Skill_Draw(void)
{
  ...
  M_DrawTransPic(16, 4, Draw_CachePic("gfx/qplaque.lmp"));
  p = Draw_CachePic(m_skill_usecustomtitle && !m_skill_canresume
                      ? "gfx/p_skill.lmp"
                      : "gfx/ttl_sgl.lmp");
  M_DrawPic((320 - p->width) / 2, 4, p);
  ...
  M_PrintScroll(
    x, 32, 30 * 8, m_skill_maptitle, m_skill_ticker.scroll_time, false);
  ...
  if (m_skill_canresume) {
    ...
    if (m_skill_lastplayed) {
      ...
      COM_DescribeDuration(
        duration, sizeof(duration), difftime(m_skill_lastplayed, now));
...
M_Setup_Draw(void)
{
  ...
  if (setup_cursor == 0)
    M_DrawTextCursor(168 + 8 * strlen(setup_hostname),
                     setup_cursor_table[setup_cursor]);

  if (setup_cursor == 1)
    M_DrawTextCursor(168 + 8 * strlen(setup_myname),
                     setup_cursor_table[setup_cursor]);
}
...
M_Setup_Mousemove(int cx, int cy)
{
  ...
  M_UpdateCursorWithTable(
    cy, setup_cursor_table, NUM_SETUP_CMDS, &setup_cursor);
...
VID_Menu_RebuildRateList(void)
{
  ...
  for (i = 0; i < nummodes; i++) {
    ...
    if (modelist[i].width != vid_width.value ||
        modelist[i].height != vid_height.value || modelist[i].bpp < 24)
      continue;
...
*/
static void
VID_Menu_ChooseNextResolution(int dir)
{
  int i;
...
*/
static void
VID_Menu_ChooseNextRate(int dir)
{
  int i;
...
static void
VID_Menu_SetDisplayMode(windowmode_t mode)
{
...
chooses next window mode in order, then updates vid_fullscreen and
vid_borderless cvars
================
*/
static void
...
*/
static void
VID_Menu_ChooseNextAnisotropy(int dir)
{
  ...
  Cvar_SetValueQuick(&gl_texture_anisotropy,
                     CLAMP(1, aniso, (int)gl_max_anisotropy));
}
...
*/
static void
VID_Menu_ChooseNextScale(int dir)
{
...
*/
static void
VID_Menu_ChooseNextTexFilter(void)
{
...
*/
static const char*
VID_Menu_GetTexFilterDesc(void)
{
...
*/
static void
VID_Menu_ChooseNextFPSLimit(int dir)
{
  static const int values[] = { 0,   60,  72,  100, 120, 144, 165,
                                180, 200, 240, 300, 360, 500 };
...
*/
static const char*
VID_Menu_GetWaterWarpDesc(void)
{
...
*/
static const char*
VID_Menu_GetParticlesDesc(void)
{
...
*/
static void
VID_Menu_SetAlphaMode(int mode)
{
...
*/
static void
VID_Menu_ChooseNextAlphaMode(int dir)
{
  VID_Menu_SetAlphaMode((VID_Menu_GetAlphaMode() + ALPHAMODE_COUNT + dir) %
                        ALPHAMODE_COUNT);
}
...
*/
static const char*
VID_Menu_GetAlphaModeDesc(void)
{
  switch (VID_Menu_GetAlphaMode()) {
    case ALPHAMODE_BASIC:
      return "Basic";
    case ALPHAMODE_SORTED:
      return "Dynamic";
    case ALPHAMODE_OIT:
      return "Modern";
    default:
...
#define OPTIONS_LIST(def)                                                      \
  def(OPT_VIDEO, "Video Options") def(OPT_CUSTOMIZE, "Controls") def(          \
    OPT_GAMEPAD,                                                               \
    "Gamepad") def(OPT_MODS, "Mods") def(OPT_CONSOLE,                          \
                                         "Go To Console") def(OPT_DEFAULTS,    \
                                                              "Reset Config")  \
                                                                               \
    def(OPT_SPACE1, "")                                                        \
                                                                               \
      def(OPT_GAMMA, "Brightness") def(OPT_CONTRAST, "Contrast") def(          \
        OPT_SCALE, "UI Scale") def(OPT_PIXELASPECT,                            \
                                   "UI Pixels") def(OPT_UIMOUSE, "UI Mouse")   \
        def(OPT_HUDSTYLE, "HUD") def(OPT_SBALPHA, "HUD Alpha") def(            \
          OPT_HUDLEVEL, "HUD Detail") def(OPT_CROSSHAIR, "Crosshair")          \
                                                                               \
          def(OPT_SPACE2, "")                                                  \
                                                                               \
            def(OPT_MOUSESPEED, "Mouse Speed") def(                            \
              OPT_INVMOUSE,                                                    \
              "Invert Mouse") def(OPT_ALWAYSMLOOK,                             \
                                  "Mouse Look") def(OPT_FOV, "Field Of View")  \
              def(OPT_FOVDISTORT, "Gun Distortion") def(                       \
                OPT_RECOIL, "Recoil") def(OPT_VIEWBOB,                         \
                                          "View Bob") def(OPT_ALWAYRUN,        \
                                                          "Always Run")        \
                                                                               \
                def(OPT_SPACE3, "")                                            \
                                                                               \
                  def(OPT_SNDVOL, "Sound Volume")                              \
                    def(OPT_MUSICVOL, "Music Volume") def(                     \
                      OPT_MUSICEXT,                                            \
                      "External Music") ////////////////////////////////////////////////////
#define VIDEO_OPTIONS_LIST(def)                                                \
  def(VID_OPT_RESOLUTION, "Resolution") def(                                   \
    VID_OPT_DISPLAYMODE,                                                       \
    "Display Mode") def(VID_OPT_REFRESHRATE,                                   \
                        "Refresh Rate") def(VID_OPT_TEST,                      \
                                            "Test changes") def(VID_OPT_APPLY, \
                                                                "Apply "       \
                                                                "changes")     \
                                                                               \
    def(VID_OPT_SPACE1, "")                                                    \
                                                                               \
      def(VID_OPT_VSYNC, "Vertical Sync") def(                                 \
        VID_OPT_FSAA,                                                          \
        "Antialiasing") def(VID_OPT_FSAA_MODE,                                 \
                            "AA Mode") def(VID_OPT_SCALE,                      \
                                           "Render Scale") def(VID_OPT_ANISO,  \
                                                               "Anisotropic")  \
        def(VID_OPT_TEXFILTER, "Textures") def(VID_OPT_MD5, "Models") def(     \
          VID_OPT_ANIMLERP, "Animations") def(VID_OPT_PARTICLES, "Particles")  \
          def(VID_OPT_ALPHAMODE, "Transparency") def(                          \
            VID_OPT_WATERWARP,                                                 \
            "Underwater FX") def(VID_OPT_DLIGHTS,                              \
                                 "Dynamic Lights") def(VID_OPT_SOFTEMU,        \
                                                       "8-bit Mode")           \
            def(VID_OPT_FPSLIMIT, "FPS Limit") def(                            \
              VID_OPT_SHOWFPS,                                                 \
              "Show FPS") ////////////////////////////////////////////////////
#define GPAD_OPTIONS_LIST(def)                                                 \
  def(GPAD_OPT_DEVICE, "Gamepad")                                              \
                                                                               \
    def(GPAD_OPT_SPACE1, "") def(GPAD_OPT_SPACE2, "")                          \
                                                                               \
      def(GPAD_OPT_SENSX, "Yaw Speed") def(GPAD_OPT_SENSY, "Pitch Speed") def( \
        GPAD_OPT_INVERT, "Invert Pitch") def(GPAD_OPT_SWAP_MOVELOOK,           \
                                             "Look Stick")                     \
                                                                               \
        def(GPAD_OPT_SPACE3, "")                                               \
                                                                               \
          def(GPAD_OPT_EXPONENT_LOOK,                                          \
              "Look Accel") def(GPAD_OPT_EXPONENT_MOVE, "Move Accel")          \
                                                                               \
            def(GPAD_OPT_SPACE4, "")                                           \
                                                                               \
              def(GPAD_OPT_DEADZONE_LOOK, "Look Deadzone") def(                \
                GPAD_OPT_DEADZONE_MOVE,                                        \
                "Move Deadzone") def(GPAD_OPT_DEADZONE_TRIG, "Trigger Thresh") \
                                                                               \
                def(GPAD_OPT_SPACE5, "")                                       \
                                                                               \
                  def(GPAD_OPT_RUMBLE, "Vibration")                            \
                    ...
                    def(GPAD_OPT_SPACE6, "")                                   \
                                                                               \
                      def(GPAD_OPT_GYROENABLE, "Gyro") def(                    \
                        GPAD_OPT_FLICKSTICK,                                   \
                        "Flick Stick") def(GPAD_OPT_GYROMODE, "Gyro Button")   \
                        def(GPAD_OPT_GYROAXIS, "Gyro Axis") def(               \
                          GPAD_OPT_GYROSENSX,                                  \
                          "Gyro Yaw Speed") def(GPAD_OPT_GYROSENSY,            \
                                                "Gyro Pitch Speed")            \
                          def(GPAD_OPT_GYRONOISE, "Gyro Noise Thresh") def(    \
                            GPAD_OPT_CALIBRATE,                                \
                            "Calibrate") ////////////////////////////////////////////////////
...
#define ADD_OPTION_ENUM(id, name) id,
  OPTIONS_LIST(ADD_OPTION_ENUM) VIDEO_OPTIONS_LIST(ADD_OPTION_ENUM)
...
#undef COUNT_OPTION
  ...
  GYRO_OPTIONS_ITEMS =
    GPAD_OPTIONS_FIRST + GPAD_OPTIONS_ITEMS - GYRO_OPTIONS_FIRST,
};

static qboolean
M_Options_IsGyroId(int id)
{
...
#define ADD_OPTION_NAME(id, name) name,
  OPTIONS_LIST(ADD_OPTION_NAME) VIDEO_OPTIONS_LIST(ADD_OPTION_NAME)
...
M_Options_UpdateLayout(void)
{
  ...
  // Enforce a minimum height, so that if the number of items is relatively
  // small the title pic doesn't get drawn below its usual position
  ...
  if (height <= m_height) {
    ...
    optionsmenu.list.viewsize =
      (m_height - OPTIONS_LISTOFS - optionsmenu.yofs - 8) / 8;
  }
...
static int
M_Options_GetSelected(void)
{
  return optionsmenu.list.cursor + optionsmenu.first_item;
}

static qboolean
M_Options_IsEnabled(int index)
{
  ...
  if ((unsigned int)index >= countof(options_names))
    return false;
  if (index > GPAD_OPTIONS_FIRST &&
      index < GPAD_OPTIONS_FIRST + GPAD_OPTIONS_ITEMS && !IN_HasGamepad())
    return false;
...
static qboolean
M_Options_IsSelectable(int index)
{
...
M_Options_Init(enum m_state_e state)
{
  ...
  if (!M_Options_IsSelectable(optionsmenu.list.cursor))
    M_List_SelectNextActive(
      &optionsmenu.list, optionsmenu.list.cursor, 1, true);
...
static void
M_Options_SetUIMouse(uimouse_t opt)
{
  switch (opt) {
    case UI_MOUSE_OFF:
...
M_AdjustSliders(int dir)
{
  ...
  switch (M_Options_GetSelected()) {
    ...
    case OPT_HUDSTYLE: // hud style
      Cvar_SetValueQuick(
        &scr_hudstyle,
        ((int)q_max(scr_hudstyle.value, 0.f) + (int)HUD_COUNT + dir) %
          (int)HUD_COUNT);
      break;
    ...
    case OPT_RECOIL: // gun kick
      Cvar_SetValueQuick(&v_gunkick,
                         ((int)q_max(v_gunkick.value, 0.f) + 3 + dir) % 3);
      break;

    case OPT_FOV: // field of view
      Cvar_SetValueQuick(&scr_fov,
                         CLAMP(FOV_MIN, scr_fov.value + dir * 5.f, FOV_MAX));
      break;

    case OPT_FOVDISTORT: // FOV distortion
      Cvar_SetValueQuick(&cl_gun_fovscale,
                         CLAMP(0.0, cl_gun_fovscale.value - dir * 0.1, 1.0));
      break;
    ...
    case VID_OPT_PARTICLES:
      Cvar_SetValueQuick(&r_particles,
                         (int)(q_max(r_particles.value, 0.f) + 3 + dir) % 3);
      break;
    case VID_OPT_WATERWARP:
      Cvar_SetValueQuick(&r_waterwarp,
                         (int)(q_max(r_waterwarp.value, 0.f) + 3 + dir) % 3);
      break;
    ...
    case VID_OPT_SOFTEMU:
      Cvar_SetValueQuick(&r_softemu,
                         (int)(q_max(r_softemu.value, 0.f) + 4 + dir) % 4);
      break;
    ...
    case GPAD_OPT_DEVICE:
      // Skip "off" option when using a gamepad to avoid getting into an awkward
      // state on systems where the gamepad is the primary input method, such as
      // the Steam Deck.
      IN_UseNextGamepad(dir,
                        m_lastkey < (int)K_GAMEPAD_BEGIN ||
                          m_lastkey >= (int)K_GAMEPAD_END);
      break;
    case GPAD_OPT_SENSX:
      Cvar_SetValueQuick(&joy_sensitivity_yaw,
                         CLAMP(MIN_JOY_SENS,
                               joy_sensitivity_yaw.value + dir * 10.f,
                               MAX_JOY_SENS));
      break;
    case GPAD_OPT_SENSY:
      Cvar_SetValueQuick(&joy_sensitivity_pitch,
                         CLAMP(MIN_JOY_SENS,
                               joy_sensitivity_pitch.value + dir * 10.f,
                               MAX_JOY_SENS));
      break;
    ...
    case GPAD_OPT_EXPONENT_LOOK:
      Cvar_SetValueQuick(&joy_exponent,
                         CLAMP(MIN_JOY_EXPONENT,
                               joy_exponent.value + dir * 0.5f,
                               MAX_JOY_EXPONENT));
      break;
    case GPAD_OPT_EXPONENT_MOVE:
      Cvar_SetValueQuick(&joy_exponent_move,
                         CLAMP(MIN_JOY_EXPONENT,
                               joy_exponent_move.value + dir * 0.5f,
                               MAX_JOY_EXPONENT));
      break;
    case GPAD_OPT_DEADZONE_LOOK:
      Cvar_SetValueQuick(&joy_deadzone_look,
                         CLAMP(MIN_STICK_DEADZONE,
                               joy_deadzone_look.value + dir * 0.05f,
                               MAX_STICK_DEADZONE));
      break;
    case GPAD_OPT_DEADZONE_MOVE:
      Cvar_SetValueQuick(&joy_deadzone_move,
                         CLAMP(MIN_STICK_DEADZONE,
                               joy_deadzone_move.value + dir * 0.05f,
                               MAX_STICK_DEADZONE));
      break;
    case GPAD_OPT_DEADZONE_TRIG:
      Cvar_SetValueQuick(&joy_deadzone_trigger,
                         CLAMP(MIN_TRIGGER_DEADZONE,
                               joy_deadzone_trigger.value + dir * 0.05f,
                               MAX_TRIGGER_DEADZONE));
      break;
    case GPAD_OPT_RUMBLE:
      Cvar_SetValueQuick(&joy_rumble,
                         CLAMP(0.f, joy_rumble.value + dir * 0.1f, 1.f));
      break;
    ...
    case GPAD_OPT_GYROSENSX:
      Cvar_SetValueQuick(&gyro_yawsensitivity,
                         CLAMP(MIN_GYRO_SENS,
                               gyro_yawsensitivity.value + dir * .1f,
                               MAX_GYRO_SENS));
      break;
    case GPAD_OPT_GYROSENSY:
      Cvar_SetValueQuick(&gyro_pitchsensitivity,
                         CLAMP(MIN_GYRO_SENS,
                               gyro_pitchsensitivity.value + dir * .1f,
                               MAX_GYRO_SENS));
      break;
    case GPAD_OPT_GYRONOISE:
      Cvar_SetValueQuick(&gyro_noise_thresh,
                         CLAMP(MIN_GYRO_NOISE_THRESH,
                               gyro_noise_thresh.value + dir * .5f,
                               MAX_GYRO_NOISE_THRESH));
      break;
...
void
M_DrawSliderWithMarkers(int x,
                        int y,
                        float range,
                        const slidermarker_t* markers,
                        int nummarkers,
                        const char* desc)
{
  int i;
  ...
  for (i = 0; i < nummarkers; i++) {
    ...
    M_DrawCharacter(
      x + (int)((SLIDER_RANGE - 1) * 8 * CLAMP(0.f, marker->frac, 1.f) + 0.5f),
      y + marker->yofs,
      marker->glyph);
  }
...
void
M_DrawSlider(int x, int y, float range, const char* desc)
{
  M_DrawSliderWithMarkers(x, y, range, NULL, 0, desc);
}

void
M_DrawThresholdSlider(int x,
                      int y,
                      float range,
                      qboolean enabled,
                      float live,
...
qboolean
M_SetSliderValue(int option, float f)
{
  float l;
  f = CLAMP(0.f, f, 1.f);
...
float
M_MouseToRawSliderFraction(int cx)
{
  return (cx - 4) / (float)((SLIDER_RANGE - 1) * 8);
}

float
M_MouseToSliderFraction(int cx)
{
...
static void
M_Options_DrawItem(int y, int item)
{
  char buf[256];
  const char* str;
  ...
  COM_TintSubstring(
    options_names[item], optionsmenu.list.search.text, buf, sizeof(buf));
  ...
  switch (item) {
    ...
    case VID_OPT_DISPLAYMODE:
      switch (VID_Menu_GetDisplayMode()) {
        case DISPLAYMODE_FULLSCREEN:
          M_Print(x, y, "Fullscreen");
          break;
        case DISPLAYMODE_WINDOWED:
          M_Print(x, y, "Windowed");
          break;
        case DISPLAYMODE_BORDERLESS:
          M_Print(x, y, "Borderless");
          break;
        default:
          M_Print(x, y, "Other");
          break;
    ...
    case VID_OPT_FSAA:
      M_Print(x,
              y,
              framebufs.scene.samples >= 2 ? va("%ix", framebufs.scene.samples)
                                           : "Off");
      break;
    case VID_OPT_FSAA_MODE:
    ...
    case VID_OPT_ANISO:
      M_Print(
        x,
        y,
        gl_texture_anisotropy.value >= 2.f
          ? va("%ix",
               q_min((int)gl_texture_anisotropy.value, (int)gl_max_anisotropy))
          : "Off");
      break;
    case VID_OPT_TEXFILTER:
    ...
    case VID_OPT_FPSLIMIT:
      M_Print(
        x, y, host_maxfps.value ? va("%i", (int)host_maxfps.value) : "Off");
      break;
    case VID_OPT_SHOWFPS:
    ...
    case GPAD_OPT_DEVICE:
      ...
      M_PrintWordWrap(x,
                      ...
                      320 - x,
                      16,
                      true); // note: hijacking the empty line below this option
      break;
    case GPAD_OPT_SENSX:
      r = (joy_sensitivity_yaw.value - MIN_JOY_SENS) /
          (MAX_JOY_SENS - MIN_JOY_SENS);
      M_DrawSlider(x, y, r, va("%.0f", joy_sensitivity_yaw.value));
      break;
    case GPAD_OPT_SENSY:
      r = (joy_sensitivity_pitch.value - MIN_JOY_SENS) /
          (MAX_JOY_SENS - MIN_JOY_SENS);
    ...
    case GPAD_OPT_SWAP_MOVELOOK:
      ...
      break;
    case GPAD_OPT_EXPONENT_LOOK:
      r = (joy_exponent.value - MIN_JOY_EXPONENT) /
          (MAX_JOY_EXPONENT - MIN_JOY_EXPONENT);
      M_DrawSlider(x, y, r, va("%.1f", joy_exponent.value));
      break;
    case GPAD_OPT_EXPONENT_MOVE:
      r = (joy_exponent_move.value - MIN_JOY_EXPONENT) /
          (MAX_JOY_EXPONENT - MIN_JOY_EXPONENT);
      M_DrawSlider(x, y, r, va("%.1f", joy_exponent_move.value));
      break;
    case GPAD_OPT_DEADZONE_LOOK:
      r = (joy_deadzone_look.value - MIN_STICK_DEADZONE) /
          (MAX_STICK_DEADZONE - MIN_STICK_DEADZONE);
      l = (IN_GetRawLookMagnitude() - MIN_STICK_DEADZONE) /
          (MAX_STICK_DEADZONE - MIN_STICK_DEADZONE);
      M_DrawThresholdSlider(x,
                            y,
                            r,
                            selected && IN_HasGamepad(),
                            l,
                            va("%.0f%%", joy_deadzone_look.value * 100.f));
      break;
    case GPAD_OPT_DEADZONE_MOVE:
      r = (joy_deadzone_move.value - MIN_STICK_DEADZONE) /
          (MAX_STICK_DEADZONE - MIN_STICK_DEADZONE);
      l = (IN_GetRawMoveMagnitude() - MIN_STICK_DEADZONE) /
          (MAX_STICK_DEADZONE - MIN_STICK_DEADZONE);
      M_DrawThresholdSlider(x,
                            y,
                            r,
                            selected && IN_HasGamepad(),
                            l,
                            va("%.0f%%", joy_deadzone_move.value * 100.f));
      break;
    case GPAD_OPT_DEADZONE_TRIG:
      r = (joy_deadzone_trigger.value - MIN_TRIGGER_DEADZONE) /
          (MAX_TRIGGER_DEADZONE - MIN_TRIGGER_DEADZONE);
      l = (IN_GetRawTriggerMagnitude() - MIN_TRIGGER_DEADZONE) /
          (MAX_TRIGGER_DEADZONE - MIN_TRIGGER_DEADZONE);
      M_DrawThresholdSlider(x,
                            y,
                            r,
                            selected && IN_HasGamepad(),
                            l,
                            va("%.0f%%", joy_deadzone_trigger.value * 100.f));
      break;
    case GPAD_OPT_RUMBLE:
      r = joy_rumble.value;
      if (!IN_HasRumble())
    ...
    case GPAD_OPT_GYROMODE:
      switch ((int)gyro_mode.value) {
        case GYRO_BUTTON_ENABLES:
          M_Print(x, y, "Enable gyro");
          break;
        case GYRO_BUTTON_DISABLES:
          M_Print(x, y, "Disable gyro");
          break;
        case GYRO_BUTTON_INVERTS_DIR:
          M_Print(x, y, "Invert dir");
          break;
        default:
          M_Print(x, y, "Ignore");
          break;
    ...
    case GPAD_OPT_GYROAXIS:
      ...
      break;
    case GPAD_OPT_GYROSENSX:
      r = (gyro_yawsensitivity.value - MIN_GYRO_SENS) /
          (MAX_GYRO_SENS - MIN_GYRO_SENS);
      M_DrawSlider(x, y, r, va("%.1f", gyro_yawsensitivity.value));
      break;
    case GPAD_OPT_GYROSENSY:
      r = (gyro_pitchsensitivity.value - MIN_GYRO_SENS) /
          (MAX_GYRO_SENS - MIN_GYRO_SENS);
      M_DrawSlider(x, y, r, va("%.1f", gyro_pitchsensitivity.value));
      break;
    case GPAD_OPT_GYRONOISE:
      r = (gyro_noise_thresh.value - MIN_GYRO_NOISE_THRESH) /
          (MAX_GYRO_NOISE_THRESH - MIN_GYRO_NOISE_THRESH);
      l = (IN_GetRawGyroMagnitude() - MIN_GYRO_NOISE_THRESH) /
          (MAX_GYRO_NOISE_THRESH - MIN_GYRO_NOISE_THRESH);
      M_DrawThresholdSlider(x,
                            y,
                            r,
                            selected && IN_HasGyro() && gyro_enable.value,
                            l,
                            va("%.1f", gyro_noise_thresh.value));
      break;
...
M_Options_Draw(void)
{
  ...
  *optionsmenu.last_cursor = optionsmenu.list.cursor;
  ...
  if (optionsmenu.subtitle && optionsmenu.subtitle[0])
    M_PrintWhite((320 - 8 * strlen(optionsmenu.subtitle)) / 2,
                 y - 16,
                 optionsmenu.subtitle);
  ...
  if (M_List_GetOverflow(&optionsmenu.list) > 0) {
    if (optionsmenu.list.scroll > 0)
      M_DrawEllipsisBar(x, y - 8, cols);
    if (optionsmenu.list.scroll + optionsmenu.list.viewsize <
...
M_Options_Mousemove(int cx, int cy)
{
  ...
  M_List_Mousemove(&optionsmenu.list,
                   cy - optionsmenu.y - OPTIONS_LISTOFS - optionsmenu.yofs);
}
...
static void
M_Keys_UpdateLayout(void)
{
  int height;

  M_UpdateBounds();

  // Note: we use NUMCOMMANDS instead of keysmenu.list.numitems to have a stable
  // layout when switching between keyboard+mouse/gamepad tabs (different number
  // of items)
...
static qboolean
M_Keys_IsSelectable(int index)
{
  return keysmenu.items[index].command[0] != '\0';
}

static qboolean
M_Keys_Match(int index)
{
...
M_Keys_Populate(void)
{
  ...
  for (i = 0; i < limit; i++) {
    ...
    if (VEC_SIZE(keysmenu.items) > 0 && !menubinds[i].command[0] &&
        !VEC_LAST(keysmenu.items).command[0])
...
M_Menu_Keys_f(void)
{
  ...
  keysmenu.devicemask =
    (lastactive == KD_GAMEPAD) ? KDM_GAMEPAD : KDM_KEYBOARD_AND_MOUSE;
...
void
M_FindKeysForCommand(const char* command, int* threekeys)
{
...
M_Keys_Draw(void)
{
  ...
  // draw tabs
  M_PrintAlignedEx(320 * 1 / 4,
                   y + 32,
                   ALIGN_CENTER,
                   8,
                   keysmenu.devicemask == KDM_GAMEPAD,
                   "Keyboard & Mouse");
  M_PrintAlignedEx(320 * 3 / 4,
                   y + 32,
                   ALIGN_CENTER,
                   8,
                   keysmenu.devicemask != KDM_GAMEPAD,
  ...
  while (numvis-- > 0) {
    ...
    if (keysmenu.items[i].command[0]) {
      ...
      COM_TintSubstring(keysmenu.items[i].description,
                        keysmenu.list.search.text,
                        buf,
                        sizeof(buf));
  ...
  if (bind_grab)
    hint = keysmenu.devicemask == KDM_GAMEPAD
             ? va("Press new button, or %s to cancel",
                  Key_KeynumToFriendlyString(K_START))
             : va("Press new key, or %s to cancel",
                  Key_KeynumToFriendlyString(K_ESCAPE));
  else
    hint = IN_GetLastActiveDeviceType() == KD_GAMEPAD
             ? va("%s = change, %s = clear",
                  Key_KeynumToFriendlyString(K_ABUTTON),
                  Key_KeynumToFriendlyString(K_YBUTTON))
             : va("%s = change, %s = clear",
                  Key_KeynumToFriendlyString(K_ENTER),
                  Key_KeynumToFriendlyString(K_BACKSPACE));
...
M_Menu_Quit_f(void)
{
  ...
  m_state = m_quit;
  m_entersound = true;
  msgNumber = (cl_confirmquit.value >= 2.f)
                ? (int)(realtime * (5.0 * 1.61803399)) & 7
                : 8;
}
...
textmode_t
M_Quit_TextEntry(void)
{
  return TEXTMODE_NOPOPUP;
}

void
M_Quit_Draw(void) // johnfitz -- modified for new quit message
{
...
M_LanConfig_Draw(void)
{
  ...
  if (lanConfig_cursor == 0)
    M_DrawTextCursor(basex + 9 * 8 + 8 * strlen(lanConfig_portname),
                     lanConfig_cursor_table[0]);

  if (lanConfig_cursor == 2)
    M_DrawTextCursor(basex + 16 + 8 * strlen(lanConfig_joinname),
...
textmode_t
M_LanConfig_TextEntry(void)
{
  return (lanConfig_cursor == 0 || lanConfig_cursor == 2) ? TEXTMODE_ON
                                                          : TEXTMODE_OFF;
}
...
M_LanConfig_Mousemove(int cx, int cy)
{
  ...
  M_UpdateCursorWithTable(cy,
                          ...
                          NUM_LANCONFIG_CMDS - StartingGame,
                          &lanConfig_cursor);
...
M_GameOptions_Mousemove(int cx, int cy)
{
  ...
  M_UpdateCursorWithTable(
    cy, gameoptions_cursor_table, NUM_GAMEOPTIONS, &gameoptions_cursor);
...
static qboolean
M_Mods_IsActive(const char* game)
{
...
static void
M_Mods_AddDecoration(const char* text)
{
  moditem_t item;
  memset(&item, 0, sizeof(item));
  item.name = text;
...
static void
M_Mods_AddSeparator(qboolean installed)
{
#define QBAR "\35\36\37"
...
static void
M_Mods_Add(const filelist_item_t* item)
{
  moditem_t mod;
...
static qboolean
M_Mods_IsSelectable(int index)
{
  return modsmenu.items[index].source != NULL;
}

static void
M_Mods_UpdateLayout(void)
{
  int height;
...
M_Mods_Draw(void)
{
  ...
  modsmenu.download_flash_time =
    q_max(0.0, modsmenu.download_flash_time - host_rawframetime);
  ...
  for (i = 0; i < numvis; i++) {
    ...
    int mask = item->active ? 128 : 0;
    const char* message =
      item->source ? Modlist_GetFullName(item->source) : NULL;
    ...
    if (!item->source) {
      ...
      if (modsmenu.list.search.len > 0)
        COM_TintSubstring(
          item->name, modsmenu.list.search.text, tinted, sizeof(tinted));
      else
      ...
      if (Modlist_GetStatus(item->source) == MODSTATUS_INSTALLING) {
        ...
        q_snprintf(
          buf, sizeof(buf), "\20%3.0f%%\21 %s", 100.0 * progress, item->name);
        if (flash)
          mask ^= 128;
      } else
      ...
      if (message && message[0]) {
        if (modsmenu.list.search.len > 0)
          COM_TintSubstring(
            message, modsmenu.list.search.text, buf, sizeof(buf));
        else
          q_strlcpy(buf, message, sizeof(buf));
...
void
M_OnModInstall(const char* name)
{
...
M_ModInfo_UpdateLayout(void)
{
  ...
  modinfomenu.y =
    m_top + (((m_height - height) / 2) & ~7) + MODINFO_BOXMARGIN * 8;
}
...
M_Menu_ModInfo_f(const filelist_item_t* item)
{
  ...
  m_entersound = true;
  if (Modlist_IsInstalling() ||
      Modlist_GetStatus(item) != MODSTATUS_DOWNLOADABLE) {
...
M_ModInfo_Draw(void)
{
  ...
  if (*str) {
    ...
    y +=
      M_PrintWordWrap(x2, y, str, MODINFO_INFOCOLS * 8, maxlines * 8, false) *
        8 +
      8;
  }
...
M_Init(void)
{
  ...
  Cvar_SetCallback(&ui_mouse, (cvarcallback_t)UI_Mouse_f);
...
M_Draw(void)
{
  ...
  switch (m_state) {
    ...
    case m_quit:
      if (!fitzmode && !cl_confirmquit.value) { /* QuakeSpasm customization: */
        /* Quit now! S.A. */
        key_dest = key_console;
        Host_Quit_f();
      }
...
static qboolean
M_CheckCustomGfx(const char* custompath,
                 const char* basepath,
                 int knownlength,
...
M_CheckMods(void)
{
  const unsigned int main_hashes[] = { 0x136bc7fd, 0x90555cb4 },
  ...
  m_singleplayer_showlevels = M_CheckCustomGfx(
    "gfx/sp_maps.lmp", "gfx/sp_menu.lmp", 14856, sp_hashes, countof(sp_hashes));
