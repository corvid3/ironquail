...
void
VectorAngles(const vec3_t forward, vec3_t angles)
{
  vec3_t temp;
...
#define VBROADCAST(vec, col)                                                   \
  _mm_shuffle_ps(vec, vec, _MM_SHUFFLE(col, col, col, col))
...
{
#define COPY_ROW(row)                                                          \
  dst[row * 4 + 0] = src[row + 0], dst[row * 4 + 1] = src[row + 4],            \
