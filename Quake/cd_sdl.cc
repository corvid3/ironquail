...
CDAudio_Play(byte track, qboolean looping)
{
  ...
  if (SDL_CDPlay(cd_handle,
                 cd_handle->track[track - 1].offset,
                 cd_handle->track[track - 1].length) < 0) {
    Con_Printf(
      "CDAudio_Play: Unable to play track %d: %s\n", track, SDL_GetError());
    return -1;
  }
...
CD_f(void)
{
  ...
  if (q_strcasecmp(command, "info") == 0) {
    ...
    if (playing || wasPlaying) {
      ...
      FRAMES_TO_MSF(
        cd_handle->cur_frame, &current_min, &current_sec, &current_frame);
      FRAMES_TO_MSF(cd_handle->track[playTrack - 1].length,
                    &length_min,
                    &length_sec,
                    &length_frame);
