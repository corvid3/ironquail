...
*/
static int
JSON_ReadHexDigit(const char* str)
{
...
*/
static qboolean
JSON_ReadHexNumber(const char* str, int* out)
{
...
*/
static char*
JSON_Unescape(char* dst, const char* src, int len)
{
  const char* srcend;
        ...
        if (srcend - src > 4) {
          ...
          if (JSON_ReadHexNumber(src + 1, &codepoint)) {
            if (codepoint >= 0xd800 && codepoint < 0xdbff &&
                srcend - src > 10 && src[5] == '\\' && src[6] == 'u' &&
                JSON_ReadHexNumber(src + 7, &lowsurrogate) &&
...
JSON_Parse(const char* text)
{
  ...
  json =
    (json_t*)calloc(sizeof(json_t) + sizeof(jsonentry_t) * numtokens + len, 1);
  if (!json)
    goto free_tokens;
...
*/
const jsonentry_t*
JSON_Find(const jsonentry_t* entry, const char* name, jsontype_t type)
{
  if (!entry)
    return NULL;
...
*/
const char*
JSON_FindString(const jsonentry_t* entry, const char* name)
{
...
*/
const double*
JSON_FindNumber(const jsonentry_t* entry, const char* name)
{
...
*/
const qboolean*
JSON_FindBoolean(const jsonentry_t* entry, const char* name)
{
