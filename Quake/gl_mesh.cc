...
*/
void
GL_MakeAliasModelDisplayLists(qmodel_t* aliasmodel, aliashdr_t* paliashdr)
{
  int i, j;
  int mark;
  trivertx_t* verts;
  ...
  verts = (trivertx_t*)Hunk_AllocNoFill(
  ...
  indexes =
    (unsigned short*)Hunk_Alloc(sizeof(unsigned short) * pheader->numtris * 3);
...
*/
void
GLMesh_LoadVertexBuffer(qmodel_t* m, aliashdr_t* mainhdr)
{
  int totalvbosize = 0;
  int animsize = 0;
  ...
  for (hdr = mainhdr, numverts = 0, numindexes = 0;;) {
    ...
    numindexes += hdr->numindexes;
    memcpy(ebodata + hdr->eboofs,
           (short*)((byte*)hdr + hdr->indexes),
    ...
    switch (hdr->poseverttype) {
      case aliashdr_t::PV_QUAKE1:
                  ...
                  // numposes in QuakeSpasm
        {
          ...
          const trivertx_t* tv =
            (const trivertx_t*)trivertexes + (hdr->numverts * f);
      ...
      case aliashdr_t::PV_IQM:
                  ...
                  // numposes in QuakeSpasm
        {
          ...
          const iqmvert_t* tv =
            (const iqmvert_t*)trivertexes + (hdr->numverts_vbo * f);
          vertofs += hdr->numverts_vbo * sizeof(*xyz);
        ...
        memcpy(vbodata + hdr->vboposeofs,
               (byte*)hdr + hdr->boneposedata,
               hdr->numboneposes * hdr->numbones * sizeof(bonepose_t));
  ...
  m->meshindexesvbo = GL_CreateBuffer(GL_ELEMENT_ARRAY_BUFFER,
                                      GL_STATIC_DRAW,
                                      va("%s indices", m->name),
  ...
  m->meshvbo = GL_CreateBuffer(GL_ARRAY_BUFFER,
                               GL_STATIC_DRAW,
                               va("%s vertices", m->name),
                               totalvbosize,
                               vbodata);
