...
cvar_t cl_name = { "_cl_name", "player", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t cl_color = { "_cl_color", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };

cvar_t cl_shownet = {
  "cl_shownet", "0", CVAR_NONE, 0, 0, 0, 0, 0
}; // can be 0, 1, or 2
cvar_t cl_nolerp = { "cl_nolerp", "0", CVAR_NONE, 0, 0, 0, 0, 0 };

cvar_t cfg_unbindall = { "cfg_unbindall", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };

cvar_t freelook = { "freelook", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t lookspring = { "lookspring", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t lookstrafe = { "lookstrafe", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t sensitivity = { "sensitivity", "3", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };

cvar_t m_pitch = { "m_pitch", "0.022", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t m_yaw = { "m_yaw", "0.022", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t m_forward = { "m_forward", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t m_side = { "m_side", "0.8", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };

cvar_t cl_maxpitch = {
  "cl_maxpitch", "90", CVAR_ARCHIVE, 0, 0, 0, 0, 0
}; // johnfitz -- variable pitch clamping
cvar_t cl_minpitch = {
  "cl_minpitch", "-90", CVAR_ARCHIVE, 0, 0, 0, 0, 0
}; // johnfitz -- variable pitch clamping

cvar_t cl_mwheelpitch = { "cl_mwheelpitch", "5", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };

cvar_t cl_startdemos = { "cl_startdemos", "1", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t cl_confirmquit = { "cl_confirmquit", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
...
CL_ClearState(void)
{
  ...
  cl_entities =
    (entity_t*)Hunk_AllocName(cl_max_edicts * sizeof(entity_t), "cl_entities");
  // johnfitz
...
*/
void
CL_SetLightstyle(int i, const char* str)
{
...
*/
static void
CL_RocketTrail(entity_t* ent, int type)
{
...
CL_RelinkEntities(void)
{
  ...
  for (i = 1, ent = cl_entities + 1; i < cl.num_entities; i++, ent++) {
    ...
    if (ent->msgtime != cl.mtime[0]) {
      ...
      ent->lerpflags |=
        LERP_RESETMOVE |
        LERP_RESETANIM; // johnfitz -- next time this entity slot is reused, the
                        // lerp will need to be reset
      continue;
    }
...
*/
static void
CL_Viewpos_Completion_f(const char* partial)
{
  if (Cmd_Argc() != 2)
    return;
