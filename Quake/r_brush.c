...
*/
static void
Chart_Init(chart_t* chart, int width, int height)
{
  if (chart->width != width) {
    chart->allocated =
      (int*)realloc(chart->allocated, sizeof(chart->allocated[0]) * width);
    if (!chart->allocated)
      Sys_Error("Chart_Init: could not allocate %" SDL_PRIu64 " bytes",
                (uint64_t)(sizeof(chart->allocated[0]) * width));
  }
...
*/
static qboolean
Chart_Add(chart_t* chart, int w, int h, short* outx, short* outy)
{
  int i, x, y;
  if (chart->width < w || chart->height < h)
    Sys_Error("Chart_Add: block too large %dx%d, max is %dx%d",
              w,
              h,
              chart->width,
              chart->height);
...
*/
static int
AllocBlock(int w, int h, short* x, short* y)
{
  int texnum;
  ...
  for (texnum = last_lightmap_allocated;
    ...
    if (texnum == lightmap_count) {
      lightmap_count++;
      lightmaps =
        (lightmap_t*)realloc(lightmaps, sizeof(*lightmaps) * lightmap_count);
      memset(&lightmaps[texnum], 0, sizeof(lightmaps[texnum]));
      // as we're only tracking one texture, we don't need multiple copies any
      // more.
...
*/
static int
GL_NumLightmapTaps(const msurface_t* surf)
{
...
*/
static void
GL_FillSurfaceLightmap(msurface_t* surf)
{
  lightmap_t* lm;
  int smax, tmax;
  int xofs, yofs;
  ...
  } else if (surf->styles[2] == 255) // 2 lightstyles
  {
    for (t = 0; t < tmax; t++, dst += lightmap_width) {
      for (s = 0; s < smax; s++, src += 3) {
        dst[s] = src[0] | (src[1] << 8) | (src[2] << 16) | 0xff000000u;
        dst[s + smax] = src[0 + facesize] | (src[1 + facesize] << 8) |
                        (src[2 + facesize] << 16) | 0xff000000u;
      }
    }
  } else // 3 or 4 lightstyles
  {
    for (t = 0; t < tmax; t++, dst += lightmap_width) {
      for (s = 0; s < smax; s++, src += 3) {
        ...
        for (map = 0; map < 4 && surf->styles[map] != 255;
             map++, mapsrc += facesize) {
          r |= mapsrc[0] << (map << 3);
          g |= mapsrc[1] << (map << 3);
          b |= mapsrc[2] << (map << 3);
...
GL_PackLitSurfaces(void)
{
  ...
  blacklm =
    AllocBlock(maxblack[0] + 1, maxblack[1] + 1, &blackofs[0], &blackofs[1]);

  if (VEC_SIZE(lit_surfs) == 0)
    return;

  lit_surf_order[0] = (int*)realloc(
    lit_surf_order[0], sizeof(lit_surf_order[0][0]) * VEC_SIZE(lit_surfs));
  lit_surf_order[1] = (int*)realloc(
    lit_surf_order[1], sizeof(lit_surf_order[1][0]) * VEC_SIZE(lit_surfs));

  if (!lit_surf_order[0] || !lit_surf_order[1])
    Sys_Error("GL_PackLitSurfaces: out of memory (%" SDL_PRIu64 " surfs)",
              (uint64_t)VEC_SIZE(lit_surfs));
...
GL_BuildLightmaps(void)
{
  ...
  if (q_max(lightmap_width, lightmap_height) > gl_max_texture_size) {
    // dimensions get zero-ed out in GL_FreeLightmapData, save them for the
    // error message
  ...
  Con_DPrintf("Lightmap size:   %d x %d (%d/%d blocks)\n"
              ...
              lightmap_count,
              xblocks * yblocks,
              (lightmap_bytes * lmsize) / (float)0x100000,
              100.0 * num_lightmap_samples / lmsize);

  lightmap_data = (unsigned*)calloc(lmsize, sizeof(*lightmap_data));
  if (!lightmap_data)
    Sys_Error("GL_BuildLightmaps: out of memory on %" SDL_PRIu64 " bytes",
              (uint64_t)(lmsize * sizeof(*lightmap_data)));
  ...
  lightmap_texture =
    TexMgr_LoadImage(cl.worldmodel,
                     "lightmap",
                     lightmap_width,
                     lightmap_height,
                     SRC_LIGHTMAP,
...
*/
void
GL_BuildBModelVertexBuffer(void)
{
  ...
  if (!varray)
    Sys_Error("GL_BuildBModelVertexBuffer: out of memory on %u bytes",
              varray_bytes);
  varray_index = 0;
    ...
    for (i = 0; i < m->numsurfaces; i++) {
      ...
      if (fa->flags & SURF_DRAWTILED) {
        ...
        if (fa->flags & SURF_DRAWTURB) {
          texscaley = texscalex =
            1.f / 128.f; // warp animation repeats every 128
          useofs = 0.f;
        } else {
      ...
      for (k = 0; k < fa->numedges; k++, vert++) {
        ...
        s = DotProduct(vec, fa->texinfo->vecs[0]) +
            fa->texinfo->vecs[0][3] * useofs;
        s *= texscalex;

        t = DotProduct(vec, fa->texinfo->vecs[1]) +
            fa->texinfo->vecs[1][3] * useofs;
        t *= texscaley;
        ...
        if (!(fa->flags & SURF_DRAWTILED)) {
          ...
          vert->st[3] = t;
          vert->lmofs = lmofs;
          vert->styles = fa->styles[0] | (fa->styles[1] << 8) |
                         (fa->styles[2] << 16) | (fa->styles[3] << 24);
        } else {
  ...
  gl_bmodel_vbo = GL_CreateBuffer(
    GL_ARRAY_BUFFER, GL_STATIC_DRAW, "brushverts", varray_bytes, varray);
  free(varray);
}
...
*/
void
GL_BuildBModelMarkBuffers(void)
{
  int i, j, k, sum;
  ...
  if (!idx)
    Sys_Error("GL_BuildBModelMarkBuffers: out of memory (%d indices)",
              numtris * 3);
  leafs = (bmodel_gpu_leaf_t*)calloc(cl.worldmodel->numleafs, sizeof(leafs[0]));
  if (!leafs)
    Sys_Error("GL_BuildBModelMarkBuffers: out of memory (%d leafs)",
              cl.worldmodel->numleafs);
  surfs =
    (bmodel_gpu_surf_t*)calloc(cl.worldmodel->numsurfaces, sizeof(surfs[0]));
  if (!surfs)
    Sys_Error("GL_BuildBModelMarkBuffers: out of memory (%d surfs)",
              cl.worldmodel->numsurfaces);
  texidx = (int*)calloc(maxnumtex, sizeof(texidx[0]));
  if (!texidx)
    Sys_Error("GL_BuildBModelMarkBuffers: out of memory (%d tex indices)",
              maxnumtex);
  ...
  for (i = 0; i < cl.worldmodel->numleafs; i++) {
    ...
    dst->surfcountsky =
      (src->nummarksurfaces << 1) | (src->contents == CONTENTS_SKY);
  }
  ...
  for (i = 0; i < cl.worldmodel->numsurfaces; i++) {
    ...
    if (src->texinfo->texnum < 0 ||
        src->texinfo->texnum >= cl.worldmodel->numtextures)
      Sys_Error("GL_BuildBModelMarkBuffers: bad texnum %d (total=%d)",
                src->texinfo->texnum,
  ...
  for (j = 1; j < MAX_MODELS; j++) {
    ...
    for (i = 0, s = m->surfaces + m->firstmodelsurface; i < m->nummodelsurfaces;
         i++, s++)
      cmds[m->firstcmd + texidx[s->texinfo->texnum]].count +=
        (q_max(s->numedges, 2) - 2) * 3;
  }
  ...
  for (j = 1; j < MAX_MODELS; j++) {
    ...
    for (i = 0, s = m->surfaces + m->firstmodelsurface; i < m->nummodelsurfaces;
         i++, s++) {
      bmodel_draw_indirect_t* draw =
        &cmds[m->firstcmd + texidx[s->texinfo->texnum]];
  ...
  // create gpu buffers
  gl_bmodel_indirect_buffer = GL_CreateBuffer(GL_SHADER_STORAGE_BUFFER,
                                              GL_DYNAMIC_DRAW,
                                              "bmodel indirect cmds",
                                              sizeof(cmds[0]) * numtex,
                                              cmds);
  gl_bmodel_ibo = GL_CreateBuffer(GL_ELEMENT_ARRAY_BUFFER,
                                  GL_DYNAMIC_DRAW,
                                  "bmodel indices",
                                  sizeof(idx[0]) * numtris * 3,
                                  idx);
  gl_bmodel_leaf_buffer =
    GL_CreateBuffer(GL_SHADER_STORAGE_BUFFER,
                    GL_STATIC_DRAW,
                    "bmodel leafs",
                    sizeof(leafs[0]) * cl.worldmodel->numleafs,
                    leafs);
  gl_bmodel_surf_buffer =
    GL_CreateBuffer(GL_SHADER_STORAGE_BUFFER,
                    GL_STATIC_DRAW,
                    "bmodel surfs",
                    sizeof(surfs[0]) * cl.worldmodel->numsurfaces,
                    surfs);
  gl_bmodel_marksurf_buffer = GL_CreateBuffer(
    GL_SHADER_STORAGE_BUFFER,
    GL_STATIC_DRAW,
    "bmodel marksurfs",
    sizeof(cl.worldmodel->marksurfaces[0]) * cl.worldmodel->nummarksurfaces,
    cl.worldmodel->marksurfaces);
