...
static const keyname_t keynames[] = {
  ...
  { "NUMLOCK",
    K_KP_NUMLOCK }, // Note: added a second time, without the KP_ prefix, for
                    // consistency with the other LOCK keys
  ...
  // We don't expose the key names in order to avoid having the player bind them
  // in the console.
  //{"JOY_START", K_START},
  //{"JOY_BACK", K_BACK},
...
#define GAMEPAD_KEY_NAME(keycode, value, xboxname, psname, nintendoname)       \
  xboxname,
...
Key_Console(int key)
{
  ...
  switch (key) {
    ...
    case K_BACKSPACE:
      ...
      if (key_linepos > 1) {
        int numchars =
          keydown[K_CTRL] ? key_linepos - Key_FindWordBoundary(-1) : 1;
    ...
    case K_DEL:
      ...
      if (*workline) {
        int numchars =
          keydown[K_CTRL] ? Key_FindWordBoundary(1) - key_linepos : 1;
        SDL_assert(numchars > 0);
        len = strlen(workline);
        SDL_assert((int)len >= numchars);
        memmove(workline, workline + numchars, len + 1 - numchars);
    ...
    case K_HOME:
      if (keydown[K_CTRL]) {
        ...
        con_backscroll = CLAMP(0,
                               con_current - i % con_totallines - 2,
    ...
    case K_DPAD_RIGHT:
      ...
      if ((int)len == key_linepos) {
        ...
        workline += key_linepos;
        *workline =
          key_lines[(edit_line + (CMDLINES - 1)) & (CMDLINES - 1)][key_linepos];
        workline[1] = 0;
        key_linepos++;
      } else {
        if (keydown[K_CTRL])
...
const char*
Key_GetChatBuffer(void)
{
  return chat_buffer;
}

int
Key_GetChatMsgLen(void)
{
...
*/
const char*
Key_KeynumToFriendlyString(int keynum)
{
...
*/
keydevice_t
Key_GetDeviceForKeynum(int keynum)
{
...
*/
keydevicemask_t
Key_GetDeviceMaskForKeynum(int keynum)
{
...
*/
int
Key_GetKeysForCommand(const char* command,
                      int* keys,
                      int maxkeys,
                      keydevicemask_t devmask)
{
  int i, count;
...
*/
void
Key_GetGrabbedInput(int* lastkey, int* lastchar)
{
  if (lastkey)
...
*/
void
Key_EventWithKeycode(int key, qboolean down, int keycode)
{
  char* kb;
  char cmd[1024];
  qboolean wasdown;
    ...
    if (keydown[key]) {
      ...
      if (Key_GetKeysForCommand("menu_options", &optkey, 1, KDM_ANY))
        Con_Printf("%s is unbound, hit %s to set.\n",
                   ...
                   Key_KeynumToFriendlyString(optkey));
      else
  ...
  // generate char events if we want text input without popping up an on-screen
  // keyboard when a physical one isn't present, e.g. when using a searchable
  // menu on the Steam Deck
  ...
  if (cls.demoplayback && key_dest == key_game) {
    switch (key) {
      ...
      case K_SHIFT:
      case K_CTRL:
        // Temporary modifiers: they don't perform their actions on up/down
        // events, but are queried per frame instead to avoid having to manage
        // state transitions (e.g. pressing esc while still holding left arrow
        // to rewind).
        return;
