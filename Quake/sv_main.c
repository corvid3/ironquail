...
SV_CalcStats(client_t* client, int* statsi, float* statsf, const char** statss)
{
  ...
  statsf[STAT_ACTIVEWEAPON] =
    ent->v.weapon; // sent in a way that does NOT depend upon the current mod...
  ...
  for (i = 0; i < sv.numcustomstats; i++) {
    ...
    switch (sv.customstats[i].type) {
      ...
      case ev_string: // not supported in this build... send with
                      // svcfte_updatestatstring on change, which is annoying.
...
SV_Init(void)
{
  ...
  Cvar_SetCallback(&sv_gravity, (cvarcallback_t)Host_Callback_Notify);
  Cvar_SetCallback(&sv_friction, (cvarcallback_t)Host_Callback_Notify);
  Cvar_RegisterVariable(&sv_edgefriction);
  Cvar_RegisterVariable(&sv_stopspeed);
  Cvar_RegisterVariable(&sv_maxspeed);
  Cvar_SetCallback(&sv_maxspeed, (cvarcallback_t)Host_Callback_Notify);
...
*/
void
SV_LocalSound(client_t* client, const char* sample)
{
...
static qboolean
SV_IsLocalClient(client_t* client)
{
...
*/
qboolean
SV_VisibleToClient(edict_t* client, edict_t* test, qmodel_t* worldmodel)
{
  byte* pvs;
  vec3_t org;
...
SV_WriteEntitiesToClient(edict_t* clent, sizebuf_t* msg)
{
  ...
  for (e = 1; e < qcvm->num_edicts; e++, ent = NEXT_EDICT(ent)) {
    if (ent != clent) // clent already added before the loop
    {
      ...
      if (sv_netsort.value) {
        // compute ent bbox size and distance from org to the closest point in
        // ent's bbox
        dist = size = 0.f;
        for (i = 0; i < 3; i++) {
          float delta =
            CLAMP(ent->v.absmin[i], org[i], ent->v.absmax[i]) - org[i];
          dist += delta * delta;
        ...
        dist = 0.f;
        for (i = 0; i < 3; i++)
          dist += ((forward[i] < 0.f ? ent->v.absmin[i] : ent->v.absmax[i]) -
                   org[i]) *
                  forward[i];
        if (dist < 0.f)
          net_edict_dists[numents] |=
            128; // deprioritize entities behind the client
  ...
  for (j = 0; j < numents; j++) {
    ...
    if (ent->alpha == ENTALPHA_ZERO &&
        !((int)ent->v.effects & qcvm->effects_mask))
      continue;
    // johnfitz
...
SV_WriteClientdataToMessage(edict_t* ent, sizebuf_t* msg)
{
  ...
  if (bits & SU_WEAPONALPHA)
    MSG_WriteByte(msg, ent->alpha); // for now, weaponalpha = client entity
                                    // alpha
  // johnfitz
  ...
  if ((byte)ent->v.weapon != (int)ent->v.weapon &&
      msg->cursize + 6 <= msg->maxsize) {
...
TODO: group multiple stats in a single stuffcmd, the client already supports
this
=======================
*/
void
SV_WriteStats(client_t* client)
{
...
*/
void
SV_WriteUnderwaterOverride(client_t* client)
{
...
SV_SpawnServer(const char* server)
{
  ...
  qcvm->max_edicts = CLAMP(MIN_EDICTS,
                           (int)max_edicts.value,
                           MAX_EDICTS); // johnfitz -- max_edicts cvar
  qcvm->edicts = (edict_t*)malloc(
    qcvm->max_edicts *
    qcvm->edict_size); // ericw -- sv.edicts switched to use malloc()
  if (!qcvm->edicts)
