...
qboolean
Sys_GetFileTime(const char* path, time_t* out)
{
  struct stat st;
  if (stat(path, &st) != 0)
    return false;
...
qboolean
Sys_GetSteamDir(char* path, size_t pathsize)
{
  ...
  if ((size_t)q_snprintf(path, pathsize, "%s/%s", home_dir, STEAM_DIR) >=
      pathsize)
    return false;
...
qboolean
Sys_GetSteamQuakeUserDir(char* path, size_t pathsize, const char* library)
{
  const char* fmt = "%s/steamapps/compatdata/2310/pfx/drive_c/users/steamuser/"
...
static void
Sys_GetUserdir(char* dst, size_t dstsize)
{
  dev_t volume = dev_for_path("/boot");
  char buffer[B_PATH_NAME_LENGTH];
  status_t result;

  result = find_directory(
    B_USER_NONPACKAGED_DATA_DIRECTORY, volume, false, buffer, sizeof(buffer));
  if (result != B_OK)
...
qboolean
Sys_GetAltUserPrefDir(qboolean remastered, char* dst, size_t dstsize)
{
  const char* home_dir = NULL;
  struct passwd* pwent;
...
readdir_filtered(DIR* handle, const char* ext)
{
  while (1) {
...
findfile_t*
Sys_FindFirst(const char* dir, const char* ext)
{
  unixfindfile_t* ret;
  DIR* handle;
  struct dirent* data;
...
void
Sys_ReportError(const char* error, ...)
{
  va_list argptr;
  char text[1024];
