...
Max_Edicts_f(__attribute__((unused)) cvar_t* var)
{
  ...
  if (cls.state == ca_connected || sv.active)
    Con_Printf("Changes to max_edicts will not take effect until the next time "
               "a map is loaded.\n");
}
...
*/
void
Host_ReportError(const char* error, ...)
{
  va_list argptr;
  char string[1024];
...
void
Host_Callback_Notify(cvar_t* var)
{
  if (sv.active)
...
*/
void
Host_WriteConfigurationToFile(const char* name)
{
  FILE* f;
...
Host_WriteConfig_f(void)
{
  ...
  q_strlcpy(
    filename, Cmd_Argc() >= 2 ? Cmd_Argv(1) : CONFIG_NAME, sizeof(filename));
...
Host_InitLocal(void)
{
  ...
  Cvar_SetCallback(&host_maxfps, (cvarcallback_t)Max_Fps_f);
  Max_Fps_f(&host_maxfps);
  Cvar_RegisterVariable(&host_timescale); // johnfitz

  Cvar_RegisterVariable(&cl_nocsqc);  // spike
  Cvar_RegisterVariable(&max_edicts); // johnfitz
  Cvar_SetCallback(&max_edicts, (cvarcallback_t)Max_Edicts_f);
  ...
  Cvar_SetCallback(&fraglimit, (cvarcallback_t)Host_Callback_Notify);
  Cvar_SetCallback(&timelimit, (cvarcallback_t)Host_Callback_Notify);
  Cvar_SetCallback(&teamplay, (cvarcallback_t)Host_Callback_Notify);
  Cvar_RegisterVariable(&samelevel);
  Cvar_RegisterVariable(&noexit);
  Cvar_SetCallback(&noexit, (cvarcallback_t)Host_Callback_Notify);
...
Host_AdvanceTime(double dt)
{
  ...
  // johnfitz
    ...
    host_frametime = CLAMP(0.0001, host_frametime, 0.1); // johnfitz -- use
                                                         // CLAMP
}
...
Host_CheckAutosave(void)
{
  ...
  if (cls.signon == SIGNONS) {
    ...
    if (pr_global_struct->found_secrets != sv.autosave.prev_secrets) {
      ...
      sv.autosave.secret_boost =
        q_max(0.f, sv.autosave.secret_boost - host_frametime / 1.5f);
  }
  ...
  if (sv_player->v.movetype == MOVETYPE_NOCLIP ||
      (int)sv_player->v.flags & (FL_GODMODE | FL_NOTARGET)) {
  ...
  // inflicting damage, so we can't assume it's safe to save just because we're
  // no longer falling
  ...
  // Scale down the score if health + armor is below 100 (save less often with
  // lower health)
  score *= q_min(100.f,
                 (sv_player->v.health +
                  sv_player->v.armortype * sv_player->v.armorvalue)) /
           100.f;
...
*/
static void
GetGameSummary(summary_t* s)
{
  if (!cl_titlestats.value || cls.state != ca_connected ||
      cls.signon != SIGNONS) {
    s->map[0] = 0;
...
UpdateWindowTitle(void)
{
  ...
  if (!strcmp(current.map, last.map) &&
  ...
  if (current.map[0]) {
    ...
    q_snprintf(title,
               sizeof(title),
               utf8name[0] ? "%s (%s)  |  skill %d  |  %d/%d kills  |  %d/%d "
                             "secrets  -  " WINDOW_TITLE_STRING
                           : "%s%s  |  skill %d  |  %d/%d kills  |  %d/%d "
                             "secrets  -  " WINDOW_TITLE_STRING,
               utf8name,
               current.map,
               current.stats.skill,
               current.stats.monsters,
               current.stats.total_monsters,
               current.stats.secrets,
               current.stats.total_secrets);
...
CL_LoadCSProgs(void)
{
  ...
  if (pr_checkextension.value &&
      !cl_nocsqc.value) { // only try to use csqc if qc extensions are enabled.
    PR_SwitchQCVM(&cl.qcvm);

    // try csprogs.dat first, then fall back on progs.dat in case someone tried
    // merging the two. we only care about it if it actually contains a
    // CSQC_DrawHud, otherwise its either just a (misnamed) ssqc progs or a full
    // csqc progs that would just crash us on 3d stuff.
    if ((PR_LoadProgs("csprogs.dat", false) &&
      ...
      if (!qcvm->extfuncs
             .CSQC_DrawHud) { // no simplecsqc entry points... abort entirely!
        PR_ClearProgs(qcvm);
        PR_SwitchQCVM(NULL);
        return;
      }
      ...
      if (qcvm->extglobals.player_localnum)
        *qcvm->extglobals.player_localnum =
          cl.viewentity -
          1; // this is a guess, but is important for scoreboards.
      ...
      if (qcvm->extfuncs.CSQC_Init) {
        ...
        G_FLOAT(OFS_PARM2) = 10000 * IRONWAIL_VER_MAJOR +
                             100 * IRONWAIL_VER_MINOR + IRONWAIL_VER_PATCH;
...
*/
static void
Host_PrintTimes(const double times[],
                const char* names[],
                int count,
                qboolean showtotal)
{
...
_Host_Frame(double time)
{
  ...
  accumtime += host_netinterval ? CLAMP(0.0, time, 0.2)
                                : 0.0; // for renderer/server isolation
