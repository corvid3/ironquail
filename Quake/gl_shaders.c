...
*/
static void
GL_InitError(const char* message, ...)
{
  const char* fmt;
  char buf[4096];
  size_t len;
  ...
  fmt = "Your system appears to meet the minimum requirements,\n"
  ...
  Sys_Error(
    fmt, buf, (int)sizeof(void*) * 8, gl_version, gl_renderer, gl_vendor);
}
...
*/
static qboolean
AppendString(char** dst, const char* dstend, const char* str, int len)
{
...
*/
static GLuint
GL_CreateShader(GLenum type,
                const char* source,
                const char* extradefs,
                const char* name)
{
...
*/
static GLuint
GL_CreateProgramFromShaders(const GLuint* shaders,
                            int numshaders,
                            const char* name)
{
  GLuint program;
...
*/
static GLuint
GL_CreateProgramFromSources(int count,
                            ...
                            va_list argptr)
{
  ...
  for (i = 0; i < count; i++)
    if (sources[i])
      shaders[realcount++] =
        GL_CreateShader(types[i], sources[i], macros, name);
...
====================
*/
static FUNC_PRINTF(3, 4) GLuint GL_CreateProgram(const GLchar* vertSource,
...
====================
*/
static FUNC_PRINTF(2, 3) GLuint
  GL_CreateComputeProgram(const GLchar* source, const char* name, ...)
{
...
*/
void
GL_CreateShaders(void)
{
  int palettize, dither, mode, alphatest, warp, oit, md5;

  glprogs.gui = GL_CreateProgram(gui_vertex_shader, gui_fragment_shader, "gui");
  glprogs.viewblend = GL_CreateProgram(
    viewblend_vertex_shader, viewblend_fragment_shader, "viewblend");
  for (warp = 0; warp < 2; warp++)
    glprogs.warpscale[warp] = GL_CreateProgram(warpscale_vertex_shader,
                                               warpscale_fragment_shader,
                                               "view warp/scale|WARP %d",
                                               warp);
  for (palettize = 0; palettize < 3; palettize++)
    glprogs.postprocess[palettize] =
      GL_CreateProgram(postprocess_vertex_shader,
                       postprocess_fragment_shader,
                       "postprocess|PALETTIZE %d",
                       palettize);

  for (mode = 0; mode < 2; mode++)
    glprogs.oit_resolve[mode] = GL_CreateProgram(oit_resove_vertex_shader,
                                                 oit_resove_fragment_shader,
                                                 "oit resolve|MSAA %d",
                                                 mode);
  ...
  for (oit = 0; oit < 2; oit++)
    for (dither = 0; dither < 3; dither++)
      for (mode = 0; mode < 3; mode++)
        glprogs.world[oit][dither][mode] =
          GL_CreateProgram(world_vertex_shader,
                           world_fragment_shader,
                           "world|OIT %d; DITHER %d; MODE %d",
                           oit,
                           dither,
                           mode);
  ...
  for (dither = 0; dither < 2; dither++) {
    for (oit = 0; oit < 2; oit++) {
      glprogs.water[oit][dither] = GL_CreateProgram(water_vertex_shader,
                                                    water_fragment_shader,
                                                    "water|OIT %d; DITHER %d",
                                                    oit,
                                                    dither);
      glprogs.particles[oit][dither] =
        GL_CreateProgram(particles_vertex_shader,
                         particles_fragment_shader,
                         "particles|OIT %d; DITHER %d",
                         oit,
                         dither);
    }
    for (mode = 0; mode < 2; mode++)
      glprogs.skycubemap[mode][dither] =
        GL_CreateProgram(sky_cubemap_vertex_shader,
                         sky_cubemap_fragment_shader,
                         "sky cubemap|ANIM %d; DITHER %d",
                         mode,
                         dither);
    glprogs.skylayers[dither] = GL_CreateProgram(sky_layers_vertex_shader,
                                                 sky_layers_fragment_shader,
                                                 "sky layers|DITHER %d",
                                                 dither);
    glprogs.skyboxside[dither] = GL_CreateProgram(sky_boxside_vertex_shader,
                                                  sky_boxside_fragment_shader,
                                                  "skybox side|DITHER %d",
                                                  dither);
    glprogs.sprites[dither] = GL_CreateProgram(sprites_vertex_shader,
                                               sprites_fragment_shader,
                                               "sprites|DITHER %d",
                                               dither);
  }
  glprogs.skystencil =
    GL_CreateProgram(skystencil_vertex_shader, NULL, "sky stencil");
      ...
      for (alphatest = 0; alphatest < 2; alphatest++)
        for (md5 = 0; md5 < 2; md5++)
          glprogs.alias[oit][mode][alphatest][md5] =
            GL_CreateProgram(alias_vertex_shader,
                             alias_fragment_shader,
                             "alias|OIT %d; MODE %d; ALPHATEST %d; MD5 %d",
                             oit,
                             mode,
                             alphatest,
                             md5);
  ...
  glprogs.debug3d =
    GL_CreateProgram(debug3d_vertex_shader, debug3d_fragment_shader, "debug3d");

  glprogs.clear_indirect = GL_CreateComputeProgram(
    clear_indirect_compute_shader, "clear indirect draw params");
  glprogs.gather_indirect = GL_CreateComputeProgram(
    gather_indirect_compute_shader, "indirect draw gather");
  glprogs.cull_mark =
    GL_CreateComputeProgram(cull_mark_compute_shader, "cull/mark");
  glprogs.cluster_lights =
    GL_CreateComputeProgram(cluster_lights_compute_shader, "light cluster");
  for (mode = 0; mode < 3; mode++)
    glprogs.palette_init[mode] = GL_CreateComputeProgram(
      palette_init_compute_shader, "palette init|MODE %d", mode);
  glprogs.palette_postprocess = GL_CreateComputeProgram(
    palette_postprocess_compute_shader, "palette postprocess");
}
