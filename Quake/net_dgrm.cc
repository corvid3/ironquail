...
Datagram_GetMessage(qsocket_t* sock)
{
  ...
  while (1) {
    ...
    if (flags & NETFLAG_DATA) {
      ...
      sfunc.Write(
        sock->socket, (byte*)&packetBuffer, NET_HEADERSIZE, &readaddr);
      ...
      if (flags & NETFLAG_EOM) {
        SZ_Clear(&net_message);
        SZ_Write(
          &net_message, sock->receiveMessage, sock->receiveMessageLength);
        SZ_Write(&net_message, packetBuffer.data, length);
...
Test2_Poll(__attribute__((unused)) void* unused)
{
  ...
  len =
    dfunc.Read(test2Socket, net_message.data, net_message.maxsize, &clientaddr);
  if (len < (int)sizeof(int))
    goto Reschedule;
...
_Datagram_CheckNewConnections(void)
{
  ...
  len =
    dfunc.Read(acceptsock, net_message.data, net_message.maxsize, &clientaddr);
  if (len < (int)sizeof(int))
    return NULL;
  net_message.cursize = len;
  ...
  if (clientaddr.qsa_family == AF_INET) {
    ...
    if ((testAddr & banMask.s_addr) == banAddr.s_addr) {
      ...
      *((int*)net_message.data) =
        BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
      dfunc.Write(
        acceptsock, net_message.data, net_message.cursize, &clientaddr);
      SZ_Clear(&net_message);
      return NULL;
    }
  }
...
_Datagram_SearchForHosts(qboolean xmit)
{
  ...
  while (
    (ret = dfunc.Read(
       dfunc.controlSock, net_message.data, net_message.maxsize, &readaddr)) >
    0) {
