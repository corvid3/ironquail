...
typedef struct
{
  ...
  // base demo speed; different from demospeed when fast-forwarding/rewinding or
  // when playback is paused (we want to be able to set playback speed to 1/2x,
  // pause, and then resume playback at 1/2x not 1x)
  float basedemospeed;
...
typedef struct
{
  ...
  char stuffcmdbuf[1024]; // comment-extensions are a thing with certain
  ...
  qboolean
    forceunderwater; // force underwater warping/sound distortion even when
                     // camera is not submerged (e.g. alk1.2 liquidbrush)
} client_state_t;
...
CL_SendCmd(void);
void
CL_SendMove(const usercmd_t* cmd);
int
...
void
Chase_UpdateForClient(void); // johnfitz
void
Chase_UpdateForDrawing(void); // johnfitz
