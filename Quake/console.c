...
typedef enum
{
  ...
  // - picks the closest edge horizontally, on whichever line contains the
  // cursor vertically
...
static void
Con_ScreenToCanvas(int x, int y, int* outx, int* outy)
{
...
*/
static qboolean
Con_CanvasToOffset(int x, int y, conofs_t* ofs, contest_t testmode)
{
  qboolean ret = true;
...
*/
static qboolean
Con_ScreenToOffset(int x, int y, conofs_t* ofs, contest_t testmode)
{
...
*/
static int
Con_OfsCompare(const conofs_t* lhs, const conofs_t* rhs)
{
...
*/
static qboolean
Con_OfsInRange(const conofs_t* ofs, const conofs_t* begin, const conofs_t* end)
{
...
*/
static void
Con_GetCurrentRange(conofs_t* begin, conofs_t* end)
{
...
*/
static qboolean
Con_IntersectRanges(conofs_t* begin,
                    conofs_t* end,
                    const conofs_t* selbegin,
                    const conofs_t* selend)
{
...
*/
static conlink_t*
Con_GetLinkAtOfs(const conofs_t* ofs)
{
  size_t lo, hi;
...
*/
static conlink_t*
Con_GetLinkAtPixel(int x, int y)
{
  conofs_t ofs;
...
*/
static qboolean
Con_GetMousePos(conofs_t* ofs, contest_t testmode)
{
  int x, y;
...
*/
static qboolean
Con_GetNormalizedSelection(conofs_t* begin, conofs_t* end)
{
...
*/
static int
Con_TestWordBoundary(int pos, const char* text, int len)
{
  if (pos <= 0)
    return 1;
  if (pos >= len)
...
Con_ApplyMouseSelection(void)
{
  ...
  // Clamp starting point to the end of the actual content (one character past
  // it) so that double-clicking beyond the end of the line selects the last
  // word
  con_selection.begin.col = q_min(con_selection.begin.col, len);

  // Special case: if we're selecting whole words, the initial click was on a
  // word boundary, and the current selection hasn't advanced towards the actual
  // content (either left or right), then we nudge the starting point by one
  // character so that the word adjacent to the initial click is always
  // selected.
  if (con_mouseclicks == 2) {
    int boundary =
      IntSign(Con_TestWordBoundary(con_selection.begin.col, line, len));
...
*/
static void
Con_SetMouseState(conmouse_t state)
{
  int x, y;
  conofs_t pos;
  ...
  switch (state) {
    case CMS_PRESSED:
      ...
      if (con_mouseclicks == 0 || con_mouseclickdelay >= DOUBLECLICK_TIME ||
          Con_OfsCompare(&pos, &con_mouseselection.end) != 0)
        con_mouseclicks = 1;
      else
        con_mouseclicks++;
    ...
    case CMS_NOTPRESSED:
      if (con_mousestate != CMS_DRAGGING && con_hotlink &&
          !Sys_Explore(con_hotlink->path))
...
Con_Mousemove(int x, int y)
{
  if (con_mousestate == CMS_NOTPRESSED) {
    ...
    VID_SetMouseCursor(con_hotlink ? MOUSECURSOR_HAND
                       : inside    ? MOUSECURSOR_IBEAM
                                   : MOUSECURSOR_DEFAULT);
  } else {
    int cx, cy, delta;
    float frac;
    ...
    if (delta < 0) {
      ...
      // NOTE: This is not an issue on the bottom because of the input line and
      // margin.
      int moved = cy - con_clicky;
      int scrolled =
        q_min(con_mouseselection.end.line - con_mouseselection.begin.line, 0) *
        CHARSIZE;
...
*/
qboolean
Con_CopySelectionToClipboard(void)
{
  ...
  for (cursor = selbegin; Con_OfsCompare(&cursor, &selend) <= 0;
  ...
  if (!utf8)
    Sys_Error("Con_CopySelectionToClipboard: out of memory on %" SDL_PRIu64
              " bytes",
              (uint64_t)maxsize);
...
Con_Dump_f(void)
{
  ...
  q_strlcpy(
    relname, Cmd_Argc() >= 2 ? Cmd_Argv(1) : "condump.txt", sizeof(relname));
...
void
Con_RecalcOffset(conofs_t* ofs, int oldnumlines)
{
...
Con_CheckResize(void)
{
  ...
  width = (vid.conwidth >> 3) -
          CON_MARGIN * 2; // johnfitz -- use vid.conwidth instead of vid.width
...
Con_Init(void)
{
  ...
  con_text = (char*)Hunk_AllocNameNoFill(
    con_buffersize,
...
*/
static const char*
Con_StripControlPrefixes(const char* txt)
{
  // colored text
  if (txt[0] == 1 || txt[0] == 2)
    txt++;
...
*/
void
Con_LinkPrintf(const char* addr, const char* fmt, ...)
{
  conlink_t* link;
  size_t len;
  va_list argptr;
  ...
  if (!link)
    Sys_Error("Con_LinkPrintf: out of memory on %" SDL_PRIu64 " bytes",
              (uint64_t)(sizeof(conlink_t) + len + 1));
  ...
  // Because of wrapping our text might actually start on the next line, so we
  // skip leading spaces
...
*/
void
Con_CenterPrintf(int linewidth, const char* fmt, ...) FUNC_PRINTF(2, 3);
void
Con_CenterPrintf(int linewidth, const char* fmt, ...)
{
  va_list argptr;
...
*/
void
Con_LogCenterPrint(const char* str)
{
...
void
Con_AddToTabList(const char* name, const char* partial, const char* type)
{
  tab_t *t, *insert;
  char *i_bash, *i_bash2;
...
*/
qboolean
Con_Match(const char* str, const char* partial)
{
...
static qboolean
CompleteFileList(const char* partial, void* param)
{
...
*/
static void
Con_FormatTabMatch(const tab_t* t, char* dst, size_t dstsize)
{
...
*/
void
Con_TabComplete(tabcomplete_t mode)
{
  ...
  if (mode == TABCOMPLETE_AUTOHINT) {
    ...
    if ((size_t)key_linepos >= sizeof(key_lines[edit_line]) ||
                          ...
                          // (Otherwise, use previous.)
  {
    ...
    c = key_lines[edit_line] + key_linepos - 1; // start one space left of
                                                // cursor
...
Con_DrawInput(void)
{
  ...
  if (key_tabhint[0]) {
    ...
    for (i = 0;
         key_tabhint[i] && i + 1 + len - ofs < con_linewidth + CON_MARGIN * 2;
         i++)
      Draw_Character(
        (i + 1 + len - ofs) << 3, vid.conheight - 16, key_tabhint[i] | 0x80);
...
*/
void
Con_DrawSelectionHighlight(int x, int y, int line)
{
  conofs_t selbegin, selend;
  conofs_t begin, end;
  size_t len;
...
Con_DrawConsole(int lines, qboolean drawinput)
{
  ...
  for (i = con_current - rows + 1; i <= con_current - sb; i++, y += 8) {
    ...
    for (x = 0; x < con_linewidth; x++) {
      ...
      ofs.col = x;
      if (con_hotlink &&
          Con_OfsInRange(&ofs, &con_hotlink->begin, &con_hotlink->end)) {
        if (keydown[K_MOUSE1])
          c &= 0x7f;
...
LOG_Init(quakeparms_t* parms)
{
  ...
  q_snprintf(
    logfilename, sizeof(logfilename), "%s/qconsole.log", parms->basedir);
