...
*/
static int
FGetLittleLong(FILE* f, qboolean* ok)
{
  int v;
...
*/
static short
FGetLittleShort(FILE* f, qboolean* ok)
{
  short v;
  *ok &= fread(&v, sizeof(v), 1, f) == 1;
...
*/
static int
WAV_ReadChunkInfo(FILE* f, char* name)
{
  int len, r;
  qboolean ok = true;
...
*/
static int
WAV_FindRIFFChunk(FILE* f, const char* chunk)
{
  char name[5];
  int len;
...
*/
static qboolean
WAV_ReadRIFFHeader(const char* name, FILE* file, snd_info_t* info)
{
  char dump[16];
  int wav_format;
  int fmtlen = 0;
  ...
  if (info->channels != 1 && info->channels != 2) {
    Con_Printf(
      "Unsupported number of channels %d in %s\n", info->channels, name);
    return false;
  }
...
*/
static qboolean
S_WAV_CodecOpenStream(snd_stream_t* stream)
{
...
static int
S_WAV_CodecRewindStream(snd_stream_t* stream)
{
  FS_rewind(&stream->fh);
  return 0;
}
...
static qboolean
S_WAV_CodecInitialize(void)
{
  return true;
}

static void
S_WAV_CodecShutdown(void)
{
}
