...
*/
void
CL_AddDemoRewindSound(int entnum,
                      int channel,
                      sfx_t* sfx,
                      vec3_t pos,
                      int vol,
  ...
  if (cls.demospeed > 0.f) {
    if (cls.signon < SIGNONS) {
      ...
      for (i = 0; i < MAX_LIGHTSTYLES; i++)
        q_strlcpy(demo_rewind.prev.lightstyles[i],
                  cl_lightstyle[i].map,
                  MAX_STYLESTRING);
...
CL_FinishDemoFrame(void)
{
  ...
  if (cls.demospeed > 0.f) // forward playback
  {
    ...
    if (memcmp(&demo_rewind.prev.cshift, &cshift_empty, sizeof(cshift_t)) !=
        0) {
      VEC_PUSH(demo_rewind.frame_events, DFE_CSHIFT);
      Vec_Append((void**)&demo_rewind.frame_events,
                 1,
                 &demo_rewind.prev.cshift,
                 sizeof(cshift_t));
    ...
    for (i = 0; i < MAX_LIGHTSTYLES; i++) {
      ...
      Vec_Append((void**)&demo_rewind.frame_events,
                 1,
                 demo_rewind.prev.lightstyles[i],
                 len);
      ...
      while (begin < end) {
        ...
        switch (datatype) {
          ...
          case DFE_SOUND: {
            ...
            if (snd.sfx)
              S_StartSound(snd.ent,
                           snd.channel,
                           snd.sfx,
                           snd.pos,
                           snd.volume / 255.0,
                           snd.attenuation / 64.f);
            else
...
CL_GetDemoMessage(void)
{
  ...
  if (cls.signon == SIGNONS) // always grab until fully connected
  {
    ...
    } else if (/* cl.time > 0 && */ cls.demospeed > 0.f
                 ? cl.time <= cl.mtime[0]
                 : cl.time >= cl.mtime[0]) {
...
CL_Record_f(void)
{
  ...
  // save the demo name here, before potentially loading a new map (which would
  // change argv[1])
  ...
  if (c == 2 && cls.state == ca_connected) {
    ...
    for (i = 0; i < MAX_CL_STATS; i++) {
      ...
      if (net_message.cursize > 4096) { // periodically flush so that large maps
                                        // don't need larger than vanilla limits
        CL_WriteDemoMessage();
        SZ_Clear(&net_message);
      }

      if ((double)cl.stats[i] != cl.statsf[i] &&
          (unsigned int)cl.stats[i] <=
            0x00ffffff) { // if the float representation seems to have more
                          // precision then use that, unless its getting huge in
                          // which case we're probably getting fpu truncation,
                          // so go back to more compatible ints
