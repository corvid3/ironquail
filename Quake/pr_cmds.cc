...
int
PR_MakeTempString(const char* val)
{
...
static const char*
PF_GetStringArg(int idx, void* userdata)
{
  if (userdata)
    idx += *(int*)userdata;
...
PF_VarString(int first)
{
  ...
  if (LOC_HasPlaceholders(format)) {
    ...
    for (i = first; i < qcvm->argc; i++) {
      s =
        q_strlcat(out, LOC_GetString(G_STRING(OFS_PARM0 + i * 3)), sizeof(out));
      if (s >= sizeof(out)) {
...
static void
SetMinMaxSize(edict_t* e, float* minvec, float* maxvec, qboolean rotate)
{
  float* angles;
  vec3_t rmin, rmax;
  float bounds[2][3];
...
PF_aim(void)
{
  ...
  if (tr.ent && tr.ent->v.takedamage == static_cast<int>(DAMAGE_AIM) &&
  ...
  for (i = 1; i < qcvm->num_edicts; i++, check = NEXT_EDICT(check)) {
    if (check->v.takedamage != static_cast<int>(DAMAGE_AIM))
      continue;
    if (check == ent)
      continue;
...
*/
static void
PF_finalefinished(void)
{
  G_FLOAT(OFS_RETURN) = 0;
}
static void
PF_CheckPlayerEXFlags(void)
{
  G_FLOAT(OFS_RETURN) = 0;
}
static void
PF_walkpathtogoal(void)
{
...
void
PR_Markup_Begin(struct markup_s* mu, const char* text, float* rgb, float alpha)
{
  if (*text == '\1' || *text == '\2') {
    mu->mask = 128;
    text++;
...
int
PR_Markup_Parse(struct markup_s* mu)
{
  static const vec4_t q3rgb[10] = {
    { 0.00, 0.00, 0.00, 1.0 }, { 1.00, 0.33, 0.33, 1.0 },
    { 0.00, 1.00, 0.33, 1.0 }, { 1.00, 1.00, 0.33, 1.0 },
    { 0.33, 0.33, 1.00, 1.0 }, { 0.33, 1.00, 1.00, 1.0 },
    { 1.00, 0.33, 1.00, 1.0 }, { 1.00, 1.00, 1.00, 1.0 },
  ...
  while ((c = *mu->txt)) {
    if (c == '^' &&
        false /*pr_checkextension.value*/) { // parse markup like FTE/DP might.
      switch (mu->txt[1]) {
        ...
        case 'h': // toggle half-alpha
          ...
          mu->txt += 2;
          continue;
        case 'd': // reset to defaults (fixme: should reset ^m without resetting
                  // \1)
        ...
        case '&':
          if ((ishex(mu->txt[2]) || mu->txt[2] == '-') &&
              (ishex(mu->txt[3]) ||
               mu->txt[3] == '-')) { // ignore fte's fore/back ansi colours
            mu->txt += 4;
            continue;
          }
          break;  // malformed
        ...
        case 'U': // ucs-2 unicode codepoint
          if (ishex(mu->txt[2]) && ishex(mu->txt[3]) && ishex(mu->txt[4]) &&
              ishex(mu->txt[5])) {
            c = (dehex(mu->txt[2]) << 12) | (dehex(mu->txt[3]) << 8) |
            ...
            if (c >= 0xe000 && c <= 0xe0ff)
              ...
              c = '?'; // otherwise its some unicode char that we don't know how
                       // to handle.
            return c;
          }
          break;  // malformed
        case '{': // full unicode codepoint, for chars up to 0x10ffff
          ...
          if (c >= 0xe000 && c <= 0xe0ff)
            ...
            c &= 0x7f; // ascii is okay too.
          // it would be nice to include a table to de-accent latin scripts, as
          // well as translate cyrilic somehow, but not really necessary.
          else
            c = '?'; // otherwise its some unicode char that we don't know how
                     // to handle.
          return c;
...
void
PF_cl_playerkey_internal(int player, const char* key, qboolean retfloat)
{
  char buf[1024];
  const char* ret = buf;
  ...
  if (player < 0 || player >= MAX_SCOREBOARD)
    ...
    byte* pal =
      (byte*)&d_8to24table[((cl.scores[player].colors & 0x0f) << 4) + 8];
    q_snprintf(buf,
               sizeof(buf),
               "%g %g %g",
               pal[0] / 255.0,
    ...
    q_snprintf(buf, sizeof(buf), "%i", cl.scores[player].colors & 0x0f);
             ...
             key,
             "team")) // quakeworld uses team infokeys to decide teams (instead
                      // of colours). but NQ never did, so that's fun. Lets
                      // allow mods to use either so that they can favour QW and
                      // let the engine hide differences .
...
// #define PICFLAG_DOWNLOAD	(1u<<8)	//request to download it from the
// gameserver if its not stored locally.
#define PICFLAG_BLOCK (1u << 9) // wait until the texture is fully loaded.
#define PICFLAG_NOLOAD (1u << 31)
static qpic_t*
DrawQC_CachePic(const char* picname, unsigned int flags)
{ // okay, so this is silly. we've ended up with 3 different cache levels.
  // qcpics, pics, and images.
  size_t i;
  unsigned int texflags;
  for (i = 0; i < numqcpics; i++) { // binary search? something more sane?
  ...
  if (i + 1 > maxqcpics) {
    maxqcpics = i + 32;
    qcpics = (struct qcpics_s*)realloc(qcpics, maxqcpics * sizeof(*qcpics));
  }
  ...
  if (flags & PICFLAG_WRAP)
    texflags &=
      ~(TEXPREF_PAD |
        TEXPREF_CLAMP); // don't allow padding if its going to need to wrap
                        // (even if we don't enable clamp-to-edge normally). I
                        // just hope we have npot support.
  ...
  if (flags & PICFLAG_WAD)
    qcpics[i].pic = Draw_PicFromWad2(
      picname + (strncmp(picname, "gfx/", 4) ? 0 : 4), texflags);
...
static void
DrawQC_CharacterQuad(float x, float y, int num, float w, float h)
{
  Draw_CharacterEx(x, y, w, h, num);
}
static void
PF_cl_drawcharacter(void)
{
...
static void
PF_cl_drawresetclip(void)
{
  Draw_ResetClipping();
}

static void
PF_cl_precachepic(void)
{
...
PF_vectorvectors(void)
{
  ...
  CrossProduct(pr_global_struct->v_right,
               pr_global_struct->v_forward,
               pr_global_struct->v_up);
}
...
// part of PF_strconv
static int
chrconv_number(int i, int base, int conv)
{
  i -= base;
  switch (conv) {
    default:
...
// part of PF_strconv
static int
chrconv_punct(int i, int base, int conv)
{
  i -= base;
  switch (conv) {
    default:
...
// part of PF_strconv
static int
chrchar_alpha(int i, int basec, int baset, int convc, int convt, int charnum)
{
...
PF_strconv(void)
{
  ...
  int redalpha = G_FLOAT(
    OFS_PARM1); // 0 same, 1 white, 2 red,  5 alternate, 6 alternate-alternate
  int rednum =
    G_FLOAT(OFS_PARM2); // 0 same, 1 white, 2 red, 3 redspecial, 4 whitespecial,
                        // 5 alternate, 6 alternate-alternate
...
static void
PF_sprintf_internal(const char* s, int firstarg, char* outbuf, int outbuflen)
{
  const char* s0;
          ...
          switch (*s) {
              ...
              if (precision < 0) // not set
                q_snprintf(o,
                           ...
                           (isfloat ? (unsigned int)GETARG_FLOAT(thisarg)
                                    : (unsigned int)GETARG_INT(thisarg)));
              else
                q_snprintf(o,
                           end - o,
                           formatbuf,
            ...
            case 'V':
              ...
              if (precision < 0) // not set
                q_snprintf(o,
                           ...
                           va("%s %s %s",
                              /* NESTED SPRINTF IS NESTED */ formatbuf,
                              formatbuf,
                              formatbuf),
                           width,
                           (isfloat ? (double)GETARG_VECTOR(thisarg)[0]
                                    : (double)GETARG_INTVECTOR(thisarg)[0]),
                           width,
                           (isfloat ? (double)GETARG_VECTOR(thisarg)[1]
                                    : (double)GETARG_INTVECTOR(thisarg)[1]),
                           width,
                           (isfloat ? (double)GETARG_VECTOR(thisarg)[2]
                                    : (double)GETARG_INTVECTOR(thisarg)[2]));
              else
                q_snprintf(o,
                           end - o,
                           va("%s %s %s",
            ...
            case 'c':
              ...
              /*							else
                                                                      {
                                                                              unsigned
                 int c = (isfloat ? (unsigned int) GETARG_FLOAT(thisarg) :
                 (unsigned int) GETARG_INT(thisarg)); char charbuf16[16]; const
                 char *buf = u8_encodech(c, NULL, charbuf16); if(!buf) buf = "";
                                                                              if(precision
                 < 0) // not set precision = end - o - 1; o += u8_strpad(o, end
                 - o, buf, (flags & PRINTF_LEFT) != 0, width, precision);
                                                                      }
              */
              break;
            case 'S': { // tokenizable string
              const char* quotedarg = GETARG_STRING(thisarg);

              // try and escape it... hopefully it won't get truncated by
              // precision limits...
              ...
              if (strchr(quotedarg, '\"') || strchr(quotedarg, '\n') ||
                  strchr(quotedarg, '\r') ||
                  l + 3 >= sizeof(quotedbuf)) { // our escapes suck...
              ...
              //& PRINTF_ALTERNATE)
              {
                if (precision < 0) // not set
                  ...
                  q_snprintf(
                    o, end - o, formatbuf, width, precision, quotedarg);
                o += strlen(o);
              }
/*								else
								{
...
case 's':
  ...
  // PRINTF_ALTERNATE)
  {
    if (precision < 0) // not set
      ...
      q_snprintf(
        o, end - o, formatbuf, width, precision, GETARG_STRING(thisarg));
    o += strlen(o);
  }
  /*							else
                                                          {
                                                                  if(precision <
     0) // not set precision = end - o - 1; o += u8_strpad(o, end - o,
     GETARG_STRING(thisarg), (flags & PRINTF_LEFT) != 0, width, precision);
                                                          }
  */
  break;
default:
    ...
    for (;;) {
      ...
      if (found) {
        ...
        memcpy(qctoken[qctoken_count].token,
               start + qctoken[qctoken_count].start,
               tlen);
...
static void
PF_cl_registercommand(void)
{
  const char* cmdname = G_STRING(OFS_PARM0);
  Cmd_AddCommand(cmdname, NULL);
}

static struct svcustomstat_s*
PR_CustomStat(int idx, int type)
{
  size_t i;
...
builtindef_t pr_builtindefs[] = {
  { "makevectors",
    ...
    1,
    qcextension_t{} }, // void(entity e) makevectors		= #1
  { "setorigin",
    PF_SSQC(PF_setorigin),
    2,
    qcextension_t{} }, // void(entity e, vector o) setorigin	= #2
  { "setmodel",
    PF_SSQC(PF_setmodel),
    3,
    qcextension_t{} }, // void(entity e, string m) setmodel	= #3
  { "setsize",
    PF_SSQC(PF_setsize),
    4,
    qcextension_t{} }, // void(entity e, vector min, vector max) setsize
                       // = #4
  { "break", PF_BOTH(PF_break), 6, qcextension_t{} },   // void() break = #6
  { "random", PF_BOTH(PF_random), 7, qcextension_t{} }, // float() random = #7
  { "sound",
    PF_SSQC(PF_sound),
    8,
    qcextension_t{} }, // void(entity e, float chan, string samp) sound	= #8
  { "normalize",
    PF_BOTH(PF_normalize),
    9,
    qcextension_t{} }, // vector(vector v) normalize		= #9
  { "error",
    PF_SSQC(PF_error),
    10,
    qcextension_t{} }, // void(string e) error = #10
  { "objerror",
    PF_SSQC(PF_objerror),
    11,
    qcextension_t{} }, // void(string e) objerror = #11
  { "vlen",
    PF_BOTH(PF_vlen),
    12,
    qcextension_t{} }, // float(vector v) vlen = #12
  { "vectoyaw",
    PF_BOTH(PF_vectoyaw),
    13,
    qcextension_t{} }, // float(vector v) vectoyaw = #13
  { "spawn", PF_SSQC(PF_Spawn), 14, qcextension_t{} }, // entity() spawn = #14
  { "remove",
    PF_SSQC(PF_Remove),
    15,
    qcextension_t{} }, // void(entity e) remove = #15
  { "traceline",
    PF_SSQC(PF_traceline),
    16,
    qcextension_t{} }, // float(vector v1, vector v2, float tryents) traceline
                       // = #16
  { "checkclient",
    PF_SSQC(PF_checkclient),
    17,
    qcextension_t{} }, // entity() clientlist			= #17
  { "find",
    PF_SSQC(PF_Find),
    18,
    qcextension_t{} }, // entity(entity start, .string fld, string match) find
                       // = #18
  { "precache_sound",
    PF_SSQC(PF_precache_sound),
    19,
    qcextension_t{} }, // void(string s) precache_sound	= #19
  { "precache_model",
    PF_SSQC(PF_precache_model),
    20,
    qcextension_t{} }, // void(string s) precache_model	= #20
  { "stuffcmd",
    PF_SSQC(PF_stuffcmd),
    21,
    qcextension_t{} }, // void(entity client, string s)stuffcmd	= #21
  { "findradius",
    PF_SSQC(PF_findradius),
    22,
    qcextension_t{} }, // entity(vector org, float rad) findradius	= #22
  { "bprint",
    PF_SSQC(PF_bprint),
    23,
    qcextension_t{} }, // void(string s) bprint = #23
  { "sprint",
    PF_SSQC(PF_sprint),
    24,
    qcextension_t{} }, // void(entity client, string s) sprint	= #24
  { "dprint",
    PF_BOTH(PF_dprint),
    25,
    qcextension_t{} }, // void(string s) dprint = #25
  { "ftos",
    PF_BOTH(PF_ftos),
    26,
    qcextension_t{} }, // void(string s) ftos = #26
  { "vtos",
    PF_BOTH(PF_vtos),
    27,
    qcextension_t{} }, // void(string s) vtos = #27
  { "coredump", PF_SSQC(PF_coredump), 28, qcextension_t{} },
  { "traceon", PF_BOTH(PF_traceon), 29, qcextension_t{} },
  { "traceoff", PF_BOTH(PF_traceoff), 30, qcextension_t{} },
  { "eprint",
    PF_SSQC(PF_eprint),
    31,
    qcextension_t{} }, // void(entity e) debug print an entire entity
  { "walkmove",
    PF_SSQC(PF_walkmove),
    32,
    qcextension_t{} }, // float(float yaw, float dist) walkmove
  { "droptofloor", PF_SSQC(PF_droptofloor), 34, qcextension_t{} },
  { "lightstyle", PF_SSQC(PF_lightstyle), 35, qcextension_t{} },
  { "rint", PF_BOTH(PF_rint), 36, qcextension_t{} },
  { "floor", PF_BOTH(PF_floor), 37, qcextension_t{} },
  { "ceil", PF_BOTH(PF_ceil), 38, qcextension_t{} },
  { "checkbottom", PF_SSQC(PF_checkbottom), 40, qcextension_t{} },
  { "pointcontents", PF_SSQC(PF_pointcontents), 41, qcextension_t{} },
  { "fabs", PF_BOTH(PF_fabs), 43, qcextension_t{} },
  { "aim", PF_SSQC(PF_aim), 44, qcextension_t{} },
  { "cvar", PF_BOTH(PF_cvar), 45, qcextension_t{} },
  { "localcmd", PF_BOTH(PF_localcmd), 46, qcextension_t{} },
  { "nextent", PF_SSQC(PF_nextent), 47, qcextension_t{} },
  { "particle", PF_SSQC(PF_particle), 48, qcextension_t{} },
  { "ChangeYaw", PF_SSQC(PF_changeyaw), 49, qcextension_t{} },
  { "vectoangles", PF_BOTH(PF_vectoangles), 51, qcextension_t{} },

  { "WriteByte", PF_SSQC(PF_WriteByte), 52, qcextension_t{} },
  { "WriteChar", PF_SSQC(PF_WriteChar), 53, qcextension_t{} },
  { "WriteShort", PF_SSQC(PF_WriteShort), 54, qcextension_t{} },
  { "WriteLong", PF_SSQC(PF_WriteLong), 55, qcextension_t{} },
  { "WriteCoord", PF_SSQC(PF_WriteCoord), 56, qcextension_t{} },
  { "WriteAngle", PF_SSQC(PF_WriteAngle), 57, qcextension_t{} },
  { "WriteString", PF_SSQC(PF_WriteString), 58, qcextension_t{} },
  { "WriteEntity", PF_SSQC(PF_WriteEntity), 59, qcextension_t{} },
  ...
  { "movetogoal", PF_SSQC(SV_MoveToGoal), 67, qcextension_t{} },
  { "precache_file", PF_SSQC(PF_precache_file), 68, qcextension_t{} },
  { "makestatic", PF_SSQC(PF_makestatic), 69, qcextension_t{} },

  { "changelevel", PF_SSQC(PF_changelevel), 70, qcextension_t{} },

  { "cvar_set", PF_BOTH(PF_cvar_set), 72, qcextension_t{} },
  { "centerprint", PF_SSQC(PF_centerprint), 73, qcextension_t{} },

  { "ambientsound", PF_SSQC(PF_ambientsound), 74, qcextension_t{} },

  { "precache_model2", PF_SSQC(PF_precache_model), 75, qcextension_t{} },
  { "precache_sound2",
    PF_SSQC(PF_precache_sound),
    76,
    qcextension_t{} }, // precache_sound2 is different only for qcc
  { "precache_file2", PF_SSQC(PF_precache_file), 77, qcextension_t{} },

  { "setspawnparms", PF_SSQC(PF_setspawnparms), 78, qcextension_t{} },

  // 2021 re-release
  { "finaleFinished",
    PF_SSQC(PF_finalefinished),
    79,
    qcextension_t{} }, // float() finaleFinished = #79
  { "localsound",
    PF_SSQC(PF_localsound),
    80,
    qcextension_t{} }, // void localsound (entity client, string sample) = #80
  ...
  { "ex_centerprint",
    ...
    0,
    qcextension_t{} }, // void(entity client, string s, ...)
  { "ex_bprint",
    PF_SSQC(PF_bprint),
    0,
    qcextension_t{} }, // void(string s, ...)
  { "ex_sprint",
    PF_SSQC(PF_sprint),
    0,
    qcextension_t{} }, // void(entity client, string s, ...)
  ...
  { "ex_CheckPlayerEXFlags",
    ...
    0,
    qcextension_t{} }, // float(entity playerEnt)
  { "ex_walkpathtogoal",
    PF_SSQC(PF_walkpathtogoal),
    0,
    qcextension_t{} }, // float(float movedist, vector goal)
  { "ex_localsound",
    PF_SSQC(PF_localsound),
    0,
    qcextension_t{} }, // void(entity client, string sample)

  { "min",
    PF_BOTH(PF_min),
    94,
    DP_QC_MINMAXBOUND }, // float(float a, float b, ...)
  { "max",
    PF_BOTH(PF_max),
    95,
    DP_QC_MINMAXBOUND }, // float(float a, float b, ...)
  { "bound",
    PF_BOTH(PF_bound),
    96,
    DP_QC_MINMAXBOUND }, // float(float minimum, float val, float maximum)
  ...
  { "checkextension",
    ...
    99,
    qcextension_t{} }, // float(string extname)

  { "strlen", PF_BOTH(PF_strlen), 114, FRIK_FILE }, // float(string s)
  { "strcat",
    PF_BOTH(PF_strcat),
    115,
    FRIK_FILE }, // string(string s1, optional string s2, optional string s3,
                 // optional string s4, optional string s5, optional string s6,
                 // optional string s7, optional string s8)
  { "substring",
    PF_BOTH(PF_substring),
    116,
    FRIK_FILE }, // string(string s, float start, float length)
  ...
  { "str2chr",
    PF_BOTH(PF_str2chr),
    222,
    FTE_STRINGS }, // float(string str, float index)
  { "chr2str",
    PF_BOTH(PF_chr2str),
    223,
    FTE_STRINGS }, // string(float chr, ...)
  { "strconv",
    PF_BOTH(PF_strconv),
    224,
    FTE_STRINGS }, // string(float ccase, float redalpha, float redchars, string
                   // str, ...)

  { "clientstat",
    PF_SSQC(PF_clientstat),
    232,
    qcextension_t{} }, // void(float num, float type, .__variant fld)

  { "mod", PF_BOTH(PF_mod), 245, qcextension_t{} }, // float(float a, float n)

  { "ftoi", PF_BOTH(PF_ftoi), 0, qcextension_t{} }, // int(float)
  { "itof", PF_BOTH(PF_itof), 0, qcextension_t{} }, // float(int)
  ...
  { "iscachedpic",
    PF_CSQC(PF_cl_iscachedpic),
    316,
    qcextension_t{} }, // float(string name)
  { "precache_pic",
    PF_CSQC(PF_cl_precachepic),
    317,
    qcextension_t{} }, // string(string name, optional float flags)
  { "drawgetimagesize",
    PF_CSQC(PF_cl_getimagesize),
    318,
    qcextension_t{} }, // #define draw_getimagesize
                       // drawgetimagesize\nvector(string picname)
  { "drawcharacter",
    PF_CSQC(PF_cl_drawcharacter),
    320,
    qcextension_t{} }, // float(vector position, float character, vector size,
                       // vector rgb, float alpha, optional float drawflag)
  { "drawrawstring",
    PF_CSQC(PF_cl_drawrawstring),
    321,
    qcextension_t{} }, // float(vector position, string text, vector size,
                       // vector rgb, float alpha, optional float drawflag)
  { "drawpic",
    PF_CSQC(PF_cl_drawpic),
    322,
    qcextension_t{} }, // float(vector position, string pic, vector size, vector
                       // rgb, float alpha, optional float drawflag)
  { "drawfill",
    PF_CSQC(PF_cl_drawfill),
    323,
    qcextension_t{} }, // float(vector position, vector size, vector rgb, float
                       // alpha, optional float drawflag)
  { "drawsetcliparea",
    PF_CSQC(PF_cl_drawsetclip),
    324,
    qcextension_t{} }, // void(float x, float y, float width, float height)
  { "drawresetcliparea",
    PF_CSQC(PF_cl_drawresetclip),
    325,
    qcextension_t{} }, // void(void)
  { "drawstring",
    PF_CSQC(PF_cl_drawstring),
    326,
    qcextension_t{} }, // float(vector position, string text, vector size,
                       // vector rgb, float alpha, float drawflag)
  { "stringwidth",
    PF_CSQC(PF_cl_stringwidth),
    327,
    qcextension_t{} }, // float(string text, float usecolours, vector
                       // fontsize='8 8')
  { "drawsubpic",
    PF_CSQC(PF_cl_drawsubpic),
    328,
    qcextension_t{} }, // void(vector pos, vector sz, string pic, vector srcpos,
  ...
  { "getstati",
    ...
    330,
    qcextension_t{} }, // #define getstati_punf(stnum)
                       // (float)(__variant)getstati(stnum)\nint(float stnum)
  { "getstatf",
    PF_CSQC(PF_cl_getstat_float),
    331,
    qcextension_t{} }, // #define getstatbits getstatf\nfloat(float stnum,
                       // optional float firstbit, optional float bitcount)
  { "getstats",
    PF_CSQC(PF_cl_getstat_string),
    332,
    qcextension_t{} }, // string(float stnum)

  { "getplayerkeyvalue",
    PF_CSQC(PF_cl_playerkey_s),
    348,
    qcextension_t{} }, // string(float playernum, string keyname)
  { "getplayerkeyfloat",
    PF_CSQC(PF_cl_playerkey_f),
    0,
    qcextension_t{} }, // float(float playernum, string keyname,
                       // optional float assumevalue)

  { "registercommand",
    PF_CSQC(PF_cl_registercommand),
    352,
    qcextension_t{} }, // void(string cmdname)

  { "vectorvectors",
    PF_BOTH(PF_vectorvectors),
    432,
    DP_QC_VECTORVECTORS }, // void(vector dir)

  { "clientcommand",
    PF_SSQC(PF_clientcommand),
    440,
    KRIMZON_SV_PARSECLIENTCOMMAND }, // void(entity e, string s)
  { "tokenize",
    PF_BOTH(PF_Tokenize),
    441,
    KRIMZON_SV_PARSECLIENTCOMMAND }, // float(string s)
  { "argv",
    PF_BOTH(PF_ArgV),
    442,
    KRIMZON_SV_PARSECLIENTCOMMAND },                // string(float n)
  { "argc", PF_BOTH(PF_ArgC), 0, qcextension_t{} }, // float()

  { "asin",
    PF_BOTH(PF_asin),
    471,
    DP_QC_ASINACOSATANATAN2TAN }, // float(float s)
  { "acos",
    PF_BOTH(PF_acos),
    472,
    DP_QC_ASINACOSATANATAN2TAN }, // float(float c)
  { "atan",
    PF_BOTH(PF_atan),
    473,
    DP_QC_ASINACOSATANATAN2TAN }, // float(float t)
  { "atan2",
    PF_BOTH(PF_atan2),
    474,
    DP_QC_ASINACOSATANATAN2TAN }, // float(float c, float s)
  { "tan", PF_BOTH(PF_tan), 475, DP_QC_ASINACOSATANATAN2TAN }, // float(float a)

  { "tokenize_console",
    PF_BOTH(PF_tokenize_console),
    514,
    DP_QC_TOKENIZE_CONSOLE }, // float(string str)

  { "sprintf",
    PF_BOTH(PF_sprintf),
    627,
    DP_QC_SPRINTF }, // string(string fmt, ...)
};
int pr_numbuiltindefs = Q_COUNTOF(pr_builtindefs);

COMPILE_TIME_ASSERT(builtin_buffer_size,
                    countof(pr_builtindefs) + 1 <= MAX_BUILTINS);
