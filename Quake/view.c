...
cvar_t scr_ofsx = { "scr_ofsx", "0", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t scr_ofsy = { "scr_ofsy", "0", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t scr_ofsz = { "scr_ofsz", "0", CVAR_NONE, 0, 0, 0, 0, 0 };

cvar_t cl_rollspeed = { "cl_rollspeed", "200", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t cl_rollangle = { "cl_rollangle", "2.0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };

cvar_t cl_bob = { "cl_bob", "0.02", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
cvar_t cl_bobcycle = { "cl_bobcycle", "0.6", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t cl_bobup = { "cl_bobup", "0.5", CVAR_NONE, 0, 0, 0, 0, 0 };

cvar_t v_kicktime = { "v_kicktime", "0.5", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_kickroll = { "v_kickroll", "0.6", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_kickpitch = { "v_kickpitch", "0.6", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_gunkick = {
  "v_gunkick", "2", CVAR_ARCHIVE, 0, 0, 0, 0, 0
}; // johnfitz

cvar_t v_iyaw_cycle = { "v_iyaw_cycle", "2", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_iroll_cycle = { "v_iroll_cycle", "0.5", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_ipitch_cycle = { "v_ipitch_cycle", "1", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_iyaw_level = { "v_iyaw_level", "0.3", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_iroll_level = { "v_iroll_level", "0.1", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_ipitch_level = { "v_ipitch_level", "0.3", CVAR_NONE, 0, 0, 0, 0, 0 };

cvar_t v_idlescale = { "v_idlescale", "0", CVAR_NONE, 0, 0, 0, 0, 0 };

cvar_t crosshair = { "crosshair", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0 };
char crosshair_char = '\0';

cvar_t gl_cshiftpercent = {
  "gl_cshiftpercent", "100", CVAR_NONE, 0, 0, 0, 0, 0
};
cvar_t gl_cshiftpercent_contents = {
  "gl_cshiftpercent_contents", "100", CVAR_NONE, 0, 0, 0, 0, 0
}; // QuakeSpasm
cvar_t gl_cshiftpercent_damage = {
  "gl_cshiftpercent_damage", "100", CVAR_NONE, 0, 0, 0, 0, 0
}; // QuakeSpasm
cvar_t gl_cshiftpercent_bonus = {
  "gl_cshiftpercent_bonus", "100", CVAR_NONE, 0, 0, 0, 0, 0
}; // QuakeSpasm
cvar_t gl_cshiftpercent_powerup = {
  "gl_cshiftpercent_powerup", "100", CVAR_NONE, 0, 0, 0, 0, 0
}; // QuakeSpasm

cvar_t r_viewmodel_quake = {
  "r_viewmodel_quake", "0", CVAR_ARCHIVE, 0, 0, 0, 0, 0
};
...
cvar_t v_centermove = { "v_centermove", "0.15", CVAR_NONE, 0, 0, 0, 0, 0 };
cvar_t v_centerspeed = { "v_centerspeed", "500", CVAR_NONE, 0, 0, 0, 0, 0 };
...
V_UpdateBlend(void)
{
  int i, j;
  float frametime =
    fabs(cl.time - cl.oldtime); // time can go backwards when rewinding demos
...
V_PolyBlend(void)
{
  ...
  if (softemu) {
    // If softemu is active then it's generally best to perform color shifting
    // through palette remapping, which is consistent with the old software
    // renderers, and also avoids palettization artifacts (e.g. underwater).

    // However, a black v_cshift is sometimes used as a background for
    // centerprint text to make it easier to read (e.g. books in Arcane
    // Dimensions). Applying the effect at the end of the frame would defeat its
    // purpose (and make the UI harder to use), so we detect this case and apply
    // the effect here.
    int maxcolor =
      q_max(cshift_empty.destcolor[0],
            q_max(cshift_empty.destcolor[1], cshift_empty.destcolor[2]));
    if (!cshift_empty.percent || maxcolor > 0)
      return;
  } else {
    // If we're already rendering to an intermediate FBO (for warp/scale/MSAA)
    // then we can apply the color blending when blitting the intermediate
    // texture (to save the memory bandwidth an extra full-screen alpha blend
    // would consume)
  ...
  v_blend[3] =
    0.f; // make sure this doesn't get applied again later in the pipeline
}
...
V_CalcRefdef(void)
{
  ...
  if (v_gunkick.value == 2) // lerped kick
  {
    ...
    r_refdef.viewangles[0] +=
      v_punchangles[1][0] +
      (v_punchangles[0][0] - v_punchangles[1][0]) * punchblend;
    r_refdef.viewangles[1] +=
      v_punchangles[1][1] +
      (v_punchangles[0][1] - v_punchangles[1][1]) * punchblend;
    r_refdef.viewangles[2] +=
      v_punchangles[1][2] +
      (v_punchangles[0][2] - v_punchangles[1][2]) * punchblend;
  }
  // johnfitz
...
*/
static void
V_Crosshair_f(cvar_t* var)
{
  int i;
  if (var->string && *var->string && sscanf(var->string, "%d", &i) != 1) {
    Cvar_SetValueQuick(var,
                       -(float)(unsigned char)*var->string); // custom character
    return;
  }
...
V_Init(void)
{
  ...
  Cvar_SetCallback(&crosshair, (cvarcallback_t)V_Crosshair_f);
