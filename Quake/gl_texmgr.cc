...
unsigned int
  d_8to24table_opaque[256]; // standard palette with alpha 255 for all colors
unsigned int d_8to24table[256];         // standard palette, 255 is transparent
unsigned int d_8to24table_fbright[256]; // fullbright palette, 0-223 are black
                                        // (for additive blending)
unsigned int
  d_8to24table_alphabright[256]; // palette with lighting mask in alpha channel
                                 // (0=fullbright, 255=lit)
unsigned int
  d_8to24table_fbright_fence[256]; // fullbright palette, for fence textures
unsigned int d_8to24table_nobright[256]; // nobright palette, 224-255 are black
                                         // (for additive blending)
unsigned int
  d_8to24table_nobright_fence[256]; // nobright palette, for fence textures
unsigned int
  d_8to24table_conchars[256]; // conchars palette, 0 and 255 are transparent

uint32_t is_fullbright[256 / 32];

static void
GL_DeleteTexture(gltexture_t* texture);
...
*/
static void
TexMgr_DescribeTextureModes_f(void)
{
  int i;
  for (i = 0; i < NUM_GLMODES; i++)
    Con_SafePrintf("   %2i: %s\n", i + 1, glmodes[i].name);
...
TexMgr_SetFilterModes(gltexture_t* glt)
{
  ...
  if (glt->flags & TEXPREF_NEAREST) {
    ...
    glTexParameterf(
      glt->target, GL_TEXTURE_MAG_FILTER, glmodes[gl_texfilter.mode].magfilter);
    glTexParameterf(
      glt->target, GL_TEXTURE_MIN_FILTER, glmodes[gl_texfilter.mode].minfilter);
    glTexParameterf(
      glt->target, GL_TEXTURE_MAX_ANISOTROPY_EXT, gl_texfilter.anisotropy);
    glTexParameterf(glt->target, GL_TEXTURE_LOD_BIAS, gl_texfilter.lodbias);
  } else {
    glTexParameterf(
      glt->target, GL_TEXTURE_MAG_FILTER, glmodes[gl_texfilter.mode].magfilter);
    glTexParameterf(
      glt->target, GL_TEXTURE_MIN_FILTER, glmodes[gl_texfilter.mode].magfilter);
  }
...
*/
static void
TexMgr_Imagelist_Completion_f(const char* partial)
{
  gltexture_t* glt;
...
TexMgr_Imagelist_f(void)
{
  ...
  for (glt = active_gltextures; glt; glt = glt->next) {
    ...
    unsigned int layers =
      glt->flags & TEXPREF_CUBEMAP ? glt->depth * 6 : glt->depth;
...
TexMgr_Imagedump_f(void)
{
  ...
  for (glt = active_gltextures; glt; glt = glt->next) {
    ...
    if (!buffer)
      Sys_Error("TexMgr_Imagedump_f: out of memory (%dx%dx%d %d bpp)",
                glt->width,
                glt->height,
                glt->depth,
                channels * 8);
    ...
    if (glt->flags & TEXPREF_CUBEMAP) {
      ...
      for (i = 0; i < 6; i++) {
        q_snprintf(
          tganame, sizeof(tganame), "imagedump/%s%s.tga", tempname, suf[i]);
        glGetTexImage(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
                      0,
                      format,
                      GL_UNSIGNED_BYTE,
                      buffer);
        Image_WriteTGA(tganame,
                       buffer,
                       glt->width,
                       glt->height * glt->depth,
                       channels * 8,
                       true);
      }
    } else {
...
*/
static qboolean
TexMgr_CanCompress(gltexture_t* glt)
{
  return glt->source_format != SRC_LIGHTMAP &&
         (glt->flags & TEXPREF_PERSIST) == 0;
}
...
*/
void
TexMgr_CompressTextures_f(cvar_t* var)
{
...
*/
gltexture_t*
TexMgr_FindTexture(qmodel_t* owner, const char* name)
{
  gltexture_t* glt;
...
*/
void
TexMgr_FreeTexture(gltexture_t* kill)
{
  gltexture_t* glt;
...
*/
void
TexMgr_FreeTextures(unsigned int flags, unsigned int mask)
{
...
*/
void
TexMgr_FreeTexturesForOwner(qmodel_t* owner)
{
  gltexture_t *glt, *next;
...
*/
void
TexMgr_DeleteTextureObjects(void)
{
  gltexture_t* glt;
...
static void
SetColor(uint32_t* dst, byte r, byte g, byte b, byte a)
{
  ((byte*)dst)[0] = r;
  ((byte*)dst)[1] = g;
  ((byte*)dst)[2] = b;
...
{
  TexMgr_FreeTextures(
    0, TEXPREF_PERSIST); // deletes all textures where TEXPREF_PERSIST is unset
  TexMgr_LoadPalette();
}
...
TexMgr_Init(void)
{
  ...
  static byte whitetexture_data[16] = {
    255, 255, 255, 255, 255, 255, 255, 255,
    255, 255, 255, 255, 255, 255, 255, 255
  }; // white
  static byte greytexture_data[16] = {
    127, 127, 127, 255, 127, 127, 127, 255,
    127, 127, 127, 255, 127, 127, 127, 255
  }; // 50% grey
  static byte blacktexture_data[16] = { 0, 0, 0, 0, 0, 0, 0, 0,
  ...
  Cvar_SetCallback(&gl_texturemode, (cvarcallback_t)TexMgr_TextureMode_f);
  Cvar_SetCompletion(&gl_texturemode,
                     (cvarcompletion_t)TexMgr_TextureMode_Completion_f);
  Cvar_RegisterVariable(&gl_lodbias);
  Cvar_SetCallback(&gl_lodbias, (cvarcallback_t)TexMgr_LodBias_f);
  Cvar_RegisterVariable(&r_softemu);
  Cvar_SetCallback(&r_softemu, (cvarcallback_t)TexMgr_SoftEmu_f);
  ...
  notexture = TexMgr_LoadImage(NULL,
                               "notexture",
                               2,
                               2,
                               SRC_RGBA,
                               ...
                               TEXPREF_NEAREST | TEXPREF_PERSIST |
                                 TEXPREF_NOPICMIP | TEXPREF_BINDLESS);
  nulltexture = TexMgr_LoadImage(NULL,
                                 "nulltexture",
                                 2,
                                 2,
                                 SRC_RGBA,
                                 ...
                                 TEXPREF_NEAREST | TEXPREF_PERSIST |
                                   TEXPREF_NOPICMIP | TEXPREF_BINDLESS);
  whitetexture = TexMgr_LoadImage(NULL,
                                  "whitetexture",
                                  2,
                                  2,
                                  SRC_RGBA,
                                  whitetexture_data,
                                  "",
                                  (src_offset_t)whitetexture_data,
                                  TEXPREF_NEAREST | TEXPREF_PERSIST |
                                    TEXPREF_NOPICMIP | TEXPREF_BINDLESS);
  greytexture = TexMgr_LoadImage(NULL,
                                 "greytexture",
                                 2,
                                 2,
                                 SRC_RGBA,
                                 greytexture_data,
                                 "",
                                 (src_offset_t)greytexture_data,
                                 TEXPREF_NEAREST | TEXPREF_PERSIST |
                                   TEXPREF_NOPICMIP | TEXPREF_BINDLESS);
  blacktexture = TexMgr_LoadImage(NULL,
                                  "blacktexture",
                                  2,
                                  2,
                                  SRC_RGBA,
                                  blacktexture_data,
                                  "",
                                  (src_offset_t)blacktexture_data,
                                  TEXPREF_NEAREST | TEXPREF_PERSIST |
                                    TEXPREF_NOPICMIP | TEXPREF_BINDLESS);
...
*/
static unsigned*
TexMgr_MipMapW(unsigned* data, int width, int height, int depth)
{
  int i, size;
  byte *out, *in;
...
*/
static unsigned*
TexMgr_MipMapH(unsigned* data, int width, int height, int depth)
{
  int i, j;
  byte *out, *in;
...
*/
static void
TexMgr_AlphaEdgeFix(byte* data, int width, int height)
{
...
*/
static void
TexMgr_PadEdgeFixW(byte* data, int width, int height)
{
  byte *src, *dst;
  int i, padw, padh;
...
*/
static void
TexMgr_PadEdgeFixH(byte* data, int width, int height)
{
  byte *src, *dst;
  int i, padw, padh;
...
*/
static unsigned*
TexMgr_8to32(byte* in, int pixels, unsigned int* usepal)
{
  int i;
  unsigned *out, *data;
...
*/
static byte*
TexMgr_PadImageW(byte* in, int width, int height, byte padbyte)
{
  int i, j, outwidth;
  byte *out, *data;
...
*/
static byte*
TexMgr_PadImageH(byte* in, int width, int height, byte padbyte)
{
  int i, srcpix, dstpix;
  byte *data, *out;
...
GL_TexImage(gltexture_t* glt,
            ...
            const GLvoid* pixels)
{
  const GLvoid** images = (const GLvoid**)
    pixels; // for arrays/cubemaps "pixels" is actually an array of pointers
  unsigned int i;

  switch (glt->target) {
    case GL_TEXTURE_2D_ARRAY:
      GL_TexImage3DFunc(glt->target,
                        level,
                        internalformat,
                        width,
                        height,
      ...
      for (i = 0; i < glt->depth; i++)
        GL_TexSubImage3DFunc(glt->target,
                             level,
                             0,
                             0,
                             i,
    ...
    default:
      Sys_Error(
        "GL_TexImage: unknown target %d for %s", glt->target, glt->name);
      break;
  }
}
...
*/
static void
TexMgr_LoadImage32(gltexture_t* glt, unsigned* data)
{
  ...
  internalformat = (glt->flags & TEXPREF_HASALPHA) ? glformats[compress].alpha
                                                   : glformats[compress].solid;
...
*/
static void
TexMgr_LoadImage8(gltexture_t* glt, byte* data)
{
  ...
  if (glt->flags & TEXPREF_ALPHABRIGHT) {
    usepal =
      gl_fullbrights.value ? d_8to24table_alphabright : d_8to24table_opaque;
    padbyte = 0;
  ...
  // convert to 32bit
  data =
    (byte*)TexMgr_8to32(data, glt->width * glt->height * glt->depth, usepal);
...
*/
static void
TexMgr_LoadLightmap(gltexture_t* glt, byte* data)
{
  // upload it
  glt->compression = 1;
  GL_Bind(GL_TEXTURE0, glt);
...
*/
gltexture_t*
TexMgr_LoadImageEx(qmodel_t* owner,
                   const char* name,
                   int width,
                   int height,
                   int depth,
...
*/
gltexture_t*
TexMgr_LoadImage(qmodel_t* owner,
                 const char* name,
                 int width,
                 int height,
                 enum srcformat format,
                 ...
                 unsigned flags)
{
  return TexMgr_LoadImageEx(owner,
                            name,
                            width,
                            height,
                            1,
...
*/
void
TexMgr_ReloadImage(gltexture_t* glt, int shirt, int pants)
{
  ...
  if (glt->source_file[0] && glt->source_offset) {
    ...
    data = Image_LoadImage(glt->source_file,
                           (int*)&glt->source_width,
                           (int*)&glt->source_height,
                           &fmt); // simple file
  } else if (!glt->source_file[0] && glt->source_offset) {
  ...
  if (fmt != glt->source_format)
    Sys_Error("Texture %s changed format from %i to %i",
              glt->name,
              glt->source_format,
              fmt);
...
*/
void
TexMgr_ReloadNobrightImages(void)
{
  gltexture_t* glt;
...
*/
static void
GL_SelectTexture(GLenum texunit)
{
...
*/
qboolean
GL_Bind(GLenum texunit, gltexture_t* texture)
{
  if (!texture)
    texture = nulltexture;
...
*/
void
GL_BindTextures(GLuint first, GLsizei count, gltexture_t** textures)
{
  GLuint handles[8];
  GLsizei i;
...
*/
qboolean
GL_BindNative(GLenum texunit, GLenum type, GLuint handle)
{
...
*/
static void
GLPalette_InvalidateRemapped(void)
{
  int i;
  cached_gamma = -1.f;
  cached_contrast = -1.f;
...
*/
void
GLPalette_DeleteResources(void)
{
...
*/
void
GLPalette_CreateResources(void)
{
  int i;
  ...
  for (i = 0; i < 2; i++)
    gl_palette_buffer[i] =
      GL_CreateBuffer(GL_SHADER_STORAGE_BUFFER,
                      GL_STATIC_DRAW,
...
*/
void
GLPalette_UpdateLookupTable(void)
{
  softemu_metric_t metric;
  int i;

  if (r_softemu_metric.value < 0.f) {
    qboolean oklab = softemu != SOFTEMU_BANDED || cls.state != ca_connected ||
                     cls.signon != SIGNONS || !cl.worldmodel ||
  ...
  if (cached_softemu_metric == metric &&
      !memcmp(cached_palette, d_8to24table, sizeof(cached_palette)))
    return;
  ...
  GL_BindImageTextureFunc(
    0, gl_palette_lut, 0, GL_TRUE, 0, GL_WRITE_ONLY, GL_R8UI);
  GL_BindBufferRange(
    GL_SHADER_STORAGE_BUFFER, 0, gl_palette_buffer[0], 0, 256 * sizeof(GLuint));
  GL_BufferSubDataFunc(
    GL_SHADER_STORAGE_BUFFER, 0, 256 * sizeof(GLuint), d_8to24table);
...
GLPalette_Postprocess(void)
{
  ...
  if (vid_gamma.value == 1.f && vid_contrast.value == 1.f && blend[3] == 0.f)
    return 0;
  ...
  GL_BindBufferRange(
    GL_SHADER_STORAGE_BUFFER, 0, gl_palette_buffer[0], 0, 256 * sizeof(GLuint));
  GL_BindBufferRange(
    GL_SHADER_STORAGE_BUFFER, 1, gl_palette_buffer[1], 0, 256 * sizeof(GLuint));
