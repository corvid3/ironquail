...
CL_ParseServerInfo(void)
{
  ...
  cl.scores =
    (scoreboard_t*)Hunk_AllocName(cl.maxclients * sizeof(*cl.scores), "scores");
  ...
  warn_about_nehahra_protocol = true; // johnfitz -- warn about nehahra protocol
                                      // hack once per server connection
...
*/
void
CL_ParseBaseline(entity_t* ent, int version) // johnfitz -- added argument
{
  int i;
  int bits; // johnfitz
  ...
  ent->baseline.alpha = (bits & B_ALPHA)
                          ? MSG_ReadByte()
...
*/
void
CL_ParseStatic(int version) // johnfitz -- added a parameter
{
  entity_t* ent;
  int i;
...
*/
void
CL_ParseStaticSound(int version) // johnfitz -- added argument
{
  vec3_t org;
...
static void
CL_ParseStuffText(const char* msg)
{
  const char* str;
      ...
      if (*cl.stuffcmdbuf && net_message.cursize < 512)
        CL_ParseStuffText(
          "\n"); // there's a few mods that forget to write \ns, that then fuck
                 // up other things too. So make sure it gets flushed to the
                 // cbuf. the cursize check is to reduce backbuffer overflows
                 // that would give a false positive.
