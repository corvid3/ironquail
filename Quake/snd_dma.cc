...
void
S_SetUnderwaterIntensity(float intensity);
...
S_Init(void)
{
  ...
  Cvar_SetCallback(&sfxvolume, (cvarcallback_t)SND_Callback_sfxvolume);
  Cvar_SetCallback(&snd_filterquality,
                   (cvarcallback_t)SND_Callback_snd_filterquality);
             ...
             float attenuation)
{
  ...
  if (entnum > 0 && entchannel > 0 && target_chan->entnum == entnum &&
  ...
  // previous sound playing on the same ent/channel so that when we do rewind
  // past this frame we start playing it instead
  if (cls.demoplayback && cls.demospeed > 0.f && sc->loopstart != -1)
    CL_AddDemoRewindSound(
      entnum, entchannel, old_sfx, old_origin, old_vol, old_atten);
...
*/
static float
S_UnderwaterIntensityForContents(int contents)
{
  switch (contents) {
    case CONTENTS_WATER:
    case CONTENTS_SLIME:
...
S_SoundList(void)
{
  ...
  Con_Printf("%i sounds, %i bytes\n", num_sfx, total); // johnfitz -- added
                                                       // count
}

void
S_LocalSound(const char* name)
{
  sfx_t* sfx;
