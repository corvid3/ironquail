...
*/
void
Draw_FillClampTexels(const byte* src,
                     int w,
                     int h,
                     int srcpitch,
                     byte* dst,
...
*/
void
Scrap_FillTexels(int x, int y, int w, int h, const byte* data)
{
...
*/
qboolean
Scrap_Compatible(unsigned int texflags)
{
...
*/
qpic_t*
Draw_PicFromWad2(const char* name, unsigned int texflags)
{
  int i, x, y;
  cachepic_t* pic;
  qpic_t* p;
  ...
  if ((size_t)info->size > sizeof(int) * 2 + p->width * p->height)
    Con_DPrintf(
      "Draw_PicFromWad: pic \"%s\" over-sized (%u*%u requires only %u bytes)\n",
      name,
      p->width,
      p->height,
      8 + p->width * p->height);
  ...
  if (Scrap_Compatible(texflags) &&
      Scrap_AllocBlock(p->width, p->height, &x, &y)) {
    ...
    q_snprintf(
      texturename, sizeof(texturename), "%s:%s", WADFILENAME, name); // johnfitz
    ...
    gl.sl = 0;
    gl.sh = (texflags & TEXPREF_PAD)
              ? (float)p->width / (float)TexMgr_PadConditional(p->width)
              : 1; // johnfitz
    gl.tl = 0;
    gl.th = (texflags & TEXPREF_PAD)
              ? (float)p->height / (float)TexMgr_PadConditional(p->height)
              : 1; // johnfitz
  }
...
Draw_TryCachePic(q_strview path, unsigned int texflags)
{
  ...
  if (Scrap_Compatible(texflags) &&
      Scrap_AllocBlock(dat->width, dat->height, &x, &y)) {
...
{
  qpic_t* pic = Draw_TryCachePic(
    path, TEXPREF_ALPHA | TEXPREF_PAD | TEXPREF_NOPICMIP | TEXPREF_CLAMP);
  if (!pic)
...
Draw_LoadPics(void)
{
  ...
  for (i = 0; i < 256; i++) {
    row = i / 16;
    col = i % 16;
    Draw_FillClampTexels(data + row * (16 * 8 * 8) + col * 8,
                         8,
                         8,
                         16 * 8,
                         char_texture_data + row * (16 * 10 * 10) + col * 10,
                         16 * 10);
  }
  char_texture = TexMgr_LoadImage(NULL,
                                  WADFILENAME ":conchars",
                                  16 * 10,
                                  16 * 10,
                                  SRC_INDEXED,
                                  char_texture_data,
                                  "",
                                  (src_offset_t)char_texture_data,
                                  TEXPREF_ALPHA | TEXPREF_NEAREST |
                                    TEXPREF_NOPICMIP | TEXPREF_CONCHARS);

  draw_disc = Draw_PicFromWad("disc");
  draw_backtile = Draw_PicFromWad2(
    "backtile",
    TEXPREF_ALPHA |
      TEXPREF_NOPICMIP); // no pad flag to force separate allocation
}
...
*/
static void
Draw_CreateWinQuakeMenuBgTex(void)
{
  ...
  winquakemenubg = TexMgr_LoadImage(NULL,
                                    "winquakemenubg",
                                    4,
                                    2,
                                    SRC_RGBA,
                                    (byte*)winquakemenubg_pixels,
                                    "",
                                    (src_offset_t)winquakemenubg_pixels,
                                    TEXPREF_ALPHA | TEXPREF_NEAREST |
                                      TEXPREF_PERSIST | TEXPREF_NOPICMIP);
}
...
Draw_Flush(void)
{
  ...
  GL_SetState(glcanvas.blendmode | GLS_NO_ZTEST | GLS_NO_ZWRITE |
              GLS_CULL_NONE | GLS_ATTRIBS(3));
  GL_Bind(GL_TEXTURE0, glcanvas.texture);

  GL_Upload(GL_ARRAY_BUFFER,
            batchverts,
            sizeof(batchverts[0]) * 4 * numbatchquads,
            &buf,
            &ofs);
  ...
  GL_Upload(GL_ELEMENT_ARRAY_BUFFER,
            batchindices,
            sizeof(batchindices[0]) * 6 * numbatchquads,
            &buf,
            &ofs);
...
*/
static void
Draw_SetTexture(gltexture_t* tex)
{
...
*/
static void
Draw_SetBlending(unsigned blend)
{
...
*/
void
GL_SetCanvasColor(float r, float g, float b, float a)
{
...
*/
static void
Draw_SetVertex(guivertex_t* v, float x, float y, float s, float t)
{
...
*/
void
Draw_CharacterQuadEx(float x, float y, float dimx, float dimy, char num)
{
  int row, col;
...
*/
void
Draw_CharacterQuad(int x, int y, char num)
{
...
*/
void
Draw_CharacterEx(float x, float y, float dimx, float dimy, int num)
{
...
*/
void
Draw_StringEx(float x, float y, float dim, const char* str)
{
            ...
            float alpha)
{
  ...
  verts = Draw_AllocQuad();
  Draw_SetVertex(
    verts++, x, y, LERP(gl->sl, gl->sh, s1), LERP(gl->tl, gl->th, t1));
  Draw_SetVertex(
    verts++, x + w, y, LERP(gl->sl, gl->sh, s2), LERP(gl->tl, gl->th, t1));
  Draw_SetVertex(
    verts++, x + w, y + h, LERP(gl->sl, gl->sh, s2), LERP(gl->tl, gl->th, t2));
  Draw_SetVertex(
    verts++, x, y + h, LERP(gl->sl, gl->sh, s1), LERP(gl->tl, gl->th, t2));
...
*/
void
Draw_TransPicTranslate(int x, int y, qpic_t* pic, int top, int bottom)
{
  static int oldtop = -2;
...
Draw_TileClear(int x, int y, int w, int h)
{
  ...
  uvscale = 1.f / 64.f /
            CLAMP(1.0f,
                  scr_sbarscale.value,
                  (float)vid.guiwidth / 320.0f); // use sbar scale
  uvbias = -vid.guiheight * uvscale;             // bottom-aligned tiles

  verts = Draw_AllocQuad();
  Draw_SetVertex(verts++,
                 x * scalex,
                 y * scaley,
                 x * scalex * uvscale,
                 y * scaley * uvscale + uvbias);
  Draw_SetVertex(verts++,
                 (x + w) * scalex,
                 y * scaley,
                 (x + w) * scalex * uvscale,
...
*/
void
Draw_FillEx(float x, float y, float w, float h, const float* rgb, float alpha)
{
  guivertex_t* verts;
...
{
  byte* pal =
    (byte*)d_8to24table; // johnfitz -- use d_8to24table instead of host_basepal
  float rgb[3];
...
*/
void
Draw_SetClipRect(float x, float y, float width, float height)
{
  float x2 = x + width;
  float y2 = y + height;
                ...
                drawtransform_t* out)
{
  ...
  out->offset[1] =
    (scrheight - height * scaley) * aligny / scrheight * -2.f + 1.f;

  // shift projection by a fraction of a pixel to avoid interpolation artifacts
  // at certain scales
...
*/
void
Draw_GetCanvasTransform(canvastype type, drawtransform_t* transform)
{
  ...
  switch (type) {
    case CANVAS_DEFAULT:
      Draw_Transform(vid.guiwidth,
                     vid.guiheight,
                     1.f,
                     CANVAS_ALIGN_CENTERX,
                     CANVAS_ALIGN_CENTERY,
                     transform);
      break;
    case CANVAS_CONSOLE:
      s = (float)vid.guiwidth / vid.conwidth; // use console scale
      s2 = (float)vid.guiheight / vid.conheight;
      Draw_Transform2(vid.conwidth,
                      vid.conheight,
                      s,
                      s2,
                      CANVAS_ALIGN_CENTERX,
    ...
    case CANVAS_MENU:
      ...
      s = CLAMP(1.0f, scr_menuscale.value, s);
      Draw_Transform(
        320, 200, s, CANVAS_ALIGN_CENTERX, CANVAS_ALIGN_CENTERY, transform);
      break;
    case CANVAS_CSQC:
      s = CLAMP(1.0f, scr_sbarscale.value, vid.guiwidth / 320.0f);
      Draw_Transform(vid.guiwidth / s,
                     vid.guiheight / s,
                     s,
                     CANVAS_ALIGN_CENTERX,
                     CANVAS_ALIGN_CENTERY,
    ...
    case CANVAS_SBAR:
      ...
      if (cl.gametype == GAME_DEATHMATCH &&
          (hudstyle == HUD_CLASSIC || hudstyle == HUD_QUAKEWORLD))
        Draw_Transform(
          320, 48, s, CANVAS_ALIGN_LEFT, CANVAS_ALIGN_BOTTOM, transform);
      else
        Draw_Transform(
          320, 48, s, CANVAS_ALIGN_CENTERX, CANVAS_ALIGN_BOTTOM, transform);
      break;
    case CANVAS_SBAR_QW_INV:
      s = CLAMP(1.0f, scr_sbarscale.value, (float)vid.guiheight / 240.0f);
      Draw_Transform(
        48, 48, s, CANVAS_ALIGN_RIGHT, CANVAS_ALIGN_BOTTOM, transform);
      break;
    case CANVAS_SBAR2:
    ...
    case CANVAS_CROSSHAIR: // 0,0 is center of viewport
      ...
      Draw_Transform(vid.guiwidth / s / 2,
                     vid.guiheight / s / 2,
                     s,
                     CANVAS_ALIGN_LEFT,
                     CANVAS_ALIGN_BOTTOM,
      ...
      transform->offset[1] +=
        1.f - ((scr_vrect.y + scr_vrect.height / 2) * 2 / (float)glheight);
      break;
    case CANVAS_BOTTOMLEFT:                   // used by devstats
      s = (float)vid.guiwidth / vid.conwidth; // use console scale
      Draw_Transform(
        320, 200, s, CANVAS_ALIGN_LEFT, CANVAS_ALIGN_BOTTOM, transform);
      break;
    case CANVAS_BOTTOMRIGHT:                  // used by fps/clock
      s = (float)vid.guiwidth / vid.conwidth; // use console scale
      Draw_Transform(
        320, 200, s, CANVAS_ALIGN_RIGHT, CANVAS_ALIGN_BOTTOM, transform);
      break;
    case CANVAS_TOPRIGHT:                     // used by disc
      s = (float)vid.guiwidth / vid.conwidth; // use console scale
      Draw_Transform(
        320, 200, s, CANVAS_ALIGN_RIGHT, CANVAS_ALIGN_TOP, transform);
      break;
    default:
...
*/
void
Draw_GetTransformBounds(const drawtransform_t* transform,
                        float* left,
                        float* top,
                        float* right,
                        float* bottom)
...
*/
void
GL_SetCanvas(canvastype newcanvas)
{
  ...
  Draw_GetTransformBounds(&glcanvas.transform,
                          &glcanvas.left,
                          &glcanvas.top,
                          &glcanvas.right,
                          &glcanvas.bottom);
