...
R_MarkVisSurfaces(byte* vis)
{
  ...
  COMPILE_TIME_ASSERT(vis_alignment_must_be_power_of_2,
                      (VIS_ALIGN & (VIS_ALIGN - 1)) == 0);
  COMPILE_TIME_ASSERT(vis_alignment_must_be_multiple_of_uint,
                      (VIS_ALIGN & 3) == 0);
  ...
  GL_BindBufferRange(
    GL_SHADER_STORAGE_BUFFER, 2, gl_bmodel_ibo, 0, gl_bmodel_ibo_size);
...
R_MarkSurfaces(void)
{
  ...
  for (i = 0; i < r_viewleaf->nummarksurfaces; i++)
    if (cl.worldmodel->surfaces[r_viewleaf->firstmarksurface[i]].flags &
        SURF_DRAWTURB)
...
*/
float
GL_WaterAlphaForEntityTextureType(entity_t* ent, textype_t type)
{
  float entalpha;
...
static bmodel_gpu_instance_t
  bmodel_instances[MAX_VISEDICTS + 1]; // +1 for worldspawn
static union
{
  struct
  {
...
*/
static void
R_InitBModelInstance(bmodel_gpu_instance_t* inst, entity_t* ent)
{
  vec3_t angles;
  float mat[16];
  ...
  R_EntityMatrix(mat,
                 ent->origin,
                 angles,
                 ent == &cl_entities[0] ? ENTSCALE_DEFAULT : ent->scale);

  MatrixTranspose4x3(mat, inst->world);

  inst->alpha =
    ent->alpha == ENTALPHA_DEFAULT ? -1.f : ENTALPHA_DECODE(ent->alpha);
...
*/
static void
R_ResetBModelCalls(GLuint program)
{
...
R_FlushBModelCalls(void)
{
  ...
  GL_ReserveDeviceMemory(GL_DRAW_INDIRECT_BUFFER,
                         sizeof(bmodel_draw_indirect_t) * num_bmodel_calls,
                         &cmdbuf,
                         &dstcmdofs);
  ...
  GL_BindBufferRange(GL_SHADER_STORAGE_BUFFER,
                     ...
                     gl_bmodel_indirect_buffer,
                     0,
                     gl_bmodel_indirect_buffer_size);
  GL_BindBufferRange(GL_SHADER_STORAGE_BUFFER,
                     6,
                     cmdbuf,
                     dstcmdofs,
  ...
  if (gl_bindless_able) {
    GL_Upload(GL_SHADER_STORAGE_BUFFER,
              bmodel_calls.bindless.params,
              sizeof(bmodel_calls.bindless.params[0]) * num_bmodel_calls,
              &buf,
              &ofs);
    GL_BindBufferRange(GL_SHADER_STORAGE_BUFFER,
                       1,
                ...
                qboolean zfix)
{
  ...
  if (gl_bindless_able) {
    bmodel_bindless_gpu_call_t* call =
      &bmodel_calls.bindless.params[num_bmodel_calls];
    call->flags = flags;
    call->alpha = alpha;
    ...
    bmodel_bound_gpu_call_t* call =
      &bmodel_calls.bound.params[num_bmodel_calls];
  ...
  bmodel_call_remap[num_bmodel_calls].inst =
    first_instance * MAX_BMODEL_INSTANCES + (num_instances - 1);
...
*/
static void
R_DrawBrushModels_Real(entity_t** ents,
                       int count,
                       brushpass_t pass,
                       qboolean translucent)
{
  ...
  if (pass <= BP_ALPHATEST)
    GL_Bind(GL_TEXTURE2,
            r_fullbright_cheatsafe ? greytexture : lightmap_texture);
  else if (pass == BP_SKYCUBEMAP)
    GL_Bind(GL_TEXTURE2, skybox->cubemap);

  GL_Upload(GL_SHADER_STORAGE_BUFFER,
            bmodel_instances,
            sizeof(bmodel_instances[0]) * count,
            &buf,
            &ofs);
  GL_BindBufferRange(GL_SHADER_STORAGE_BUFFER,
                     2,
  ...
  for (i = 0, baseinst = 0; i < count; /**/) {
    ...
    for (numinst = 1;
         i < count && ents[i]->model == model && numinst < MAX_BMODEL_INSTANCES;
         i++)
      numinst +=
        (ents[i]->model->texofs[texend] - ents[i]->model->texofs[texbegin]) > 0;

    for (j = model->texofs[texbegin]; j < model->texofs[texend]; j++) {
      texture_t* t = model->textures[model->usedtextures[j]];
      R_AddBModelCall(model->firstcmd + j,
                      baseinst,
                      numinst,
...
*/
static qboolean
R_EntHasWater(entity_t* ent, qboolean translucent)
{
  int i;
  for (i = TEXTYPE_FIRSTLIQUID; i < TEXTYPE_LASTLIQUID + 1; i++) {
    int numtex = ent->model->texofs[i + 1] - ent->model->texofs[i];
    if (numtex && (GL_WaterAlphaForEntityTextureType(ent, (textype_t)i) <
                   1.f) == translucent)
      return true;
  }
  return false;
...
*/
void
R_DrawBrushModels_Water(entity_t** ents, int count, qboolean translucent)
{
  int i, j;
  int totalinst, baseinst;
  unsigned state;
  ...
  GL_Upload(GL_SHADER_STORAGE_BUFFER,
            bmodel_instances,
            sizeof(bmodel_instances[0]) * totalinst,
            &buf,
            &ofs);
  GL_BindBufferRange(GL_SHADER_STORAGE_BUFFER,
                     2,
  ...
  for (i = 0, baseinst = 0; i < count; /**/) {
    ...
    for (j = model->texofs[TEXTYPE_FIRSTLIQUID];
         j < model->texofs[TEXTYPE_LASTLIQUID + 1];
         j++) {
      texture_t* t = model->textures[model->usedtextures[j]];
...
*/
static uint32_t
R_GetBModelAlphaPasses(const entity_t* ent)
{
...
*/
static qboolean
R_CanMergeBModelAlphaPasses(uint32_t mask_a, uint32_t mask_b)
{
...
#define ALLOW_MERGE(a, b) (1 << ((a) | ((b) << 2)))
    ...
    MERGE_LUT = ALLOW_MERGE(0, 0) | ALLOW_MERGE(0, 1) | ALLOW_MERGE(0, 2) |
                ALLOW_MERGE(0, 3) |

                ALLOW_MERGE(1, 0) | ALLOW_MERGE(1, 1) | ALLOW_MERGE(1, 2) |
                ALLOW_MERGE(1, 3) |

                ALLOW_MERGE(2, 0) | ALLOW_MERGE(2, 2) |

                ALLOW_MERGE(3, 0) | ALLOW_MERGE(3, 2),
...
*/
void
R_DrawBrushModels(entity_t** ents, int count)
{
  qboolean translucent;
  if (!count)
    return;
  translucent =
    (ents[0] != &cl_entities[0]) && !ENTALPHA_OPAQUE(ents[0]->alpha);
...
*/
void
R_DrawBrushModels_SkyLayers(entity_t** ents, int count)
{
...
*/
void
R_DrawBrushModels_SkyCubemap(entity_t** ents, int count)
{
...
*/
void
R_DrawBrushModels_SkyStencil(entity_t** ents, int count)
{
...
*/
void
R_DrawBrushModels_ShowTris(entity_t** ents, int count)
{
