...
void
SV_AirAccelerate(float wishspeed, vec3_t wishveloc)
{
  int i;
      ...
      switch (ccmd) {
        ...
        case clc_stringcmd:
          ...
          if (q_strncasecmp(s, "spawn", 5) && q_strncasecmp(s, "begin", 5) &&
              ...
              qcvm->extfuncs
                .SV_ParseClientCommand) { // the spawn/begin/prespawn are
                                          // because of numerous mods that
                                          // disobey the rules.
            // at a minimum, we must be able to join the server, so that we can
            // see any sprints/bprints (because dprint sucks, yes there's proper
            // ways to deal with this, but moders don't always know them).
            client_t* ohc = host_client;
            qboolean checked =
              GetBit(qcvm->checked_ext, KRIMZON_SV_PARSECLIENTCOMMAND);
