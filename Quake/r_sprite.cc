...
*/
mspriteframe_t*
R_GetSpriteFrame(entity_t* currentent)
{
  msprite_t* psprite;
...
R_FlushSpriteInstances(void)
{
  ...
  if (showtris)
    GL_SetState(GLS_BLEND_OPAQUE | GLS_NO_ZWRITE | GLS_CULL_BACK |
                GLS_ATTRIBS(2));
  else
  ...
  GL_VertexAttribPointerFunc(0,
                             ...
                             GL_FALSE,
                             sizeof(batchverts[0]),
                             ofs + offsetof(spritevert_t, pos));
  GL_VertexAttribPointerFunc(1,
                             2,
                             GL_FLOAT,
                             GL_FALSE,
                             sizeof(batchverts[0]),
                             ofs + offsetof(spritevert_t, uv));
...
*/
static void
R_DrawSpriteModel_Real(entity_t* e, qboolean showtris)
{
  msprite_t* psprite;
  ...
  if (numbatchquads)
    if (numbatchquads == countof(batchverts) / 4 || batchmodel != e->model ||
  ...
  VectorMA(
    verts[1].pos, (frame->right - frame->left) * scale, s_right, verts[2].pos);
