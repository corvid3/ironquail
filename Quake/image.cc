...
static void
Buf_Free(stdio_buffer_t* buf)
{
  free(buf);
}

static inline int
Buf_GetC(stdio_buffer_t* buf)
{
  if (buf->pos >= buf->size) {
...
Image_LoadImage(const char* name, int* width, int* height, enum srcformat* fmt)
{
  ...
  for (i = 0; stbi_formats[i]; i++) {
    q_snprintf(
      loadfilename, sizeof(loadfilename), "%s.%s", name, stbi_formats[i]);
...
static byte*
CopyFlipped(const byte* data, int width, int height, int bpp)
{
  int y, rowsize;
  byte* flipped;
               ...
               qboolean upsidedown)
{
  ...
  error =
    stbi_write_jpg(pathname, width, height, bytes_per_pixel, flipped, quality);
  if (!upsidedown)
    free(flipped);
